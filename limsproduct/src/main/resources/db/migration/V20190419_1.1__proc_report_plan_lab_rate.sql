...
BEGIN
	INSERT INTO report_plan_lab_rate(term_id,term_name,lab_id,lab_name,user_num,course_num,course_hour_num,user_hour_num,flag)
	#教务不分批
	SELECT
	ta.term,
	st.term_name,
	lr.id,
	lr.lab_room_name,
	SUM((SELECT COUNT(DISTINCT student_number) FROM school_course_student WHERE course_detail_no=ta.detail_id)) AS '上课人数',
	...
	lr.id,
	lr.lab_room_name,
	SUM((SELECT COUNT(DISTINCT username) FROM timetable_group_students WHERE group_id=tag.group_id)) AS '上课人数',
	SUM((tasn.end_week-tasn.start_week+1)) AS '课次数',
	SUM(((tasn.end_week-tasn.start_week+1)*(tasn.end_class-tasn.start_class+1))) AS '课时数',
	...
	lr.id,
	lr.lab_room_name,
	SUM((SELECT COUNT(DISTINCT student_number) FROM timetable_course_student WHERE course_code_id=ta.self_course_code)) AS '上课人数',
	SUM((tasn.end_week-tasn.start_week+1)) AS '课次数',
	SUM(((tasn.end_week-tasn.start_week+1)*(tasn.end_class-tasn.start_class+1))) AS '课时数',
