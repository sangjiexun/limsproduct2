...
IN in_courseDetailNo   varchar(255),
IN in_labrooms  varchar(255),
IN in_status  int,
...
IN in_virtual varchar(255),
IN in_weekday int,
IN in_weeks varchar(255),
IN in_classes varchar(255),
IN in_teachers varchar(255),
IN in_tutors varchar(255),
IN in_items varchar(255),
IN in_softwares varchar(255),
...
Begin
  ...
  DECLARE var_newId INT;
	...
	DECLARE var_virtual,var_group,var_courseNO TEXT;
	DECLARE startTime,endTime,appDate,var_weekday LONGTEXT;
	SET i=0;
	SET j=0;
	SET k=0;
	SET l=0;
	SET var_courseNO =in_courseCode;

	SET @labRoomlength=1+(LENGTH(in_labrooms) - LENGTH(REPLACE(in_labrooms,',','')));
	SET @weeklength=1+(LENGTH(in_weeks) - LENGTH(REPLACE(in_weeks,',','')));
	SET @teacherlength=1+(LENGTH(in_teachers) - LENGTH(REPLACE(in_teachers,',','')));
	SET @tutorlength=1+(LENGTH(in_tutors) - LENGTH(REPLACE(in_tutors,',','')));
	SET @itemlength=1+(LENGTH(in_items) - LENGTH(REPLACE(in_items,',','')));
	SET @softwarelength=1+(LENGTH(in_softwares) - LENGTH(REPLACE(in_softwares,',','')));
  SET @beforWeek = 0;
	SET @var_Weeks = '';
	...
	If LENGTH(@var_Weeks)=2 or LENGTH(@var_Weeks)=3 then
	...
	If LENGTH(@var_Classes)=2 or LENGTH(@var_Classes)=3  then
	...
	If in_virtual='' then
	  SET in_virtual=null;
	End if;
	If in_selfCourseCode=0 then
	  SET in_selfCourseCode=null;
	End if;
	-- 教务排课
	If in_timetableStyle <> 1 and in_timetableStyle <>2 and in_timetableStyle <>3 and in_timetableStyle <>4 then
	  SET in_courseDetailNo=null;
	  SET var_courseNO=null;
	End if;
  ...
  SET var_newId = @@IDENTITY;
	...
	-- 插入实验室相关排课数据
	SET i=1;
	WHILE i<@labRoomlength DO
	  SET @currLabRoom = REVERSE(SUBSTRING_INDEX(REVERSE(SUBSTRING_INDEX(in_labrooms,',',i)),',',1));
	...
	-- 插入教师相关排课数据
	SET i=1;
	WHILE i<@teacherlength DO
	  SET @currTeacher = REVERSE(SUBSTRING_INDEX(REVERSE(SUBSTRING_INDEX(in_teachers,',',i)),',',1));
		insert into timetable_teacher_related(
			appointment_id,
			teacher
		)
		VALUES(
	...
	-- 插入指导教师相关排课数据
	SET i=1;
	WHILE i<@tutorlength DO
	  SET @currTutor = REVERSE(SUBSTRING_INDEX(REVERSE(SUBSTRING_INDEX(in_tutors,',',i)),',',1));
		insert into timetable_tutor_related(
			appointment_id,
			tutor
		)
		VALUES(
	...
	-- 插入软件相关排课数据
	SET i=1;
	WHILE i<@softwarelength DO
	  SET @currSoftware = REVERSE(SUBSTRING_INDEX(REVERSE(SUBSTRING_INDEX(in_softwares,',',i)),',',1));
		insert into timetable_software_related(
			appointment_id,
			software_id
		)
		VALUES(
	...
	-- 插入实验项目相关排课数据
	SET i=1;
	WHILE i<@itemlength DO
	  SET @currItem = REVERSE(SUBSTRING_INDEX(REVERSE(SUBSTRING_INDEX(in_items,',',i)),',',1));
		insert into timetable_item_related(
			appointment_id,
			item_id
		)
		VALUES(
	...
	-- 如果输入参数in_group不为空
	If in_group > 0 then
	  insert into timetable_appointment_group(
			appointment_id,
			group_id
		)
		VALUES(
