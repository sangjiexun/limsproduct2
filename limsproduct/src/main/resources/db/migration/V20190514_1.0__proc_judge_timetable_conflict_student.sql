...
Begin
	...
	DECLARE i,count,ifConflict,out_conflict_rate INT; 
	...
	WHILE i<@userlength DO
		...
		if i = @userlength-1 then
			SET out_conflict_rate = 100-(convert(count/(@userlength-1),decimal(10,2))*100);
			select out_conflict_rate;
		END if;
