...
iStyle_LineDatepicker.prototype = {
    initialize : function (){
        ...
        $("#iStyle_Datebutton").mousedown(function(){
            $("#clanderiframe", window.parent.document).parent().css({"z-index":"10000","width":"auto"});
            $("#clanderiframe", window.parent.document).parent().animate({opacity:1},0);
        });
        ...
        setTimeout(function(){
            $("#clanderiframe", window.parent.document).contents().find("#dpTodayInput").click();
        },200);
    ...
    getdate : function () {
        ...
        if(date !== ""){
        	...
        	is_table.load(data1,date1,"admin");
            this.date = date;
            $(".iStyle_Years").text( "机时预约："+this.date.substr(0,7));
            $("#clanderiframe", window.parent.document).parent().css({"z-index":"-10000","overflow":"hidden","width":"0px"});
            $("#clanderiframe", window.parent.document).parent().animate({opacity:0},0);
            var dater = $("#clanderiframe", window.parent.document).contents().find(".WdayTable td:gt(6)[class!=WotherDay]").clone(true); //选择日期
            var daynum = $("#clanderiframe", window.parent.document).contents().find(".WdayTable tr:eq(1)").find("td[class=WotherDay]").length; //获取月初始空余天数
