...
<script type="text/javascript">
	...
	*/	
	...
	function displayNewTag(title,url){
		//window.parent.document.getElementById("other").append("<div class='iStyle_Mark' hidden='true'  src='${pageContext.request.contextPath}/teaching/exam/examList?flag=1'><span>已发布考试</span></div>");;
		parent.$("#abc").empty();
		parent.$("#abc").html("<div class='iStyle_Mark ' src='${pageContext.request.contextPath}/teaching/exam/examList?flag=0'><span>可发布考试</span></div><div class='iStyle_Mark ' src='${pageContext.request.contextPath}/teaching/exam/examList?flag=1'><span>已发布考试</span></div><div class='iStyle_Mark iStyle_ActiveMark'   src='${pageContext.request.contextPath}/teaching/exam/"+ title + "'><span>" + title + "</span></div>");;
		//parent.$("#tt3").show();
		parent.$("#ssd").empty();
...
<%--<c:if test="${status!=0 }">
 查看: 所有 | 进行中: 对学生公开，可以进行 | 关闭: 学生不能进行考试 
</c:if>
--%><table  id="my_show">
<thead>
    <c:if test="${status==0 }">
...
<tr>    
        ...
        <td>${current.title}</a><br>
	        <a href="${pageContext.request.contextPath}/teaching/test/editTest?testId=${current.id}" >编辑考试</a>|
	        <a onclick="history.go(0)" href="${pageContext.request.contextPath}/teaching/test/deployTest?testId=${current.id}">发布考试</a>|
	        <a onclick="return confirm('是否确认删除测试？')" href="${pageContext.request.contextPath}/teaching/test/deleteTestById?testId=${current.id}">删除考试</a>
	        <%-- <a href="${pageContext.request.contextPath}/teaching/test/testbeginTest?testId=${current.id}">ceshi考试</a> --%>
...
<tr>
    ...
    <td>${current.title}<br>
        <%--<a href="${pageContext.request.contextPath}/teaching/exam/beginExam?simulation=1&examId=${current.id}" target="_blank">模拟考试</a>|
        <a href="${pageContext.request.contextPath}/teaching/exam/examInfo?examId=${current.id}">查看考试</a>|
        <a href="${pageContext.request.contextPath}/teaching/exam/examInfo?examId=${current.id}">导出考试</a>|
