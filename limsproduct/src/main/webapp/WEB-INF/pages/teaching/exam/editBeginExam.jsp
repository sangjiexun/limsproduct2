...
<body>
 ...
 --%>						<form:form action="${pageContext.request.contextPath}/teaching/exam/saveTAssignmentItemMapping?status=1&moduleType=${moduleType}&selectId=${selectId}" method="POST" >
												...
												<b>(分值：${current1.score})</b><br>
                                                   ...
                                                   <c:forEach items="${current1.TAssignmentAnswers}" var="current2"  varStatus="k">
	                                                    ...
	                                                    <c:if test="${current1.type==1 }">
                                                           <input type="checkbox" name="answers${current1.id}" id="answers${current1.id}" value="${current2.id}" <c:if test="${recordMap[current2.id]!=null }">checked="true"</c:if>/>${current2.label}: <c:out value="${current2.text}"></c:out><br>
	                                                    </c:if>
	                                                    <c:if test="${current1.type==2 }">
                                                           <input type="radio" name="answers${current1.id}" id="answers${current1.id}" value="${current2.id}" <c:if test="${recordMap[current2.id]!=null }">checked="true"</c:if>/> <c:out value="${current2.text}"></c:out><br>
	                                                    </c:if>
	                                                    <c:if test="${current1.type==4 }">
                                                           <input type="radio" name="answers${current1.id}" id="answers${current1.id}" value="${current2.id}" <c:if test="${recordMap[current2.id]!=null }">checked="true"</c:if>/>${current2.label}: <c:out value="${current2.text}"></c:out><br>
	                                                    </c:if>
	                                                    <c:if test="${current1.type==8 }">
                                                        	<input type="hidden" name="answers${current1.id}" value="${current2.id}"/>
                                                        	<input type="text" name="answertexts${current1.id}"  value="<c:out value='${recordMap[current2.id].answerText}'></c:out>"/><br>
                                                        </c:if>
                                                   </c:forEach>
                                               	</td>
                                            </tr>
