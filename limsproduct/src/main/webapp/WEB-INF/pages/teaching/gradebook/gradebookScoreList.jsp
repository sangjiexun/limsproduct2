...
<thead>
    <tr>                   
        ...
        <th>${current.TAssignment.title }<br>${fn:substring(100*current.weight,0,fn:indexOf(100*current.weight, ".")) }%</th>
        </c:forEach>
        <th>
        	<c:if test="${type eq 'assignment' }">作业成绩</c:if>
        	<c:if test="${type eq 'exam' }">测验成绩</c:if>
        </th>
    </tr>
</thead>
<tbody>
