	...
	<div class="navigation">
		<div id="navigation">
			<ul>
				...
				<li class="end"><a href="javascript:void(0)">导师互选列表</a>
				</li>
			</ul>
		</div>
	</div>
  ...
  --%>
	      ...
	      <a href="javascript:void(0)">添加学生</a>
					...
					<div class="page">
						${pageModel.totalRecords}条记录,共${pageModel.totalPage}页 <a
							...
							onclick="targetUrl('${pageContext.request.contextPath}/nwuChose/ChoseResultList?themeId=${theme.id}&currpage=1')"
							target="_self">首页</a> <a href="javascript:void(0)"
							onclick="targetUrl('${pageContext.request.contextPath}/nwuChose/ChoseResultList?themeId=${theme.id}&currpage=${pageModel.previousPage}')"
							target="_self">上一页</a> 第<select
							onchange="javascript:window.location.href = this.options[this.selectedIndex].value;">
							<option
								value="${pageContext.request.contextPath}/nwuChose/ChoseResultList?themeId=${theme.id}&currpage=${pageModel.currpage}">${pageModel.currpage}</option>
							<c:forEach begin="${pageModel.firstPage}"
								end="${pageModel.lastPage}" step="1" varStatus="j" var="current">
								<c:if test="${j.index!=pageModel.currpage}">
									<option
										value="${pageContext.request.contextPath}/nwuChose/ChoseResultList?themeId=${theme.id}&currpage=${j.index}">${j.index}</option>
								</c:if>
							</c:forEach>
						</select>页 <a href="javascript:void(0)"
							onclick="targetUrl('${pageContext.request.contextPath}/nwuChose/ChoseResultList?themeId=${theme.id}&currpage=${pageModel.nextPage}')"
							target="_self">下一页</a> <a href="javascript:void(0)"
							onclick="targetUrl('${pageContext.request.contextPath}/nwuChose/ChoseResultList?themeId=${theme.id}&currpage=${pageModel.lastPage}')"
							target="_self">末页</a>
					</div>
			...
			<table id="my_show">
				<thead>
					<tr>
						...
						<th style="width:30% !important">性别</th>
						<th style="width:30% !important">班级</th>
						<th style="width:30% !important">所属届</th>
	...
	<script type="text/javascript">
		function studentList(themeId, professorId) {
			...
			$.ajax({
				url : "${pageContext.request.contextPath}/nwuChose/passStudentListForProfessor?themeId="
