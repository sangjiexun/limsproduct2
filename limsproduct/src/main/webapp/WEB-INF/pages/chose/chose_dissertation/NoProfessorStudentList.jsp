		...
		--%>
						...
						<tbody>
							<c:forEach items="${users}" var="curr" varStatus="i">
								<tr>
									...
									<td><a href="javascript:void(0)" onclick="addDissertation('${curr.username}')">添加立题</a></td>
								</tr
							</c:forEach>
						</tbody>
					</table>
					<div class="page">
						${pageModel.totalRecords}条记录,共${pageModel.totalPage}页 <a
							...
							onclick="targetUrl('${pageContext.request.contextPath}/choseDissertation/noDissertationStudentList?attendanceTime=${attendanceTime}&currpage=1&themeId=${ themeId}')"
							target="_self">首页</a> <a href="javascript:void(0)"
							onclick="targetUrl('${pageContext.request.contextPath}/choseDissertation/noDissertationStudentList?attendanceTime=${attendanceTime}&currpage=${pageModel.previousPage}&themeId=${themeId }')"
							target="_self">上一页</a> 第<select
							onchange="javascript:window.location.href = this.options[this.selectedIndex].value;">
							<option
								value="${pageContext.request.contextPath}/choseDissertation/noDissertationStudentList?attendanceTime=${attendanceTime}&currpage=${pageModel.currpage}&themeId=${themeId}">${pageModel.currpage}</option>
							<c:forEach begin="${pageModel.firstPage}"
								end="${pageModel.lastPage}" step="1" varStatus="j" var="current">
								<c:if test="${j.index!=pageModel.currpage}">
									<option
										value="${pageContext.request.contextPath}/choseDissertation/noDissertationStudentList?attendanceTime=${attendanceTime}&currpage=${j.index}&themeId=${themeId}">${j.index}</option>
								</c:if>
							</c:forEach>
						</select>页 <a href="javascript:void(0)"
							onclick="targetUrl('${pageContext.request.contextPath}/choseDissertation/noDissertationStudentList?attendanceTime=${attendanceTime}&currpage=${pageModel.nextPage}&themeId=${themeId }')"
							target="_self">下一页</a> <a href="javascript:void(0)"
							onclick="targetUrl('${pageContext.request.contextPath}/choseDissertation/noDissertationStudentList?attendanceTime=${attendanceTime}&currpage=${pageModel.lastPage}&themeId=${themeId }')"
							target="_self">末页</a>
					</div>
				</div>
			</div>
