...
<body>
  ...
  <div class="content-box">
    <div class="title">
	  ...
	  <a class="btn btn-new" onclick="showUsersForChoseProfessor()">添加导师</a>
	  <a class="btn btn-new" href="${pageContext.request.contextPath }/choseDissertation/finalSaveChoseTheme">完成</a>
	...
	<table class="tb" id="my_show">
	  ...
	  <tr>
	    ...
	    <%-- <td>${sessionScope.choseTheme.maxStudent}</td> --%>
	    <td>${professor.finalNumber}</td>
	    <!-- <td><a href="javascript:void(0)">论文列表</a></td> -->
	    <td>
	      <!-- <a href="javascript:void(0)">编辑</a> -->
	      <a href="javascript:void(0);" onclick="deleteChoseProfessor('${professor.user.username }')">删除</a>
	    </td>
	  </tr>
	  </c:forEach>
	  </tbody>
    ...
    <a href="javascript:void(0)" onclick="targetUrl('${pageContext.request.contextPath}/choseDissertation/ChoseProfessorListForCD?currpage=1')" target="_self">首页</a>			    
	<a href="javascript:void(0)" onclick="targetUrl('${pageContext.request.contextPath}/choseDissertation/ChoseProfessorListForCD?currpage=${pageModel.previousPage}')" target="_self">上一页</a>
	第<select onchange="javascript:window.location.href = this.options[this.selectedIndex].value;">
	<option value="${pageContext.request.contextPath}/choseDissertation/ChoseProfessorListForCD?currpage=${pageModel.currpage}">${pageModel.currpage}</option>
	<c:forEach begin="${pageModel.firstPage}" end="${pageModel.lastPage}" step="1" varStatus="j" var="current">	
    <c:if test="${j.index!=pageModel.currpage}">
    <option value="${pageContext.request.contextPath}/choseDissertation/ChoseProfessorListForCD?currpage=${j.index}">${j.index}</option>
    </c:if>
    </c:forEach></select>页
	<a href="javascript:void(0)"  onclick="targetUrl('${pageContext.request.contextPath}/choseDissertation/ChoseProfessorListForCD?currpage=${pageModel.nextPage}')" target="_self">下一页</a>
 	<a href="javascript:void(0)"  onclick="targetUrl('${pageContext.request.contextPath}/choseDissertation/ChoseProfessorListForCD?currpage=${pageModel.lastPage}')" target="_self">末页</a>
    </div>
  </div>
  </div>
  </div>
