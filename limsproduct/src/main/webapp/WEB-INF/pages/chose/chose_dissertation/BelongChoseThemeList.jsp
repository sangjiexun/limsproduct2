...
<body>
  ...
  <div id="TabbedPanels1" class="TabbedPanels">
   ...
   <div class="TabbedPanelsContentGroup">
	  ...
	  <%--</div>--%>
	   <div class="tool-box">
		<form:form name="queryForm" action="${pageContext.request.contextPath}/nwuChose/BelongChoseThemeList?currpage=1" method="post" modelAttribute="choseTheme">
		 <ul>
  		  <li>大纲编号：</li>
  		  <li><form:input id="id" path="id"/></li>
    ...
    <a href="javascript:void(0)" onclick="targetUrl('${pageContext.request.contextPath}/choseDissertation/belongChoseThemeList?currpage=1')" target="_self">首页</a>			    
	<a href="javascript:void(0)" onclick="targetUrl('${pageContext.request.contextPath}/choseDissertation/belongChoseThemeList?currpage=${pageModel.previousPage}')" target="_self">上一页</a>
	第<select onchange="javascript:window.location.href = this.options[this.selectedIndex].value;">
	<option value="${pageContext.request.contextPath}/choseDissertation/belongChoseThemeList?currpage=${pageModel.currpage}">${pageModel.currpage}</option>
	<c:forEach begin="${pageModel.firstPage}" end="${pageModel.lastPage}" step="1" varStatus="j" var="current">	
    <c:if test="${j.index!=pageModel.currpage}">
    <option value="${pageContext.request.contextPath}/choseDissertation/belongChoseThemeList?currpage=${j.index}">${j.index}</option>
    </c:if>
    </c:forEach></select>页
	<a href="javascript:void(0)"  onclick="targetUrl('${pageContext.request.contextPath}/choseDissertation/belongChoseThemeList?currpage=${pageModel.nextPage}')" target="_self">下一页</a>
 	<a href="javascript:void(0)"  onclick="targetUrl('${pageContext.request.contextPath}/choseDissertation/belongChoseThemeList?currpage=${pageModel.lastPage}')" target="_self">末页</a>
    </div>
  </div>
  </div>
  </div>
	...
	<script type="text/javascript">
	    	...
	    	if(${isChoseDissertation==null}){
		    	for(var i=0;i<st.length;i++){
		        	...
		        	if(currTime.getTime()>=startTime&&currTime.getTime()<=endTime){
		        		var t="<a href='javaScript:void(0)' onclick='checkIfReadAttention("+ids.eq(i).val()+")'>填写志愿</a>";
		        		tds.eq(i).html(t);
		        	}
		        } 
	    	}
	    		...
	    		for(var i=0;i<st.length;i++){
	    			    var t="<a href='${pageContext.request.contextPath}/choseDissertation/findBatchState?themeId="+ids.eq(i).val()+"'>查看状态</a>";
		        		tds.eq(i).html(t);
	    		}
	    	}
       ...
       //type 所属类型   1导师制互选导师   2导师制互选学生   3论文制互选导师  4论文制互选学生
   		...
   		success:function(result){
   			if(result=="success"){
   				window.location.href="${pageContext.request.contextPath}/choseDissertation/toAddBatch?i=1&currpage=1&themeId="+themeId;
   			}else{
   				//弹出注意事项框
			  layer.ready(function(){
			        layer.open({
			            ...
			            maxmin: false,
			            area: ['1000px', '420px'],
			            content: '${pageContext.request.contextPath}/nwuChose/findChoseAttentionByType?type=3&themeId='+themeId,
			            end: function(){
			            }
			        });
			    });
