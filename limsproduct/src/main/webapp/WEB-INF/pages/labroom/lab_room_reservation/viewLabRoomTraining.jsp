...
<script type="text/javascript" src="${pageContext.request.contextPath}/js/dhulims/device/viewDeivce.js"></script>
	<script type="text/javascript">
        ...
        function addReservation(id){
            //ajax判断是否可以预约
            $.ajax({
                url:"${pageContext.request.contextPath}/labReservation/ifPermitted?labRoomId="+${labRoom.id},
                type:"post",
                success:function(data){
                    if(data=="wait"){
                        alert("您已预约其他培训，请耐心等待培训结果！")
                    }else if(data=="permitted"){
                        alert("您不需要参加此实验室安全准入培训，请直接预约！")
                    }else{
                        trainingId=id;
        ...
        //保存实验设备培训人
        //保存实验设备培训人
        function joinTrainingRestNew(){//将labRoomId deviceNumber deviceName page传递到后台
            ...
            if(telephone==""||telephone==null){
                ...
                //var url = "${pageContext.request.contextPath}/labReservation/joinTrainingRest/" + ${labRoomId} + "/"+${type}+"/"+ ${deviceNumber} + "/" + "${deviceName}" + "/" +${username}+ "/" +${page}+ "/" + trainingId +"/" + telephone +"/" + eMail+"/"+schoolDevice_allowAppointment;
                var url = "${pageContext.request.contextPath}/labReservation/joinTrainingRest/" + ${labRoomId} + "/"+1+"/"+ ${deviceNumber} + "/" + "${deviceName}" + "/" +${username}+ "/" +${page}+ "/" + trainingId +"/" + telephone +"/" + eMail+"/"+schoolDevice_allowAppointment;
                //alert(url);
...
function cancleTrainingRest(id){
	...
	var url = "${pageContext.request.contextPath}/labReservation/cancleTrainingRest/" + ${labRoomId} + "/"+ ${deviceNumber} + "/" + "${deviceName}" +"/"+${username}+ "/"+${page}+"/" + id+"/"+"${labRoom_allowAppointment}";
	//alert(url);
	window.location.href=url;
}
//根据设备id查询培训
function findTrainingPeopleByTrainIdRest(id,toChangeAudit){//将labRoomId deviceNumber deviceName page传递到后台
	var url = "${pageContext.request.contextPath}/labReservation/findTrainingPeopleByTrainIdRest/" + ${labRoomId} + "/"+ ${deviceNumber} + "/" + "${deviceName}"+"/"+${username} + "/"+${page}+"/" + id+"/"+1 +"/"+toChangeAudit+"/"+"${schoolDevice_allowAppointment}";
	alert(url);
	window.location.href=url;
}
	 ...
	 <ul class="TabbedPanelsTabGroup">
	  ...
	  <div class="TabbedPanelsContentGroup">
		...
		<div class="content-box">
			...
			<a class="btn btn-new"  herf="#" onclick="window.location.href='${pageContext.request.contextPath}/LabRoomReservation/labRoomList?currpage=1';">返回</a>
			</c:if>
			</div>
				<div class="content-double">
					...
					<table >
						...
						<sec:authorize ifNotGranted="ROLE_SUPERADMIN,ROLE_EQUIPMENTADMIN"><!-- 只有学生才能参加培训 -->
							<td>
								<%-- <c:if test="${t.CDictionary.id==1}"> --%><!-- 待培训 -->
								<c:if test="${t.CDictionary.CCategory=='c_training_status'&&t.CDictionary.CNumber=='1'}">
								<c:if test="${t.number > t.joinNumber}"><!-- 没有超出人数限制 -->
									<c:forEach items="${map}" var="m">
										<c:if test="${m.key==t.id}"><!-- 匹配培训id -->
											...
											<a href="javascript:void(0);"  onclick="addReservation(${t.id})";>预约培训</a>
											</c:if>
											<c:if test="${m.value==0}">
											培训已预约
								...
								<%-- <c:if test="${t.CDictionary.id==2}"> --%>
								<c:if test="${t.CDictionary.CCategory=='c_training_status'&&t.CDictionary.CNumber=='2'}">
								<c:if test="${fn:contains(t.labRoomTrainingPeoples,user.username)}">
								培训结束，请<a href="javascript:void(0);" onclick="AccessOrNot('${t.labRoomDevice.id}');"  >点击</a>查看培训结果
								</c:if>
								<c:if test="${fn:indexOf(t.labRoomTrainingPeoples,user.username)==-1}">
								培训已结束！
								</c:if>
								</c:if>
								<%-- <c:if test="${t.CDictionary.id==3}"> 
									${t.CDictionary.name}--%>
								<c:if test="${t.CDictionary.CCategory=='c_training_status'&&t.CDictionary.CNumber=='3'}">
									${t.CDictionary.CName}
								</c:if>
								<%-- <c:if test="${t.CDictionary.id==4 }"> --%>
...
<div id="addStudentInfo" class="easyui-window" closed="true" modal="true" minimizable="true" title="联系方式" style="width: 600px;height: 200px;padding: 20px">
	  <div class="content-box">
		  <form:form name="form_material" method="post" modelAttribute="labRoomTrainingPeople" >
			  <table class="color_tb">
			  <tr>
			    <td>移动电话</td><td><form:input path="telephone" id="telephone" required="true" onblur="checkTeleNew()"/><form:hidden path="id" /></td>
			  </tr>
			  <tr>
			    <td>邮箱</td><td><form:input path="eMail" id="eMail" required="true" onblur="checkMailNew()"/></td>
			  </tr>
			  </table>
			  
			  <div class="moudle_footer">
			    <div class="submit_link">
			        <input class="btn" id="save" type="button" onclick="joinTrainingRestNew()" value="确定" />
			    </div>
			  </div>
		  </form:form>
	</div>
