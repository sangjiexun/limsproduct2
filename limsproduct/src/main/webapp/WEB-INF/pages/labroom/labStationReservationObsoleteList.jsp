    ...
    <div id="TabbedPanels1" class="TabbedPanels">
        <ul class="TabbedPanelsTabGroup">
            ...
            <c:if test="${sessionScope.selected_role eq 'ROLE_SUPERADMIN' || sessionScope.selected_role eq 'ROLE_LABMANAGER'}">
                <li class="TabbedPanelsTab selected" id="s5"><a
                    ...
                    <div class="content-box">
                        <div class="tool-box">
                            <form:form name="queryForm"
                                       action="${pageContext.request.contextPath}/LabRoomReservation/labStationReservationObsoleteList?page=1&tage=${tage}&isaudit=${isAudit}"
                                       method="post" modelAttribute="labRoomStationReservation">
                                <ul style="position: absolute;">
                                    <li><spring:message code="left.station.appointment"/>:<form:input
                        ...
                        <div class="page">
                            ...
                            <a href="javascript:void(0)"
                               onclick="first('${pageContext.request.contextPath}/LabRoomReservation/labStationReservationObsoleteList?page=1')"
                               target="_self">首页</a>
                            <a href="javascript:void(0)"
                               onclick="previous('${pageContext.request.contextPath}/LabRoomReservation/labStationReservationObsoleteList?page=')"
                               target="_self">上一页</a>
                            第<select
                                onchange="javascript:window.location.href = this.options[this.selectedIndex].value;">
                            <option value="${pageContext.request.contextPath}/LabRoomReservation/labStationReservationObsoleteList?page=${page}">${page}</option>
                            <c:forEach begin="${pageModel.firstPage}" end="${pageModel.lastPage}" step="1" varStatus="j"
                                       var="current">
                                <c:if test="${j.index!=page}">
                                    <option value="${pageContext.request.contextPath}/LabRoomReservation/labStationReservationObsoleteList?page=${j.index}">${j.index}</option>
                                </c:if>
                            </c:forEach></select>页
                            <a href="javascript:void(0)"
                               onclick="next('${pageContext.request.contextPath}/LabRoomReservation/labStationReservationObsoleteList?page=')"
                               target="_self">下一页</a>
                            <a href="javascript:void(0)"
                               onclick="last('${pageContext.request.contextPath}/LabRoomReservation/labStationReservationObsoleteList?page=${pageModel.totalPage}')"
                               target="_self">末页</a>
                        </div>
                        <!-- 分页模块 -->
                    </div>
