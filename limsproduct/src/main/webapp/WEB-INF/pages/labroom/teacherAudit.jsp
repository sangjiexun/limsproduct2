...
<c:if test="${state == 1 && isAudit == 1}">
 <form:form action="${pageContext.request.contextPath}/LabRoomReservation/saveTeacherAudit?id=${id}&tage=${tage}&state=${state }&currpage=${currpage}"  method="post"  modelAttribute="labRoomStationReservationResult" target="_parent" >
   <table>
        <tr>
           <td>审核：</td>
           <td colspan="4">
           <form:radiobutton path="auditResult"  value="1" checked="true"/>通过
           <form:radiobutton path="auditResult"  value="0" />拒绝  
            </td>
       </tr>
        <tr>
