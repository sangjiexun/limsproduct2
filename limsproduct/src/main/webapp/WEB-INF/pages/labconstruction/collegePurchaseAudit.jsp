	  ...
	  <div class="TabbedPanelsContentGroup">
		<div class="TabbedPanelsContent">
			<div class="content-box">
				...
				<c:if test="${labConstructionPurchase.stage==0 }"> <!-- 未审核阶段 -->
				<c:if test="${fn:contains(collegeLeaders,user.username) && labConstructionPurchase.auditResults==2}"> <!-- 当前登陆人是学院主管领导,当前处于审核中状态 -->
						<form:form action="${pageContext.request.contextPath}/labconstruction/saveCollegePurchaseAudit?labConstructionPurchaseId=${labConstructionPurchase.id}" method="post" modelAttribute="collegePurchaseAudit">
							<table>
								<tr>
									<td>审核意见</td>
								...
								<tr>
									<td>
										<input class="btn btn-big" type="submit" value="提交">
										<input class="btn btn-return" type="button" value="取消">
									</td>
								</tr>
				...
				<c:if test="${fn:indexOf(collegeLeaders,user.username)==-1 && labConstructionPurchase.stage<=0 && labConstructionPurchase.stage!=-1 }">  
				<!-- 当前登录人不是学院主管领导，当前阶段还没有到学院领导审核阶段，并且当前不是审核拒绝状态 -->
					<table id="listTable" width="50%" cellpadding="0" cellspacing="0" class="tablesorter" >
					    <tr><td colspan="6">学院主管领导信息 </td></tr>
					 <c:forEach items="${collegeLeaders}"  var="cl">
						 <tr align="left" >
						     <td>姓名：   ${cl.cname}</td> 
						     <td>工号：   ${cl.username}</td> 
							 <td>部门：   ${cl.schoolAcademy.academyName}</td> 
						     <td>联系方式：   ${cl.telephone}</td> 
						 </tr> 
					 </c:forEach>
			...
			<c:if test="${labConstructionPurchase.stage>0 || labConstructionPurchase.stage==-1}">   <!-- 想要看到当前的审核意见，必须高于当前阶段/或者审核拒绝状态也可以 -->
				<tr>
					<td>审核意见：</td>
					<td>${collegePurchaseAuditDone.comments}</td>
				</tr>
				<tr>	
					<td>审核结果：</td>
					<td>${collegePurchaseAuditDone.CDictionary.CName}</td>
				</tr>
			</c:if>
