...
<div id="navigation">
	<ul>
		<li><a href="javascript:void(0)"><spring:message code="left.labconstruction.acceptance"/></a></li>
...
<div class="title">
	<div id="title">实验室建设项目验收申请编辑</div>
...
<legend>基本信息（带<font color=red>*</font>为必填项）<input type="hidden" value="" id="xsd"></legend>
<table cellpadding="0" cellspacing="0" id="viewTable">
<tbody>
    <tr>
       <td class="label" valign="top">项目名称<font color=red>*</font>：</td>
	   <td class="label" valign="top" >${projectAcceptanceApplication.projectName} </td>
	...
	<%--		<form:select path="schoolAcademy.academyNumber" class="chzn-select">
			...
			${projectAcceptanceApplication.schoolAcademy.academyName}
	   </td>
    ...
    <tr>
	   ...
	   <!--申报人属性  -->
	   <input type="hidden" name="user.id" value="${projectStartedReport.labConstructApp.user.id} " >
	   </td>
	   <!--申报单位属性  -->
	   <input type="hidden" name="schoolAcademy.academyName" value="${projectStartedReport.labConstructApp.user.schoolAcademy.academyName}  " >
	   </td>
	...
	<tr>
	   ...
	   <td>				
			...
			<form:options items="${CProjectSources}" itemLabel="name" itemValue="id"/>
					...
					${projectAcceptanceApplication.CProjectSource.name}
	   </td>
	</tr>
    <tr>
       <td class="label" valign="top">用途：</td>
       <td class="label" valign="top">${projectAcceptanceApplication.projectStartedReport.labConstructApp.CProjectPurpose.name}</td><%-- 
...
<tbody>
   <tr>
	   ...
	   <td class="label" valign="top"><input id="expect_complete_date" name="expectCompleteDate" value="${expectCompleteDate}" onclick="new Calendar().show(this);"  readonly="readonly"  required="true"/></td>		 
   	   <td class="label" valign="top">实际完成时间：</td>
	   <td class="label" valign="top"><input id="reality_complete_date" name="realityCompleteDate" value="${realityCompleteDate}" onclick="new Calendar().show(this);"  readonly="readonly"  required="true"/></td> 
   </tr>
         <tr>
   	   <td class="label" valign="top">立项时间：</td>
	...
	<legend>经费预算</legend>
		<table>
		  ...
		  <table>
			...
			<tbody id="add_money">
				<c:forEach items="${projectAcceptanceApplication.projectAcceptFeeLists}" var="fee">
				<tr>
				<td>${fee.CFundAppItem.name}</td>
				<td>${fee.amount} </td>
				<td>${fee.budgetaryItem}</td>
				<td>
				<a onclick="return confirm('确认要删除吗？')" href="${pageContext.request.contextPath}/labconstruction/deleteProjectAcceptFeeList?id=${fee.id}&AppId=${projectAcceptanceApplication.id}">删除</a>
				<a href="${pageContext.request.contextPath}/labconstruction/updateProjectAcceptFeeList?id=${fee.id}&AppId=${projectAcceptanceApplication.id}&flag=2" >修改</a>
				</td>
				</tr>
				</c:forEach>
			</tbody>
		...
		<table>
		  ...
		  <table>
			...
			<tbody id="add_device">
				...
				<td>
				<a onclick="return confirm('确认要删除吗？')" href="${pageContext.request.contextPath}/labconstruction/deleteProjectAcceptDevice?id=${device.id}&AppId=${projectAcceptanceApplication.id}">删除</a>
				<a href="${pageContext.request.contextPath}/labconstruction/updateProjectAcceptDevice?id=${device.id}&AppId=${projectAcceptanceApplication.id}&flag=2" >修改</a>
				</td>
		...
		<table>
		  ...
		  <table>
			...
			<tbody id="add_Item">
				...
				<td>
				<a onclick="return confirm('确认要删除吗？')" href="${pageContext.request.contextPath}/labconstruction/deleteProjectAcceptCompletionItem?id=${item.id}&AppId=${projectAcceptanceApplication.id}">删除</a>
				<a href="${pageContext.request.contextPath}/labconstruction/updateProjectAcceptCompletionItem?id=${item.id}&AppId=${projectAcceptanceApplication.id}&flag=2" >修改</a>
				</td>
				</tr>
				</c:forEach>		
			</tbody>
...
<fieldset class="introduce-box">
						  ...
						  <tbody id="doc">
						  	<c:forEach items="${CommonDocuments}" var="doc" varStatus="i">
								<tr >
							 	...
							 	<td><a onclick="return confirm('确认要删除吗？')" href="${pageContext.request.contextPath}/labconstruction/deleteProjectAcceptanceAppCommonDocument?id=${doc.id}&AppId=${projectAcceptanceApplication.id}">删除</a> </td>
								</tr>
							</c:forEach>	
						  </tbody>
					 </table>	
     ...
     <tr><td colspan="5"></td></tr>
        <%--<input type="button" value="确定" onclick="sub('${pageContext.request.contextPath}/labconstruction/saveProjectAcceptance');"/>--%>
	...
	<div class="moudle_footer" style="background: white">
		<div class="submit_link" style="margin:5px;">
			<input class="btn btn-big" value="确定"  onclick="sub('${pageContext.request.contextPath}/labconstruction/saveProjectAcceptance');"/>
