	  ...
	  <div class="TabbedPanelsContentGroup">
		<div class="TabbedPanelsContent">
			<div class="content-box">
				...
				<c:if test="${labConstructionFunding.stage==0 }"> <!-- 未审核阶段 -->
				<c:if test="${fn:contains(excenterDirectors,user.username) && labConstructionFunding.auditResults==2}"> <!-- 当前登陆人是实训室中心主任,当前处于审核中状态 -->
						<form:form action="${pageContext.request.contextPath}/labconstruction/saveExcenterFundingAudit?labConstructionFundingId=${labConstructionFunding.id}" method="post" modelAttribute="excenterFundingAudit">
							<table>
								<tr>
									<td>审核意见</td>
				...
				<c:if test="${fn:indexOf(excenterDirectors,user.username)==-1 && labConstructionFunding.stage<=0 && labConstructionFunding.stage!=-1 }">  
				<!-- 当前登录人不是实训室中心主任，当前阶段还没有到验室中心主任审核阶段，并且当前不是审核拒绝状态 -->
					<table id="listTable" width="50%" cellpadding="0" cellspacing="0" class="tablesorter" >
					    <tr><td colspan="6">实训室中心主任信息 </td></tr>
					 <c:forEach items="${excenterDirectors}"  var="ed">
						 <tr align="left" >
						     <td>姓名：   ${ed.cname}</td> 
						     <td>工号：   ${ed.username}</td> 
							 <td>部门：   ${ed.schoolAcademy.academyName}</td> 
						     <td>联系方式：   ${ed.telephone}</td> 
						 </tr> 
					 </c:forEach>
			...
			<c:if test="${labConstructionFunding.stage>0 || labConstructionFunding.stage==-1}">   <!-- 想要看到当前的审核意见，必须高于当前阶段/或者审核拒绝状态也可以 -->
				<tr>
					<td>审核意见：</td>
					<td>${excenterFundingAuditDone.comments}</td>
				</tr>
				<tr>	
					<td>审核结果：</td>
					<td>${excenterFundingAuditDone.CDictionary.CName}</td>
				</tr>
			</c:if>
