...
<tbody>
    <tr>
       ...
       <input type="hidden" name="labConstructApp.id" value="${labConstructApp.id} " >
       <td class="label" valign="top">所属系部<font color=red>*</font>：</td>
 	    <td>				
    ...
    <tr>
       <td class="label" valign="top">申报时间<font color=red>*</font>：</td>
	   <td class="label" valign="top"><fmt:formatDate pattern="yyyy-MM-dd" type="both" value="${labConstructApp.appDate.time}"/></td>
       <td class="label" valign="top">申请人：</td>
	   <td class="label" valign="top">${labConstructApp.user.cname}</td>
	   <!--申报人属性  -->
	   <input type="hidden" name="user.username" value="${labConstructApp.user.username} " >
	   <!--申报单位属性  -->
	   <input type="hidden" name="schoolAcademy.academyName" value="${labConstructApp.user.schoolAcademy.academyName}  " >

	</tr>
	<tr>
	    <td class="label" valign="top">工号：</td>
	    <td class="label" valign="top">${labConstructApp.user.username}</td>
		<td class="label" valign="top">性别：</td>
	    <td class="label" valign="top">${labConstructApp.user.userSexy}</td>
	</tr>
	<tr>
	   <td class="label" valign="top">联系电话：</td>
	   <td class="label" valign="top">${labConstructApp.user.telephone}</td>
	   <td class="label" valign="top">E-MAIL：</td>
	   <td class="label" valign="top">${labConstructApp.user.email}</td>
	</tr>
    <tr>
	   ...
	   <td class="label" valign="top">${labConstructApp.CProjectPurpose.name}</td> 	
	</tr>
    <tr>
	...
	<legend>经费预算</legend>
		<table>
		  ...
		  <table>
			...
			<tbody id="add_money">
				<c:forEach items="${projectStartFeeLists}" var="fee">
				<tr>
				<td>${fee.CFundAppItem.name}</td>
				<td>${fee.amount} </td>
				<td>${fee.budgetaryItem}</td>
				<td>
				<a onclick="return confirm('确认要删除吗？')" href="${pageContext.request.contextPath}/labconstruction/deleteProjectStartFeeListNew?id=${fee.id}&AppId=${projectStartedReport.id}">删除</a>
				<a href="${pageContext.request.contextPath}/labconstruction/updateProjectStartFeeList?id=${fee.id}&AppId=${projectStartedReport.id}&flag=1" >修改</a>
				</td>
				</tr>
				</c:forEach>
			</tbody>
	...
	<legend>设备列表（带<font color=red>*</font>为必填项）</legend>
		<table>
		  ...
		  <table>
			...
			<tbody id="add_device">
				<c:forEach items="${projectStartDevices}" var="device" varStatus="i">
				<tr>
				<%--<td>${i.count}</td>
				...
				<td>
				<a onclick="return confirm('确认要删除吗？')" href="${pageContext.request.contextPath}/labconstruction/deleteProjectStartDeviceNew?id=${device.id}&AppId=${projectStartedReport.labConstructApp.id}">删除</a>
				<a href="${pageContext.request.contextPath}/labconstruction/updateProjectStartDevice?id=${device.id}&AppId=${projectStartedReport.id}&flag=1" >修改</a>
				</td>
	...
	<legend>建成后可开设实验（实训）项目清单</legend>
		<table>
		  ...
		  <table>
			...
			<tbody id="add_Item">
				<c:forEach items="${projectStartCompletionItems}" var="item" varStatus="i">
				<tr>
				<%--<td>${i.count}</td>
				--%><td>${item.experimentName}</td>
				<td>${item.isOriginal}</td>
				<td>
				<a onclick="return confirm('确认要删除吗？')" href="${pageContext.request.contextPath}/labconstruction/deleteProjectStartCompletionItemNew?id=${item.id}&AppId=${projectStartedReport.labConstructApp.id}">删除</a>
				<a href="${pageContext.request.contextPath}/labconstruction/updateProjectStartCompletionItem?id=${item.id}&AppId=${projectStartedReport.id}&flag=1" >修改</a>
				</td>
				</tr>
				</c:forEach>		
			</tbody>
...
<fieldset class="introduce-box">
     ...
     <tr><td colspan="5"></td></tr>
        <input type="button" value="确定" onclick="sub('${pageContext.request.contextPath}/labconstruction/saveProjectStartedReportNew');"/>
