	  ...
	  <div class="TabbedPanelsContentGroup">
		<div class="TabbedPanelsContent">
			<div class="content-box">
				...
				<c:if test="${labConstructionFunding.stage==3 }"> <!-- 财务总监审核阶段 -->
				<c:if test="${fn:contains(cfos,user.username) && labConstructionFunding.auditResults==2}"> <!-- 当前登陆人是财务总监,当前处于审核中状态 -->
						<form:form action="${pageContext.request.contextPath}/labconstruction/saveCfoFundingAudit?labConstructionFundingId=${labConstructionFunding.id}" method="post" modelAttribute="cfoFundingAudit">
							<table>
								<tr>
									<td>审核意见</td>
				...
				<c:if test="${fn:indexOf(cfos,user.username)==-1 && labConstructionFunding.stage<=3 && labConstructionFunding.stage!=-1 }">  
				<!-- 当前登录人不是财务总监，当前阶段还没有到验财务总监审核阶段，并且当前不是审核拒绝状态 -->
					<table id="listTable" width="50%" cellpadding="0" cellspacing="0" class="tablesorter" >
					    <tr><td colspan="6">财务总监信息 </td></tr>
			...
			<c:if test="${labConstructionFunding.stage>3 || labConstructionFunding.stage==-1}">   <!-- 想要看到当前的审核意见，必须高于当前阶段/或者审核拒绝状态也可以 -->
				<tr>
					<td>审核意见：</td>
					<td>${cfoFundingAuditDone.comments}</td>
				</tr>
				<tr>	
					<td>审核结果：</td>
					<td>${cfoFundingAuditDone.CDictionary.CName}</td>
				</tr>
			</c:if>
