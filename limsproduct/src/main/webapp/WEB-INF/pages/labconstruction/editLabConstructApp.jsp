...
function getUsernameArray(){
	...
	$("input[name='username']:text").each(function() { //遍历所有的name为username的 hidden  
...
function count(obj){
	...
	var count=0;
	var xxx= parseFloat(obj.value);
	for(var i=0;i<moneyArray.length;i++){
		var money=parseFloat(moneyArray[i]);
		count+=money;
	}
	if(!isNaN(xxx)){
		var m=document.getElementById("feeAmount").value;
		count=m*10000+xxx;
		alert(count);
...
<div class="navigation">
				<div id="navigation">
					<ul>
						...
						<li class="end"><a href="javascript:void(0)">编辑申请</a></li>
					</ul>
				</div>
			</div>
...
<tbody>
     <tr>
	   ...
	   --%><td><form:select path="user.username" class="chzn-select">
			<form:option value="${users.username}">${users.cname}</form:option>
	   ...
	   <!--申报单位属性  -->
	   <input type="hidden" name="schoolAcademy.academyNumber" value="${labConstructApp.user.schoolAcademy.academyNumber}  " >
	...
	<tr>
		...
		<td class="label" valign="top">用途<font color=red>*</font>：</td>
        <td><select name=purposeName id="purposeName" style="width: 280px" multiple="true" class="chzn-select" >          
            <c:forEach items="${CProjectPurposes}" var ="k">
			<option  value="${k.id}" selected="selected">${k.name}</option></c:forEach> 
            <c:forEach items="${cProjectPurposeEdit}" var="s">
			<option value="${s.id }">${s.name }</option></c:forEach>
		    </select>
	    </td>
	</tr>
	<tr>
       ...
       <td><select name=majorName id="majorName" style="width: 280px" multiple="true" class="chzn-select" >          
            <c:forEach items="${schoolMajors}" var ="k">
			<option  value="${k.majorNumber}" selected="selected">${k.majorName}</option></c:forEach> 
 	...
 	<tr>
       ...
       <td><select name=courseName id="courseName" style="width: 280px" multiple="true" class="chzn-select" >          
            <c:forEach items="${schoolCourses}" var ="k">
			<option  value="${k.courseNo}" selected="selected">${k.courseName}</option></c:forEach> 
...
<tbody>
    <tr>
	   <td class="label" valign="top" width="70px">立项依据</td>
	   <td colspan="3">
	      <form:textarea id="projectBasis_progress" path="projectBasis" />
	   <%--<form:textarea id="site_bottomContent"   path="projectBasis"></form:textarea>
	   --%></td>
    </tr>
    <tr>
	   <td class="label" valign="top">建设的基础</td>
	   <td colspan="3">
    ...
    <tr>
	   <td class="label" valign="top" >预期成果</td>
	   <td colspan="3">
		...
		<table>
		  ...
		  <table>
			...
			<tbody id="add_user">
				...
				<td>${user.position}</td>
				<td>${user.jobTitle}</td>
				<td>${user.responsibilityContent}</td>
				<td>
				<a onclick="return confirm('确认要删除吗？')" href="${pageContext.request.contextPath}/labconstruction/deleteLabConstructUser?id=${user.id}&AppId=${labConstructApp.id}">删除</a>
				<a href="${pageContext.request.contextPath}/labconstruction/updateLabConstructUser?id=${user.id}&AppId=${labConstructApp.id}">修改</a>
				</td>
				</tr>
				</c:forEach>		
			</tbody>
	...
	<legend>经费预算</legend>
		<table>
		  ...
		  <table>
			...
			<tbody id="add_money">
				<c:forEach items="${labConstructApp.projectFeeLists}" var="fee">
				<tr>
				<td>${fee.CFundAppItem.name}</td>
				<td>${fee.amount} </td>
				<td>${fee.budgetaryItem}</td>
				<td>
				<a onclick="return confirm('确认要删除吗？')" href="${pageContext.request.contextPath}/labconstruction/deleteProjectFeeList?id=${fee.id}&AppId=${labConstructApp.id}">删除</a>
				<a href="${pageContext.request.contextPath}/labconstruction/updateProjectFeeList?id=${fee.id}&AppId=${labConstructApp.id}" >修改</a>
				</td>
				</tr>
				</c:forEach>
			</tbody>
		...
		<table>
		  ...
		  <table>
			...
			<tbody id="add_device">
				...
				<td>
				<a onclick="return confirm('确认要删除吗？')" href="${pageContext.request.contextPath}/labconstruction/deleteProjectDevice?id=${device.id}&AppId=${labConstructApp.id}">删除</a>
				<a href="${pageContext.request.contextPath}/labconstruction/updateProjectDevice?id=${device.id}&AppId=${labConstructApp.id}" >修改</a>
				</td>
		...
		<table>
		  ...
		  <table>
			...
			<tbody id="add_Item">
				...
				<td>
				<a onclick="return confirm('确认要删除吗？')" href="${pageContext.request.contextPath}/labconstruction/deleteProjectCompletionItems?id=${item.id}&AppId=${labConstructApp.id}">删除</a>
				<a href="${pageContext.request.contextPath}/labconstruction/updateProjectCompletionItem?id=${item.id}&AppId=${labConstructApp.id}" >修改</a>
				</td>
				</tr>
				</c:forEach>		
			</tbody>
						  ...
						  <tbody id="doc">
						  	<c:forEach items="${labConstructApp.commonDocuments}" var="doc" varStatus="i">
								<tr >
							 	...
							 	<td><a onclick="return confirm('确认要删除吗？')" href="${pageContext.request.contextPath}/labconstruction/deleteCommonDocument?id=${doc.id}&AppId=${labConstructApp.id}">删除</a> </td>
								</tr>
							</c:forEach>	
						  </tbody>
					 </table>	
