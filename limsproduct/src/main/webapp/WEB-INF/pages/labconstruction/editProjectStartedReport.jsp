...
function uploadDocument(){
			 ...
			 $("#file_upload").uploadify({
	            ...
	            'uploader':'${pageContext.request.contextPath}/labconstruction/projectStartReport/uploadFile;jsessionid=<%=session.getId()%>',		
	            buttonText:'选择文件',
	            //上传成功之后在列表追加显示文档
...
<div class="navigation">
				<div id="navigation">
					<ul>
						<li><a href="javascript:void(0)"><spring:message code="left.labconstruction.started"/></a></li>
						<li class="end"><a href="javascript:void(0)">实验室建设项目启动报告</a></li>
					</ul>
				</div>
			</div>
		...
		<div id="TabbedPanels1" class="TabbedPanels">
			<div class="TabbedPanelsContentGroup">
				<div class="TabbedPanelsContent">
					<div class="title">
						<div id="title">实验室建设项目启动报告编辑</div>
...
<tbody>
    <tr>
	   ...
	   <td class="label" valign="top" ><form:input path="projectName" /> </td>
       <input type="hidden" name="labConstructApp.id" value="${projectStartedReport.labConstructApp.id} " >
    ...
    <tr>
	   ...
	   <td class="label" valign="top"><fmt:formatDate pattern="yyyy-MM-dd" type="both" value="${projectStartedReport.labConstructApp.appDate.time}"/></td>
       <td class="label" valign="top">申请人：</td>
	   <td class="label" valign="top">${projectStartedReport.labConstructApp.user.cname}</td>

	</tr>
	<tr>
	    <td class="label" valign="top">工号：</td>
	    <td class="label" valign="top">${projectStartedReport.labConstructApp.user.username}</td>
		<td class="label" valign="top">性别：</td>
	    <td class="label" valign="top">${projectStartedReport.labConstructApp.user.userSexy}</td>
	</tr>
	<tr>
	   <td class="label" valign="top">联系电话：</td>
	   <td class="label" valign="top">${projectStartedReport.labConstructApp.user.telephone}</td>
	   <td class="label" valign="top">E-MAIL：</td>
	   <td class="label" valign="top">${projectStartedReport.labConstructApp.user.email}</td>
	</tr>
    <tr>
	   ...
	   <td class="label" valign="top">${projectStartedReport.labConstructApp.CProjectPurpose.name}</td> 	
	</tr>
	<tr>
	   <td class="label" valign="top">申报经费：</td>
	   <td class="label" valign="top"><form:input id="feeApp" path="feeApp"></form:input></td> 
       <td class="label" valign="top">批复经费（万元）：</td>
	   <td class="label" valign="top">${projectStartedReport.feeAmount}</td> 	
	</tr>
    <tr>
	   <td class="label" valign="top">经费编号：</td>
	   <td class="label" valign="top"><form:input id="feeCode" path="feeCode"></form:input></td> 
	   <td class="label" valign="top">启动日期<font color=red>*</font>：</td>
	   <td class="label" valign="top"><input id="start_date" name="startDate" value="${date}" onclick="new Calendar().show(this);"  readonly="readonly"  required="true"/></td> 
	</tr>
    <tr>
       <td class="label" valign="top">实训室地址：</td>
	   <td class="label" valign="top"><form:input id="labAddress" path="labAddress"></form:input></td>
       <td class="label" valign="top">实训室面积（m2）：</td>
	   <td class="label" valign="top"><form:input id="labArea" path="labArea"></form:input></td>
    </tr>
</tbody>
</table> 
</fieldset>   
	...
	<legend>经费预算</legend>
		<table>
		  ...
		  <table>
			...
			<tbody id="add_money">
				<c:forEach items="${projectStartedReport.projectStartFeeLists}" var="fee">
				<tr>
				<td>${fee.CFundAppItem.name}</td>
				<td>${fee.amount} </td>
				<td>${fee.budgetaryItem}</td>
				<td>
				<a onclick="return confirm('确认要删除吗？')" href="${pageContext.request.contextPath}/labconstruction/deleteProjectStartFeeList?id=${fee.id}&AppId=${projectStartedReport.id}">删除</a>
				<a href="${pageContext.request.contextPath}/labconstruction/updateProjectStartFeeList?id=${fee.id}&AppId=${projectStartedReport.id}&flag=2" >修改</a>
				</td>
				</tr>
				</c:forEach>
			</tbody>
		...
		<table>
		  ...
		  <table>
			...
			<tbody id="add_device">
				...
				<td>
				<a onclick="return confirm('确认要删除吗？')" href="${pageContext.request.contextPath}/labconstruction/deleteProjectStartDevice?id=${device.id}&AppId=${projectStartedReport.id}">删除</a>
				<a href="${pageContext.request.contextPath}/labconstruction/updateProjectStartDevice?id=${device.id}&AppId=${projectStartedReport.id}&flag=2" >修改</a>
				</td>
		...
		<table>
		  ...
		  <table>
			...
			<tbody id="add_Item">
				...
				<td>
				<a onclick="return confirm('确认要删除吗？')" href="${pageContext.request.contextPath}/labconstruction/deleteProjectStartCompletionItem?id=${item.id}&AppId=${projectStartedReport.id}">删除</a>
				<a href="${pageContext.request.contextPath}/labconstruction/updateProjectStartCompletionItem?id=${item.id}&AppId=${projectStartedReport.id}&flag=2" >修改</a>
				</td>
				</tr>
				</c:forEach>		
			</tbody>
...
<fieldset class="introduce-box">
						  ...
						  <tbody id="doc">
						  	<c:forEach items="${CommonDocuments}" var="doc" varStatus="i">
								<tr >
							 	...
							 	<td><a onclick="return confirm('确认要删除吗？')" href="${pageContext.request.contextPath}/labconstruction/deleteProjectStartedReportCommonDocument?id=${doc.id}&AppId=${projectStartedReport.id}">删除</a> </td>
								</tr>
							</c:forEach>	
						  </tbody>
					 </table>	
	...
	<div class="moudle_footer" style="background: white">
		<div class="submit_link" style="margin:5px;">
			<input class="btn btn-big" value="确定"  onclick="sub('${pageContext.request.contextPath}/labconstruction/saveProjectStartedReport');"/>
			<input class="btn btn-return" type="button" value="取消" onclick="window.history.go(-1);"/>
		</div>
	</div>
			<%--<input type="button" value="确定" onclick="sub('${pageContext.request.contextPath}/labconstruction/saveProjectStartedReport');"/>--%>
