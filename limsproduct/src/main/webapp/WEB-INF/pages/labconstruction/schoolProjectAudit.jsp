	  ...
	  <div class="TabbedPanelsContentGroup">
		<div class="TabbedPanelsContent">
			<div class="content-box">
				...
				<c:if test="${labConstructionProject.stage==2 }"> <!-- 教务处审核完成阶段，现在是校领导审核 -->
				<c:if test="${fn:contains(schoolLeaders,user.username) && labConstructionProject.auditResults==2 }"> <!-- 当前登陆人是校领导，当前处于审核中状态 -->
						<form:form action="${pageContext.request.contextPath}/labconstruction/saveSchoolProjectAudit?labConstructionProjectId=${labConstructionProject.id}" method="post" modelAttribute="schoolProjectAudit">
							<table>
								<tr>
									<td>审核意见</td>
				...
				<c:if test="${fn:indexOf(schoolLeaders,user.username)==-1 && labConstructionProject.stage<=2 && labConstructionProject.stage!=-1 }"> 
				<!-- 当前登陆人不是校领导，当前还不到校领导审核阶段，当前不是审核拒绝状态 -->
			...
			<c:if test="${labConstructionProject.stage>2 || labConstructionProject.stage==-1}">  <!-- 想要看到当前的审核意见，必须高于当前阶段 /或者审核拒绝状态也可以-->
				<tr>
					<td>审核意见：</td>
					<td>${schoolProjectAuditDone.comments}</td>
				</tr>
				<tr>	
					<td>审核结果：</td>
					<td>${schoolProjectAuditDone.CDictionary.CName}</td>
				</tr>
			</c:if>
