        ...
        success:function(data){//AJAX查询成功
        		if(data == 'success'){
        			if(${isRest==0}){
        				...
        				var url = "${pageContext.request.contextPath}/device/saveLabManagerAuditRest/" + ${labRoomId} + "/"+ ${schoolTermId} + "/" + "${deviceName}" + "/"+${page}+"/"+resultId+"/"+remark+"/"+id;
        				window.location.href=url;
        			}
        		}
        		if(data == 'error'){
        			alert("你预约的时间和其他预约存在时间冲突或者您的预约时间小于当前时间!");
        			window.location.href="${pageContext.request.contextPath}/device/labManagerReservationAudit?id="+id;
        			if(${isRest==1}){
	        			var url2 = "${pageContext.request.contextPath}/device/labManagerReservationAuditRest/" + ${labRoomId} + "/"+ ${schoolTermId} + "/" + "${deviceName}" + "/"+${page}+"/"+id;
	 ...
	 <!--内容栏  --> 
	  <div class="TabbedPanelsContentGroup">
		<div class="TabbedPanelsContent">
			...
			<%-- <c:if test="${reservation.labRoomDevice.CActiveByLabManagerAudit.id==1}"> --%>  <!-- 需要实训室管理员审核-->
			<c:if test="${reservation.labRoomDevice.CDictionaryByLabManagerAudit.CCategory=='c_active' && reservation.labRoomDevice.CDictionaryByLabManagerAudit.CNumber=='1' }">
			<%-- <c:if test="${isUnderLabManagerAudit && reservation.CAuditResult.id==1}"> --%> <!-- stage为1，或stage为0且不需要导师审核 -->
			<c:if test="${isUnderLabManagerAudit && reservation.CAuditResult.CCategory=='c_audit_result' && reservation.CAuditResult.CNumber=='1'}">
				<c:if test="${fn:contains(reservation.labRoomDevice.labRoom.labRoomAdmins,username)}">  <!-- 当前登陆人是实训室管理员  -->
					<div class="title">实训室管理员意见</div>
							<form:form name="auditform" action="${pageContext.request.contextPath}/device/saveLabManagerAudit?id=${reservation.id}" method="post" modelAttribute="audit">
							<table>
								<tr>
									<td>审核意见</td>
								...
								<tr>
	              					...
	              					<td>
										...
										到
		   	 							...
		   	 							&nbsp;
		   	 							<a href="${pageContext.request.contextPath}/device/modifyReserva/${reservation.labRoomDevice.id}/${reservation.id}/2">修改时间</a>
	              					</td>
	              					<td>
	              					</td>
								</tr> 
				...
				<c:if test="${empty labManageAudit && fn:indexOf(reservation.labRoomDevice.labRoom.labRoomAdmins,user.username)==-1}"> <!-- 未有实训室管理员审核意见，且当前登录人不是实训室管理员-->
					<table id="listTable" width="50%" cellpadding="0" cellspacing="0" class="tablesorter" >
					    <tr><td colspan="6">实训室管理员信息 </td></tr>
					    <c:forEach items="${reservation.labRoomDevice.labRoom.labRoomAdmins}" var="admin">
					 	<tr align="center" >
			...
			<%-- <c:if test="${reservation.stage>1 || (reservation.stage==1 && reservation.labRoomDevice.CActiveByTeacherAudit.id==2)
			 ||reservation.stage==-1}"> <!-- 审核阶段高于当前阶段，或者审核拒绝 --> --%>	
			<c:if test="${reservation.stage>1 || (reservation.stage==1 && reservation.labRoomDevice.CDictionaryByTeacherAudit.CCategory=='c_active' &&reservation.labRoomDevice.CDictionaryByTeacherAudit.CNumber=='2')
			 ||reservation.stage==-1}"> <!-- 审核阶段高于当前阶段，或者审核拒绝 -->	
				<table id="listTable" width="50%" cellpadding="0" cellspacing="0" class="tablesorter">
						  <tr><td colspan="6" ><font color="blue">实训室管理员审核</font> </td></tr>
						  <c:if test="${not empty labManageAudit }">
								<tr>
									<td>审核意见：${labManageAudit.remark }  </td>
									<%-- <td>审核结果：${labManageAudit.CTrainingResult.name}</td> --%>
									<td>审核结果：${labManageAudit.CTrainingResult.CName}</td>
								</tr>
						  </c:if>
						  <c:if test="${empty labManageAudit }">
						  	<tr>
							  <td>
							  	<font color="red">没有实训室管理员审核意见。</font>
							  </td>
						  	</tr>
						  </c:if>		
				</table>
		...
		<%-- <c:if test="${reservation.labRoomDevice.CActiveByLabManagerAudit.id!=1}"> --%>  <!-- 不需要实训室管理员审核-->
		<c:if test="${reservation.labRoomDevice.CDictionaryByLabManagerAudit.CCategory=='c_active' &&reservation.labRoomDevice.CDictionaryByLabManagerAudit.CNumber!='1'}">  
			<table id="listTable" width="50%" cellpadding="0" cellspacing="0" class="tablesorter">
						  <tr><td colspan="6"><font color="blue">实训室管理员审核情况</font> </td></tr>
								<tr>
									<td>您好，该设备不需要实训室管理员审核。</td>
								</tr>
			</table>
		</c:if>
