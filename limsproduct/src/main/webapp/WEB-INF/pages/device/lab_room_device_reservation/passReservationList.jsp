...
function cancelQuery(){
	window.location.href="${pageContext.request.contextPath}/device/passReservationList?page=1";
}	

//审核通过后的审核意见修改
function alterAudit(id) {
    var sessionId=$("#sessionId").val();
    var con = '<iframe scrolling="yes" id="alter" frameborder="0"  src="${pageContext.request.contextPath}/device/alterAudit?id='+id+'&tag=1"  style="width:100%;height:100%;"></iframe>'
    $("#alterAudit").html(con);  
    //获取当前屏幕的绝对位置
...
<div id="TabbedPanels1" class="TabbedPanels">
	...
	<div class="TabbedPanelsContentGroup">
		<div class="TabbedPanelsContent">
		     <div class="tool-box">
		     <form:form name="queryForm" action="${pageContext.request.contextPath}/device/passReservationList?page=1" method="post" modelAttribute="reservation">
            ...
            <table class="tb" id="my_show"> 
                ...
                <tbody>
                       ...
                       <td>
                        ...
                        <sec:authorize ifAnyGranted="ROLE_SUPERADMIN,ROLE_EQUIPMENTADMIN,ROLE_TEACHER">
                        <td><a href="${pageContext.request.contextPath}/device/labRoomReservationCredit?id=${reservation.id}&page=${page}">信誉登记</a></td>
                        </sec:authorize>
                        <sec:authorize ifAnyGranted="ROLE_SUPERADMIN,ROLE_EQUIPMENTADMIN,ROLE_TEACHER">
                        <td>
                        ...
                        <%-- <c:if test="${(reservation.CAuditResult.id==2)}"> --%>
                        <sec:authorize ifAnyGranted="ROLE_SUPERADMIN,ROLE_EQUIPMENTADMIN">
	                        <c:if test="${reservation.CAuditResult.CCategory=='c_audit_result' && reservation.CAuditResult.CNumber =='2'}">
...
<div class="page" >
    ...
    <a href="javascript:void(0)"    onclick="first('${pageContext.request.contextPath}/device/passReservationList?page=1')" target="_self">首页</a>			    
	<a href="javascript:void(0)" onclick="previous('${pageContext.request.contextPath}/device/passReservationList?page=')" target="_self">上一页</a>
	第<select onchange="javascript:window.location.href = this.options[this.selectedIndex].value;">
	<option value="${pageContext.request.contextPath}/device/passReservationList?page=${page}">${page}</option>
	<c:forEach begin="${pageModel.firstPage}" end="${pageModel.lastPage}" step="1" varStatus="j" var="current">	
    <c:if test="${j.index!=page}">
    <option value="${pageContext.request.contextPath}/device/passReservationList?page=${j.index}">${j.index}</option>
    </c:if>
    </c:forEach></select>页
	<a href="javascript:void(0)" onclick="next('${pageContext.request.contextPath}/device/passReservationList?page=')" target="_self">下一页</a>
 	<a href="javascript:void(0)" onclick="last('${pageContext.request.contextPath}/device/passReservationList?page=${pageModel.totalPage}')" target="_self">末页</a>
    </div>
<!-- 分页模块 -->
</div>
