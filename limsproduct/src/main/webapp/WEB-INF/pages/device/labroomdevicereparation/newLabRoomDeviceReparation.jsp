...
<div id="navigation">
<ul>
	<li><a href="javascript:void(0)">易耗品赔偿流水账-新建</a></li>
</ul>
</div>
</div>
...
<div class="content-box">
	  ...
	  <c:forEach items="${ID3}" var="current1"  varStatus="i">	
	    <form:option value="${current1.username}" >工号：${current1.username} 姓名：${current1.cname}</form:option>
			...
			<td>
			<form:select class="chzn-select"  path="userByTeacher.username" id="userByCreateUser.username" cssStyle="width:200px" >
	        <c:forEach items="${ID3}" var="current1"  varStatus="i">	
	        <form:option value="${current1.username}" >工号：${current1.username} 姓名：${current1.cname}</form:option>
	        </c:forEach>
            </form:select>
