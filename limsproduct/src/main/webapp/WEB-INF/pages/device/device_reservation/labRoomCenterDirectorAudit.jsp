...
<c:if test="${state == 4 && isAudit == 1}">
   <table>
        <tr>
           ...
           <input type="radio" name="auditResult4"  value="2" />拒绝
            </td>
       </tr>
        <tr>
...
function savelabRoomCenterDirectorAudit(){
	...
	window.location.href="${pageContext.request.contextPath}/LabRoomDeviceReservation/savelabRoomCenterDirectorAudit?id=${id}&tage=${tage}&state=${state }&currpage=${currpage}&auditResult="+auditResult+"&remark="+remark;
}
</script>
</body>
<!-------------列表结束----------->
