...
<%--导师未审核时显示--%>
<c:if test="${state == 1 && isAudit == 0}">
 <table id="listTable" width="50%" cellpadding="0" cellspacing="0" class="tablesorter" >
 <tr><td colspan="6"><font style="color: red">导师尚未审核</font></td></tr>
 <tr align="center" >
     <td>审核人：</td>
     <td>${teacher.cname }</td> 
     <td>工号:</td>
...
<%--导师审核时显示--%>
<c:if test="${state == 1 && isAudit == 1}">
 <form:form action="${pageContext.request.contextPath}/LabRoomDeviceReservation/saveTeacherAudit?id=${id}&tage=${tage}&state=${state }&currpage=${currpage}"  method="post"  modelAttribute="labRoomDeviceReservationResult" target="_parent" >
   <table>
        <tr>
           <td>审核：</td>
           <td colspan="4">
           <form:radiobutton path="CDictionaryByCTrainingResult.id"  value="619" checked="true"/>通过
           <form:radiobutton path="CDictionaryByCTrainingResult.id"  value="620" />拒绝  
            </td>
       </tr>
        <tr>
           <td>审核意见：</td><td colspan="4"><form:input path="remark" /> </td>
       </tr>
        <tr>
...
<tr align="center" >
     ...
     <c:if test="${lStationReservationResult.CDictionaryByCTrainingResult eq '2'}">
