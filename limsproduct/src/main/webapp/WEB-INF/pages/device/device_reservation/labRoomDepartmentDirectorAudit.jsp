...
<%--导师未审核时显示--%>
<c:if test="${state le 5 && isAudit == 0}">
...
<%--导师审核时显示--%>
<c:if test="${state == 5 && isAudit == 1}">
   <table>
        <tr>
           <td>审核：</td>
           <td colspan="4">
           <input type="radio" name="auditResult5"  value="1" checked="true"/>通过
           <input type="radio" name="auditResult5"  value="2" />拒绝
            </td>
       </tr>
        <tr>
           <td>审核意见：</td><td colspan="4"><input type="text" id="remark5"  /> </td>
       </tr>
        <tr> 
           <td colspan="5"><input type="button" value="提交" onclick="savelabRoomDepartmentDirectorAudit()"> </td>
       </tr>
 </table>
</c:if>
<%--导师审核后显示--%>
<c:if test="${state gt 5 }">
   <table id="listTable" width="50%" cellpadding="0" cellspacing="0" class="tablesorter" >
...
function savelabRoomDepartmentDirectorAudit(){
	var auditResult = $('input:radio[name="auditResult5"]:checked').val();
	var remark = $("#remark5").val();
	//alert($('input:radio[name="auditResult2"]:checked').val());
	window.location.href="${pageContext.request.contextPath}/LabRoomDeviceReservation/savelabRoomDepartmentDirectorAudit?id=${id}&tage=${tage}&state=${state }&currpage=${currpage}&auditResult="+auditResult+"&remark="+remark;
}
</script>
</body>
<!-------------列表结束----------->
