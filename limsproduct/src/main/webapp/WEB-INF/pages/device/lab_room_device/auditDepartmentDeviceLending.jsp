	  ...
	  <div class="TabbedPanelsContentGroup">
		<div class="TabbedPanelsContent">
			<div class="content-box">
				...
				<c:if test="${lrdl.userByDepartmentHead.username==username&&(lrdl.stage==0||lrdl.stage==null)}">
					<div class="title">系主任意见</div>
				...
				<c:if test="${empty labManageAudit && fn:indexOf(reservation.labRoomDevice.labRoom.labRoomAdmins,user.username)==-1}"> <!-- 未有实训室管理员审核意见，且当前登录人不是实训室管理员-->
					<table id="listTable" width="50%" cellpadding="0" cellspacing="0" class="tablesorter" >
					    <tr><td colspan="6">系主任信息 </td></tr>
					 	<tr align="center" >
						     <td>姓名：   ${lrdl.userByDepartmentHead.cname}</td> 
						     <td>工号：   ${lrdl.userByDepartmentHead.username}</td> 
						     <td>部门：   ${lrdl.userByDepartmentHead.schoolAcademy.academyName}</td> 
						     <td>联系方式：   ${lrdl.userByDepartmentHead.telephone}</td> 
						</tr>  
					</table>
				</c:if>
				<c:if test="${lrdl.stage>=1||lrdl.stage==-3||lrdl.stage==-2}">
				<table id="listTable" width="50%" cellpadding="0" cellspacing="0" class="tablesorter">
						  <tr><td colspan="6" ><font color="blue">系主任审核</font> </td></tr>
								<tr>
									<td>审核意见：通过  </td>
								</tr>		
				</table>
				</c:if>
				<c:if test="${lrdl.stage==-1}">
					<table id="listTable" width="50%" cellpadding="0" cellspacing="0" class="tablesorter">
						<tr><td colspan="6" ><font color="blue">系主任审核</font> </td></tr>
						<tr>
...
<script type="text/javascript">
    ...
    function saveAudit(){
        ...
        $.ajax({
            url:'${pageContext.request.contextPath}/device/saveDepartmentDeviceLending?array_ok='
