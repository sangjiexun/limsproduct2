...
<div id="TabbedPanels1" class="TabbedPanels">
	<ul class="TabbedPanelsTabGroup">
		...
		<a href="${pageContext.request.contextPath}/device/allLendableDeviceList?page=1">可借用设备列表</a>
		</li>
		<li class="TabbedPanelsTab " tabindex="0">
	...
	<div class="TabbedPanelsContentGroup">
		<div class="TabbedPanelsContent">
		    <%--  <div class="tool-box">
		     <form:form name="queryForm" action="${pageContext.request.contextPath}/device/returnedDeviceKeepList?page=1" method="post" modelAttribute="reservation">
...
<div class="page" >
    ...
    <a href="javascript:void(0)"    onclick="first('${pageContext.request.contextPath}/device/returnedDeviceKeepList?page=1')" target="_self">首页</a>			    
	<a href="javascript:void(0)" onclick="previous('${pageContext.request.contextPath}/device/returnedDeviceKeepList?page=')" target="_self">上一页</a>
	第<select onchange="javascript:window.location.href = this.options[this.selectedIndex].value;">
	<option value="${pageContext.request.contextPath}/device/returnedDeviceKeepList?page=${page}">${page}</option>
	<c:forEach begin="${pageModel.firstPage}" end="${pageModel.lastPage}" step="1" varStatus="j" var="current">	
    <c:if test="${j.index!=page}">
    <option value="${pageContext.request.contextPath}/device/returnedDeviceKeepList?page=${j.index}">${j.index}</option>
    </c:if>
    </c:forEach></select>页
	<a href="javascript:void(0)" onclick="next('${pageContext.request.contextPath}/device/returnedDeviceKeepList?page=')" target="_self">下一页</a>
 	<a href="javascript:void(0)" onclick="last('${pageContext.request.contextPath}/device/returnedDeviceKeepList?page=${pageModel.totalPage}')" target="_self">末页</a>
    </div>
<!-- 分页模块 -->
</div>
