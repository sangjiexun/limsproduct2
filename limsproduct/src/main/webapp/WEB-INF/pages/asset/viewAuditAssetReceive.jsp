...
<div class="main_container cf rel w95p ma">
  ...
  <div class="title">申领单基本信息</div>
    <table id="listTable" width="50%" cellpadding="0">
      <tr>
        <th>申请编号：</th><td style="background-color: #ffffff">${assetReceive.receiveNo}</td>
        <th>申请时间：</th><td style="background-color: #ffffff"><fmt:formatDate value="${assetReceive.receiveDate.time}" pattern="yyyy-MM-dd" /></td>
      </tr>
      <tr>
      ...
      <tr>
      <th>开始时间：</th><td style="background-color: #ffffff"><fmt:formatDate value="${assetReceive.startData.time}" pattern="yyyy-MM-dd hh:mm" /></td>
      <th>结束时间：</th><td style="background-color: #ffffff"><fmt:formatDate value="${assetReceive.endDate.time}" pattern="yyyy-MM-dd hh:mm" /></td>
      </tr>
    </table>
  </div>
  </div>
  ...
  <div class="right-content" style="margin-top: 25px">
    <div id="TabbedPanels1" class="TabbedPanels">
      <div class="TabbedPanelsContentGroup">
        <div class="TabbedPanelsContent">
              ...
              <tbody>
                <c:forEach items="${listAssetReceiveRecords }" var="curr" varStatus="i">
                  ...
                  <c:if test="${assetReceive.status eq 1 || assetReceive.status eq 0}">
                    ...
                    <c:if test="${curr.asset.specifications eq null}">
	  ...
	  <div id="TabbedPanels1" class="TabbedPanels">
          ...
          <div class="content-box" style="margin-top: 25px">
              <div class="title">审核结果</div>
                <table id="listTable" width="50%" cellpadding="0">
                  <tr>
                    <th>工号：</th><td style="background-color: #ffffff">${assetReceive.user.username}</td>
                    <th>姓名：</th><td style="background-color: #ffffff">${assetReceive.user.cname}</td>
                  </tr>
                    <tr>
                        <th>审核结果：</th>
                        <td>
                            <c:if test="${assetReceive.status eq 1}">审核通过</c:if>
                            <c:if test="${assetReceive.status eq 0}">审核拒绝</c:if>
                        </td>
                        <th>审核意见：</th><td>${assetReceive.mem}</td>
                        <th>审核时间：</th><td><fmt:formatDate value="${assetAppAudit.createDate}" pattern="yyyy年MM月dd日 hh:mm" /></td>
                    </tr>
              </table>
          </div>
        </div>
