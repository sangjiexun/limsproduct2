...
//AJAX验证是否有不规范的字段
	...
	var id = ${id};
	var appId = ${assetCabinetWarehouseAccess.assetAppRecord.assetApp.id};
	$.ajax({
        url:"${pageContext.request.contextPath}/asset/checkQuantity?id="+id,
        type: 'POST',   
        async: false,  
        cache: false,  
        contentType: false,  
        ...
        success: function(data) {
        	if(data == 'match'){ 
        		 window.location.href="${pageContext.request.contextPath}/asset/viewAppRecordNeedStocks?id="+appId;
  ...
  function cancel()
  {
	  window.location.href="${pageContext.request.contextPath}/asset/viewAppRecordNeedStocks?id="+${appRecordId};
  }
  //跳转
  ...
  <div class="content-box">
	...
	<table class="tb" id="my_show">
	  ...
	  <tr>
	    ...
	    <td><a href="${pageContext.request.contextPath}/asset/deleteLocationMessages?id=${curr.id}" onclick="return confirm('确定删除？');">删除</a></td>
	  </tr>
	  </c:forEach>
	  </tbody>
	...
	<div id="addLocation" class="easyui-window " title="添加" align="left" modal="true" maximizable="false" collapsible="false" closed="true" minimizable="false" style="width: 500px; height: 250px;">
					<form:form action="${pageContext.request.contextPath}/asset/saveLoctaionMessages?id=${id}" modelAttribute="accessRecord">
						<table>
							<tr>
						    <label>药品名称：</label>
