		  ...
		  <div class="TabbedPanelsContentGroup">
			  <div class="TabbedPanelsContent">
				  ...
				  <fieldset class="introduce-box">
					...
					<c:forEach items="${ assetAppAudits}" var="curr">
						<fieldset class="introduce-box">
							<legend>审核人信息</legend>
								<table id="listTable" width="50%" cellpadding="0">
									<tr>
										<th>姓名：</th><td>${curr.user.cname}</td>
										<th>工号：</th><td>${curr.user.username}</td> 
									</tr>
							</table>
						</fieldset>
						<fieldset class="introduce-box">
									...
									<table id="listTable" width="50%" cellpadding="0">
										<tr>
											...
											<td>
												...
												<c:if test="${curr.status eq 0}">审核拒绝</c:if>
											</td>
											<th>审核意见：</th><td>${curr.result}</td>
											<th>审核时间：</th><td><fmt:formatDate value="${curr.createDate.time}" pattern="yyyy-MM-dd" /></td>
										</tr>
								</table>
						</fieldset>
					</c:forEach>
