...
<body>
  ...
  <div class="content-box">
	...
	<table class="tb" id="my_show">
	  ...
	  <tbody>
	  <c:forEach items="${listAccesss}" var="curr" varStatus="i">
	  <tr>
	    <td>${i.count+pageSize*(currpage-1)}</td> 
	    <td>${curr.asset.chName}[${curr.asset.specifications}]</td>  
	    <td>${curr.asset.cas}</td>
	    <td>${curr.assetAppRecord.styleNumber}</td>  
	    <td>
	     <c:forEach items="${curr.assetCabinetWarehouseAccessRecords}" var="currRecord">
	    	${ccurrRecord.assetCabinetWarehouse.assetCabinet.labRoom.labRoomName}
	    </c:forEach>
	    </td> 
	    <td>
	    	<c:forEach items="${curr.assetCabinetWarehouseAccessRecords}" var="currRecord">
	    	${currRecord.assetCabinetWarehouse.assetCabinet.cabinetName}
	    	</c:forEach>
	    </td> 
	    <td>
		    <c:forEach items="${curr.assetCabinetWarehouseAccessRecords}" var="currRecord">
		    ${currRecord.assetCabinetWarehouse.warehouseName}
		    </c:forEach>
	    </td>
	    <td>
	    <c:forEach items="${curr.assetCabinetWarehouseAccessRecords}" var="currRecord">
	    ${currRecord.cabinetQuantity}
	    </c:forEach>
	    </td>
	    <td>
	    	<a href="${pageContext.request.contextPath}/asset/modifyBelongOperationItem?id=${curr.id}&modifyId=${id}">修改所属项目</a> 
	    </td>
	  </tr>
	  </c:forEach>
	  </tbody>
