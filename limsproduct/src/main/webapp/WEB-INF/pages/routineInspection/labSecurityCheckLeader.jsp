                ...
                <div class="content-box">
                    <div class="tool-box">
                        <form:form name="queryForm"
                                   action="${pageContext.request.contextPath}/leaderLabSecurityCheck?currpage=1"
                    ...
                    <div class="content-box">
                        <table class="tb" id="my_show">
                            ...
                            <c:forEach items="${labSecurityChecks}" var="curr">
                                <tr>
                                    ...
                                    <td>
                                        <%--<input type="button" value="查看" onclick="dosubmitCheck(${curr.id},3);"/>--%>
                                        <a href="javascript:void(0)" onclick="dosubmitCheck(${curr.id},3);">查看</a>
                                    </td>
                                </tr>
                            </c:forEach>
                            </tbody>
                        ...
                        <div class="page">
                            ...
                            <a href="javascript:void(0)"
                               onclick="first('${pageContext.request.contextPath}/leaderLabSecurityCheck?currpage=1')"
                               target="_self">首页</a>
                            <a href="javascript:void(0)"
                               onclick="previous('${pageContext.request.contextPath}/leaderLabSecurityCheck?currpage=')"
                               target="_self">上一页</a>
                            第<select
                                onchange="javascript:window.location.href = this.options[this.selectedIndex].value;">
                            <option value="${pageContext.request.contextPath}/leaderLabSecurityCheck?currpage=${page}">${page}</option>
                            <c:forEach begin="${pageModel.firstPage}" end="${pageModel.lastPage}" step="1" varStatus="j"
                                       var="current">
                                <c:if test="${j.index!=page}">
                                    <option value="${pageContext.request.contextPath}/leaderLabSecurityCheck?currpage=${j.index}">${j.index}</option>
                                </c:if>
                            </c:forEach></select>页
                            <a href="javascript:void(0)"
                               onclick="next('${pageContext.request.contextPath}/leaderLabSecurityCheck?currpage=')"
                               target="_self">下一页</a>
                            <a href="javascript:void(0)"
                               onclick="last('${pageContext.request.contextPath}/leaderLabSecurityCheck?currpage=${pageModel.totalPage}')"
                               target="_self">末页</a>
                        </div>
                    </div>
                </div>
