...
<script type="text/javascript">
	...
	function showContent(obj){
		...
		parent.attr("class","content_right");
		parent.html($(obj).next().html());
		$(".list_box2").html(name);
...
<!-- 当前位置栏开始  -->
    <div class="nav">
			...
			<c:if test="${secondParentTCourseSiteChannel!=null }">
				<a href="${pageContext.request.contextPath}/cms/channel/findChannelList?tagId=${secondParentTCourseSiteChannel.TCourseSiteTag.id }">${secondParentTCourseSiteChannel.channelName}</a>&nbsp;>&nbsp;
				<c:if test="${parentTCourseSiteChannel.TCourseSiteChannelsForChannelId.size()==0 }">
					<a href="${pageContext.request.contextPath}/cms/channel/findArticalsByChannelId?channelId=${parentTCourseSiteChannel.id }&currpage=1">${parentTCourseSiteChannel.channelName}</a>&nbsp;>&nbsp;
				</c:if>
				<c:if test="${parentTCourseSiteChannel.TCourseSiteChannelsForChannelId.size()>0 }">
					<a href="${pageContext.request.contextPath}/cms/channel/findSecondChannelsByChannelId?channelId=${parentTCourseSiteChannel.id }">${parentTCourseSiteChannel.channelName}</a>&nbsp;>&nbsp;
				</c:if>
				<a href="${pageContext.request.contextPath}/cms/channel/findArticalsByChildenChannelId?channelId=${tCourseSiteChannel.id }&currpage=1">${tCourseSiteChannel.channelName}</a>
			</c:if>
			<c:if test="${secondParentTCourseSiteChannel==null }">
...
<!-- 左侧栏开始  -->
    <div class="content_list_left">
	    <div class="list_left_title">
    		...
    		<c:if test="${secondParentTCourseSiteChannel !=null}">
   				${secondParentTCourseSiteChannel.channelName}
    		</c:if>
    		<c:if test="${secondParentTCourseSiteChannel ==null}">
   				${parentTCourseSiteChannel.channelName}
    		</c:if>
	    </div>
		     ...
		     <ul>
				<c:forEach items="${tCourseSiteChannels}" var="current" varStatus="i">
					<li>
						...
						<c:if test="${current.TCourseSiteChannelsForChannelId.size()>0 }">
							<a href="${pageContext.request.contextPath}/cms/channel/findSecondChannelsByChannelId?channelId=${current.id }" <c:if test="${tCourseSiteChannel.id==current.id||(isCourseware&&current.TCourseSiteTag.id == 21) }">style="color:#0146a8; font-weight:bold"</c:if>>${current.channelName}</a>&nbsp;&nbsp;
						</c:if>
					</li>
				</c:forEach>
			 </ul>
						...
						<c:forEach items="${tCourseSiteArticals}" var="tCourseSiteArtical" varStatus="i">
							<li>
								...
								<span style="display: none">
									<c:if test="${isCourseware }">
										<c:if test="${tCourseSiteArtical.coursewarePath!=null&&tCourseSiteArtical.coursewarePath ne ''}">
											<iframe src="${pageContext.request.contextPath}${tCourseSiteArtical.coursewarePath}" width="600px" height="400px" ></iframe>
										</c:if>
									</c:if>
									<c:if test="${!isCourseware }">
										${tCourseSiteArtical.content }
									</c:if>
								</span>
