		  	...
		  	<div class="TabbedPanelsContentGroup">
				...
				<div class="TabbedPanelsContent">
					<div class="content-box">
						...
						<%-- <form:form action="${pageContext.request.contextPath}/teaching/exam/saveTAssignmentGradeForExam?status=1" method="POST" >
 --%>						<form:form action="${pageContext.request.contextPath}/teaching/exam/saveTAssignmentItemMapping?status=1" method="POST" >
						...
						<div class="new-classroom">
                                       		...
                                       		<tr>
												...
												<b>(分值：${current1.score})</b><br>
                                                   ...
                                                   <c:forEach items="${current1.TAssignmentAnswers}" var="current2"  varStatus="k">
                                                        ...
                                                        <c:forEach items="${tAssignmentItemMappingList}" var="current3"  varStatus="j">
	                                                        <!--多选题  -->
	                                                        <c:if test="${current1.type==1&&current3.TAssignmentItem.id==current1.id&&current2.id==current3.TAssignmentAnswer.id  }">
	                                                            <input checked="true" type="checkbox" name="answers${current1.id}" id="answers${current1.id}" value="${current2.id}"/>${current2.label}: <c:out value="${current2.text}"></c:out><br>
	                                                            <c:set var="flag" value="1"></c:set>
	                                                        </c:if>
	                                                        <!--对错题,且如果tAssignmentItemMapping有记录，则选中  -->
	                                                        <c:if test="${current1.type==2&&current3.TAssignmentItem.id==current1.id&&current2.id==current3.TAssignmentAnswer.id }"><c:out value="${current2.text}"></c:out>
	                                                            <input checked="true" type="radio" name="answers${current1.id}" id="answers${current1.id}" value="${current2.id}"/><br>
	                                                        	<c:set var="flag" value="1"></c:set>
	                                                        </c:if>
	                                                        <!--单选题,且如果tAssignmentItemMapping有记录，则选中  -->
	                                                        <c:if test="${current1.type==4&&current3.TAssignmentItem.id==current1.id&&current2.id==current3.TAssignmentAnswer.id }">
	                                                            <input checked="true" type="radio" name="answers${current1.id}" id="answers${current1.id}" value="${current2.id}"/>${current2.label}: <c:out value="${current2.text}"></c:out><br>
	                                                        	<c:set var="flag" value="1"></c:set>
	                                                        </c:if>
	                                                        <!--填空题,且如果tAssignmentItemMapping有记录，则显示  -->
	                                                        <c:if test="${current1.type==8&&current3.TAssignmentItem.id==current1.id&&current2.id==current3.TAssignmentAnswer.id }">
	                                                            <input type="hidden" name="answers${current1.id}" value="${current2.id}"/>
                                                        		<input type="text" name="answertexts${current1.id}"  value="${current3.answerText }"/><br>
	                                                        </c:if>
                                                        </c:forEach>
                                                        <!-- 未选中答案显示 -->
	                                                    <c:if test="${flag==0&&current1.type==1 }">
							...
							<input type="hidden" name="submitTime" id="submitTime" value="1">
