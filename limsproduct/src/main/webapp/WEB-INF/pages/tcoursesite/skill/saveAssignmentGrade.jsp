...
<script type="text/javascript">
	...
	function downloadOneFile(id,tCourseSiteId,url){
		
		if(url==""||url==null){
			alert("无附件!");
		}else{
			var input = "<input type='hidden' name='id' value='"+ id +"' />";
			var html = "<form action='${pageContext.request.contextPath}/tcoursesite/assignment/downloadFileForStudent?tCourseSiteId="+tCourseSiteId+"' method='post'>"+input+"</form>"; 
		  	...
		  	<div class="TabbedPanelsContentGroup">
				<div class="TabbedPanelsContent">
					<div class="content-box">
						...
						<form:form id="myForm" action="${pageContext.request.contextPath}/tcoursesite/assignment/saveSubmitTAssignmentGrade?skillId=${skillId}&tCourseSiteId=${tCourseSite.id}" method="POST" modelAttribute="tAssignmentGrade" onsubmit="return checkForm()" enctype="multipart/form-data">
		...
		<tbody>
			<c:forEach items="${tAssignmentGradings}" var="current"  varStatus="i">
		   		<tr>
					...
					<td>
						<mytag:JspSecurity realm="check" menu="tAssignment">
						    <a href="javascript:void(0)" onclick="downloadOneFile(${current.accessmentgradingId},${tCourseSite.id},'${current.gradeUrl}')" >下载</a>
						</mytag:JspSecurity>
					</td>
				</tr>
			</c:forEach>
