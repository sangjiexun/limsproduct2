...
function uploadImageForLabRoom(type){
    ...
    $("#searchFile").window('open');
			 ...
			 $('#file_upload').uploadify({
	            ...
	            'uploader':'${pageContext.request.contextPath}/tcoursesite/visualization/uploadImageForLabRoom;jsessionid=<%=session.getId()%>',		//提交的controller和要在火狐下使用必须要加的id
	            buttonText:'选择文件',
	             onQueueComplete : function(data) {//当队列中的所有文件全部完成上传时触发	
	      			    //当上传玩所有文件的时候关闭对话框并且转到显示界面
	            	 $('#searchFile').window('close');	  
	            	 window.location.href="${pageContext.request.contextPath}/tcoursesite/visualization/addLabRoomImage?id="+${labRoom.id};          	
		}
	        });
	...
	function uploadVideoForLabRoom(){
			 ...
			 $('#video_upload').uploadify({
	            ...
	            'swf': '${pageContext.request.contextPath}/swfupload/swfupload.swf',  
	            'uploader':'${pageContext.request.contextPath}/tcoursesite/visualization/uploadVideoForLabRoom;jsessionid=<%=session.getId()%>',		//提交的controller和要在火狐下使用必须要加的id
	            buttonText:'选择文件',
	             onQueueComplete : function(data) {//当队列中的所有文件全部完成上传时触发	
...
function changeCampus(){
	...
	//alert("s的值为："+s);
			...
			$.post('${pageContext.request.contextPath}/appointment/findBuildByCampusId?id='+s+'',function(data){  //serialize()序列化
				$("#build").html(data);
...
function changeBuilds(){
	...
	//alert("s的值为："+s);
			//Ajax方法 根据网站id查询出所有的栏目
			$.post('${pageContext.request.contextPath}/appointment/findRoomByBuildNumber?buildNumber='+s+'',function(data){  //serialize()序列化
				$("#room").html(data);
...
function complate(){
	var s=$("#room").val();
	document.getElementById("roomNumber").value=s;
	  ...
	  <div class="TabbedPanelsContentGroup">
		<div class="TabbedPanelsContent">
			<div class="content-box">
				<div class="title">
				<div id="title">实训室资源</div>
				
				</div> 
<form:form action="${pageContext.request.contextPath}/tcoursesite/visualization/saveLabRoom" method="POST" modelAttribute="labRoom">
		<div class="new-classroom">
					<fieldset>
						    <label>实验室名称：</label>${labRoom.labRoomName}
					</fieldset>		

			  		<fieldset>
						     <label>实训室编号：</label>${labRoom.labRoomNumber}
					 </fieldset>
					 <fieldset>
					 		<label>实训室英文名称：</label>${labRoom.labRoomEnName}
					 </fieldset>
					  <fieldset>
					  		<label>实训室简称：</label>${labRoom.labRoonAbbreviation}
					 </fieldset>
					  <%--<fieldset>
					  		<label>实训室类别：</label>${labRoom.CLabRoomType.name}
					 </fieldset>
					  --%><fieldset>
						    <label>实训室地点：</label><input type="button" onclick="checkRoom();" value="选择" />
						    	<form:input path="labRoomAddress" id="roomAddress" disabled="true"/>
						    	<form:hidden path="systemRoom.roomNumber" id="roomNumber" name="roomNumber"/>
					</fieldset>
					...
					<fieldset class="introduce-box">
						    <label> 附件 ：<input type="button" onclick="uploadImageForLabRoom(2)" value="上传附件"/></label>
						    	...
						    	<a class="btn btn-common" href="${pageContext.request.contextPath}/tcoursesite/visualization/deleteImageForLabRoom?id=${labRoom.id}&type=2">删除</a>
						    	</li>
						    	</c:if>
						     </c:forEach>
						     </ul>
					...
					<fieldset class="introduce-box">
						     <label>楼层平面图: <input type="button" onclick="uploadImageForLabRoom(1)" value="上传图片"/></label>
						     <ul class="img-box">
						     <c:forEach items="${labRoom.commonDocuments}" var="d">
						     	<c:if test="${d.type==1}">
						    	<li>
						    	...
						    	<a class="btn btn-common" href="${pageContext.request.contextPath}/tcoursesite/visualization/deleteImageForLabRoom?id=${labRoom.id}&type=1">删除</a>
						    	</li>
						    	</c:if>
						     </c:forEach>
						     </ul>
					 ...
					 <fieldset class="introduce-box">
						     <label>全景图片: <input type="button" onclick="uploadImageForLabRoom(3)" value="上传全景图片"/></label>
						     <ul class="img-box">
						     <c:forEach items="${labRoom.commonDocuments}" var="d">
						     	<c:if test="${d.type==3}">
						    	<li>
						    	<img alt="${d.documentName}" src="${pageContext.request.contextPath}/${d.documentUrl}" border="0"  width="200px" height="150px" onclick="showImage(this);"> 
						    	<a class="btn btn-common" href="${pageContext.request.contextPath}/tcoursesite/visualization/deleteImageForLabRoom?id=${labRoom.id}&type=3">删除</a>
						    	</li>
						    	</c:if>
						     </c:forEach>
						     </ul>
					...
					<fieldset class="introduce-box">
						     <label>展示图片: <input type="button" onclick="uploadImageForLabRoom(4)" value="上传展示图片"/></label>
						     <ul class="img-box">
						     <c:forEach items="${labRoom.commonDocuments}" var="d">
						     	<c:if test="${d.type==4}">
						    	<li>
						    	<img alt="${d.documentName}" src="${pageContext.request.contextPath}/${d.documentUrl}" border="0"  width="200px" height="150px" onclick="showImage(this);"> 
						    	<a class="btn btn-common" href="${pageContext.request.contextPath}/tcoursesite/visualization/deleteImageForLabRoom?id=${labRoom.id}&type=4">删除</a>
						    	</li>
						    	</c:if>
						     </c:forEach>
						     </ul>
