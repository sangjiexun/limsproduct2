        ...
        <div class="course_cont r">
            ...
            <div class="course_mod f14 mb2">
                ...
                <div class=" lh40   pl30 f14 cf ">
                    <div class="bc l w20p poi" onclick="changeOrderExerciseType(${tCourseSite.id},${questionId },4)" <c:if test="${itemType==4 }">style="color:#000; font-weight:bold"</c:if>>单选题</div>
                    <div class="bc l w20p poi" onclick="changeOrderExerciseType(${tCourseSite.id},${questionId },1)" <c:if test="${itemType==1 }">style="color:#000; font-weight:bold"</c:if>>多选题</div>
                    <div class="bc l w20p poi" onclick="changeOrderExerciseType(${tCourseSite.id},${questionId },8)" <c:if test="${itemType==8 }">style="color:#000; font-weight:bold"</c:if>>填空题</div>
                </div>
                <div class="  mt2">
                    <div class="pl30  ptb10 ">
                        <div class="cf ">
                            <c:forEach items="${items }" var="item" varStatus="i">
								<div >
									...
									<h1>${(pageModel.currpage-1)*(pageModel.pageSize)+i.count }.
										<!-- 多选，对错，单选 -->
										<c:if test="${item.type==1||item.type==2||item.type==4 }">
											${item.description }
										</c:if>
										<!-- 填空 -->
										<c:if test="${item.type==8 }">
											${item.descriptionTemp }
										</c:if>
									</h1><br>
									<!-- 多选 -->
									<c:if test="${item.type==1 }">
										<c:forEach items="${item.TAssignmentAnswers }" var="answer" varStatus="j">										
											<input type="checkbox" value="${answer.iscorrect }" class="check_box" id='answer${answer.id }'/>
									...
									<br>
									<%--<button class="btn" id="checkAnswerButton" onclick="checkAnswer(this,${item.type})">查看答案</button>
                        ...
                        <div style="padding: 50px 150px">
							<button class="btn" onclick="window.location.href='${pageContext.request.contextPath }/tcoursesite/exercise/findOrderItemListBySiteIdAndQuestionId?currpage=${pageModel.previousPage}&tCourseSiteId=${tCourseSite.id}&questionId=${questionId }&itemType=${itemType }'">上一题</button>
							<button class="btn" onclick="window.location.href='${pageContext.request.contextPath }/tcoursesite/exercise/findOrderItemListBySiteIdAndQuestionId?currpage=${pageModel.nextPage}&tCourseSiteId=${tCourseSite.id}&questionId=${questionId }&itemType=${itemType }'">下一题</button>
						</div>
    ...
    $("input[type='radio']").change(function(){
		if($(this).attr("checked")){
			...
			$.ajax({
				...
				'type':'POST',
				'data':{'answerId':answerId,'questionId':"${questionId}",'itemId':itemId,'tCourseSiteId':"${tCourseSite.id}",'type':"order"},
	...
	$(".keydown").keydown(function() {
        if (event.keyCode == "13") {//keyCode=13是回车键
            ...
            if($(this).val()!=""){
            	...
            	window.location.href="${pageContext.request.contextPath }/tcoursesite/exercise/findOrderItemListBySiteIdAndQuestionId?currpage="+currpage+"&tCourseSiteId="+"${tCourseSite.id}"+"&questionId="+"${questionId }"+"&&itemType="+"${itemType }";
