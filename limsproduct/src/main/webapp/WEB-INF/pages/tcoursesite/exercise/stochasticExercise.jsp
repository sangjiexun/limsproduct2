        ...
        <div class="course_cont r">
            ...
            <div class="course_mod f14 mb2">
                ...
                <div class=" lh40   pl30 f14 cf ">
                    <div class="bc l w20p poi" onclick="changeStochasticExerciseType(${tCourseSite.id},${questionId },4)" <c:if test="${itemType==4 }">style="color:#000; font-weight:bold"</c:if>>单选题</div>
                    <div class="bc l w20p poi" onclick="changeStochasticExerciseType(${tCourseSite.id},${questionId },1)" <c:if test="${itemType==1 }">style="color:#000; font-weight:bold"</c:if>>多选题</div>
                    <div class="bc l w20p poi" onclick="changeStochasticExerciseType(${tCourseSite.id},${questionId },8)" <c:if test="${itemType==8 }">style="color:#000; font-weight:bold"</c:if>>填空题</div>
                </div>
                <div class="  mt2">
                    <div class="pl30  ptb10 ">
                        ...
                        <div style="padding: 50px 150px">
							<button class="btn" onclick="window.location.href='${pageContext.request.contextPath }/tcoursesite/exercise/findStochasticItemListBySiteIdAndQuestionId?currpage=${pageModel.previousPage}&tCourseSiteId=${tCourseSite.id}&questionId=${questionId }&itemType=${itemType }'">上一题</button>
							<button class="btn" onclick="window.location.href='${pageContext.request.contextPath }/tcoursesite/exercise/findStochasticItemListBySiteIdAndQuestionId?currpage=${pageModel.nextPage}&tCourseSiteId=${tCourseSite.id}&questionId=${questionId }&itemType=${itemType }'">下一题</button>
						</div>
    ...
    $("input[type='radio']").change(function(){
		if($(this).attr("checked")){
			...
			$.ajax({
				...
				'type':'POST',
				'data':{'answerId':answerId,'questionId':"${questionId}",'itemId':itemId,'tCourseSiteId':"${tCourseSite.id}",'type':"stochastic"},
	...
	$(".keydown").keydown(function() {
        if (event.keyCode == "13") {//keyCode=13是回车键
            ...
            if($(this).val()!=""){
            	...
            	window.location.href="${pageContext.request.contextPath }/tcoursesite/exercise/findStochasticItemListBySiteIdAndQuestionId?currpage="+currpage+"&tCourseSiteId="+"${tCourseSite.id}"+"&questionId="+"${questionId }"+"&&itemType="+"${itemType }";
