 ...
 function trainFrom(){
	...
	var valueStr="";
   	    ...
   	    if(valueArray.length>0){
    	    //var url="${pageContext.request.contextPath}/labReservation/saveTrainResultRest/"+ ${labRoomId} + "/"+ ${type}+"/"+${deviceNumber} + "/" + "${deviceName}" +"/"+${username}+ "/"+${page}+ "/"+ idStr + "/" + valueStr+ "/"+"${labRoom_allowAppointment}";
            var url="${pageContext.request.contextPath}/labReservation/saveTrainResultRest/"+ ${labRoomId} + "/"+ 1+"/"+${deviceNumber} + "/" + "${deviceName}" +"/"+${username}+ "/"+${page}+ "/"+ idStr + "/" + valueStr+ "/"+"${labRoom_allowAppointment}";
...
<div class="right-content"><%--	
		...
		<div class="TabbedPanelsContent">
			...
			<div class="content-box">
				...
				<div class="tool-box">
					...
					<c:if test="${tag==1 }">
					<a class="btn btn-new" href="javascript:void(0);" onclick="closeMyWindow()";>返回</a></td>
					</c:if>
					<c:if test="${tag==2 }">
					<a class="btn btn-new" href="javascript:void(0);" onclick="closeMyWindow()";>返回</a></td>
					</c:if>
				...
				<input type="hidden" id="type" value="${type}">
      			...
      			<input type="hidden" id="labRoom_allowAppointment" value="${labRoom_allowAppointment}">
			</div>
		</div>
	...
	<script type="text/javascript">
		...
		function viewDeviceTrainingRest(id){
			...
			var page = $("#page").val();
			var labRoom_allowAppointment =$("#labRoom_allowAppointment").val(); 
			var url = $("#pageContext").val()+"/device/viewDeviceTrainingRest/" + labRoomId + "/"+ deviceNumber + "/" + deviceName +"/"+username+ "/"+page+"/" + id+"/"+labRoom_allowAppointment;
			//alert(url);
			window.location.href=url;
		}
	</script>
