...
<%--导师未审核时显示--%>
<c:if test="${state le 2 && isAudit == 0 && state gt 0}">
...
<%--导师审核后显示--%>
<c:if test="${state gt 2  || state le 1}">
   <table id="listTable" width="50%" cellpadding="0" cellspacing="0" class="tablesorter" >
...
function saveDeanAudit(){
	...
	window.location.href="${pageContext.request.contextPath}/labRoomLending/saveDeanAudit?id=${id}&tage=${tage}&state=${state }&page=${page}&auditResult="+auditResult+"&remark="+remark;
}
</script>
</body>
<!-------------列表结束----------->
