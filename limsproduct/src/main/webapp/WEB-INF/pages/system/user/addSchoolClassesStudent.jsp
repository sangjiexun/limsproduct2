...
<link href="${pageContext.request.contextPath}/css/iconFont.css" rel="stylesheet">
 ...
 <script type="text/javascript">
    ...
    function batchWithdraw(){
		...
		clearCookie();
		window.location.href="${pageContext.request.contextPath}/addSchoolClassesStudents?scrapAppIds="+scrapAppIds+"&classNumber="+"${classNumber}";
...
<div id="TabbedPanels1" class="TabbedPanels">
    		...
    		<div class="content-box">   
    		 <div class="title">
						<div id="title">所有学生</div>
							<a class="btn btn-new"   onclick="batchWithdraw();"><span>批量添加</span></a>
    ...
    <a href=" ${pageContext.request.contextPath}/addSchoolClassesStudent?currpage=1&classNumber=${classNumber}"    target ="_self"> 首页</a >
           <a href=" ${pageContext.request.contextPath}/addSchoolClassesStudent?currpage=${pageModel.previousPage}&classNumber=${classNumber}" target ="_self"> 上一页</a >
          第 <select onchange="javascript:window.location.href = this.options[this.selectedIndex].value;">
           <option value=" ${pageContext.request.contextPath}/addSchoolClassesStudent?currpage=${page}&classNumber=${classNumber}">${page} </option>
           <c:forEach begin= "${pageModel.firstPage}" end="${pageModel.lastPage}" step= "1" varStatus ="j" var="current">           
    <c:if test="${j.index!=page}">
    <option value=" ${pageContext.request.contextPath}/addSchoolClassesStudent?currpage=${j.index}&classNumber=${classNumber} ">${j.index} </option>
    </c:if >
    </c:forEach ></select> 页
           <a href=" ${pageContext.request.contextPath}/addSchoolClassesStudent?currpage=${pageModel. nextPage}&classNumber=${classNumber} "   target ="_self"> 下一页</a >
           <a href=" ${pageContext.request.contextPath}/addSchoolClassesStudent?currpage=${pageModel.totalPage}&classNumber=${classNumber}"   target ="_self"> 末页</a >
    </div >
