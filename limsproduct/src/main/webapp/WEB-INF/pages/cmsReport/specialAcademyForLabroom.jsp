...
<head>
    ...
    <script type="text/javascript">
        ...
        function cancelQuery(){
            if(${tag==0}){
                window.location.href="${pageContext.request.contextPath}/cmsReport/listLabRoomDeviceNew?roomId="+${roomId }+"&page=1";
            }
            if(${tag==1}){
                window.location.href="${pageContext.request.contextPath}/cmsReport/listLabRoomDevice?page=1";
            }
        }
...
<div >
    <ul class="TabbedPanelsTabGroup" style="margin-bottom:10px;">
        <li class="TabbedPanelsTab selected" id="s1"><a href="${pageContext.request.contextPath}/cmsReport/listLabRoomDeviceNew?roomId=${roomId }&page=1">设备列表</a></li>
        <li class="TabbedPanelsTab" id="s2"><a href="${pageContext.request.contextPath}/cmsReport/applyDeviceRepairList?page=1&roomId=${roomId }">保修登记</a></li>
        <li class="TabbedPanelsTab" id="s3"><a href="${pageContext.request.contextPath}/cmsReport/listLabRoomDeviceUsage?page=1&roomId=${roomId }">设备使用记录</a></li>
    </ul>
    <div class="TabbedPanelsTabGroup-box">
        <div class="TabbedPanelsContent">
            <div class="content-box">
                <div class="tool-box">
                    <form:form name="queryForm" action="${pageContext.request.contextPath}/cmsReport/listLabRoomDeviceNew?roomId=${roomId }&page=1" method="post" modelAttribute="labRoomDevice">
                        <table class="tab_fix">
                            <tr>
          ...
          <%--          <a class="btn btn-new" href="${pageContext.request.contextPath}/device/exportLabRoomDevice?roomId=${roomId }&page=1">导出</a>
                                ...
                                <div>
                                    ...
                                    <c:if test="${is_reservation eq 1}">
                                        <sec:authorize ifAnyGranted="ROLE_SUPERADMIN,ROLE_EXCENTERDIRECTOR,ROLE_LABMANAGER,ROLE_EQUIPMENTADMIN,ROLE_TEACHER,ROLE_ASSISTANT,ROLE_STUDENT">
                                            ...
                                            &lt;%&ndash;<c:if test="${labRoomDevice.CDictionaryByDeviceStatus.CNumber == 1 && labRoomDevice.CDictionaryByAllowLending.CNumber==1 && labRoomDevice.user.username != null }">
                                    ...
                                    <c:if test="${is_reservation eq 0}">
                                        &lt;%&ndash;<sec:authorize ifAnyGranted="ROLE_SUPERADMIN,ROLE_EXPERIMENTALTEACHING,ROLE_PREEXTEACHING,ROLE_EXCENTERDIRECTOR,ROLE_TEACHER">
                                        ...
                                        <sec:authorize ifNotGranted="ROLE_SUPERADMIN,ROLE_EXPERIMENTALTEACHING,ROLE_PREEXTEACHING,ROLE_EXCENTERDIRECTOR,ROLE_TEACHER">
                                            &lt;%&ndash;<c:if test="${labRoomDevice.user.username eq user.username}">
                                            <a class="btn btn-new" href="${pageContext.request.contextPath}/device/listInnerSameDevice?deviceId=${labRoomDevice.id}">关联设备</a>
                                            </c:if>&ndash;%&gt;
                                            &lt;%&ndash;<spring:message code="all.trainingRoom.labroom" />室管理员&ndash;%&gt;
                                            <c:if test="${labRoomDevice.user.username ne user.username}">
                                                <c:forEach items="${labRoomDevice.labRoom.labRoomAdmins}" var="admin" varStatus="i">
                                                    &lt;%&ndash;<c:if test="${admin.user.username eq user.username && admin.typeId eq 1}">
                                                    <a class="btn btn-new" href="${pageContext.request.contextPath}/device/listInnerSameDevice?deviceId=${labRoomDevice.id}">关联设备</a>
                                                    </c:if>&ndash;%&gt;
                                                </c:forEach>
                                            </c:if>
                                        ...
                                        &lt;%&ndash;<a class="btn btn-new" href='javascript:edit(${labRoomDevice.id})'>编辑</a>&ndash;%&gt;
                                </div>
                            </td>--%>
                        </tr>
                    </c:forEach>
                ...
                <div class="page" >
                    ${totalRecords}条记录,共${pageModel.totalPage}页
                    <a href="javascript:void(0)"    onclick="first('${pageContext.request.contextPath}/cmsReport/listLabRoomDeviceNew?roomId=${roomId }&page=1')" target="_self">首页</a>
                    <a href="javascript:void(0)" onclick="previous('${pageContext.request.contextPath}/cmsReport/listLabRoomDeviceNew?roomId=${roomId }&page=')" target="_self">上一页</a>
                    第<select onchange="first(this.options[this.selectedIndex].value)">
                    <option value="${pageContext.request.contextPath}/cmsReport/listLabRoomDeviceNew?roomId=${roomId }&page=${page}">${page}</option>
                    <c:forEach begin="${pageModel.firstPage}" end="${pageModel.lastPage}" step="1" varStatus="j" var="current">
                        <c:if test="${j.index!=page}">
                            <option value="${pageContext.request.contextPath}/cmsReport/listLabRoomDeviceNew?roomId=${roomId }&page=${j.index}">${j.index}</option>
                        </c:if>
                    </c:forEach></select>页
                    <a href="javascript:void(0)"     onclick="next('${pageContext.request.contextPath}/cmsReport/listLabRoomDeviceNew?roomId=${roomId }&page=')" target="_self">下一页</a>
                    <a href="javascript:void(0)"    onclick="last('${pageContext.request.contextPath}/cmsReport/listLabRoomDeviceNew?roomId=${roomId }&page=${pageModel.totalPage}')" target="_self">末页</a>
                </div>
            </div>
        </div>
    </div>
