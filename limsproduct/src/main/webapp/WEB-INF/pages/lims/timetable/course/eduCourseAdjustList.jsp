...
<head>
    ...
    <script src="${pageContext.request.contextPath}/bootstrap/js/plugins/bootstrap-table/locale/bootstrap-table-zh-CN.min.js"
            type="text/javascript"></script>
    <script src="${pageContext.request.contextPath}/js/lims/timetable/course/eduCourseAdjustList.js"
            type="text/javascript"></script>
        ...
        <ul class="TabbedPanelsTabGroup" style="margin-bottom:10px;">
            ...
            <li class="TabbedPanelsTab" id="s1">
                <a class="iStyle_Feelings_Tree_Leaf "
                   href="${pageContext.request.contextPath}/lims/timetable/course/eduCourseList"><spring:message code="all.timetable.edu"/></a>
            </li>
            <%--</sec:authorize>--%>
            <li class="TabbedPanelsTab" id="s2">
                <a class="iStyle_Feelings_Tree_Leaf "
                   href="${pageContext.request.contextPath}/lims/timetable/self/selfCourseList"><spring:message code="all.timetable.self"/></a>
            </li>
            <li class="TabbedPanelsTab selected" id="s3">
                <a class="iStyle_Feelings_Tree_Leaf "
                   href="${pageContext.request.contextPath}/lims/timetable/course/eduCourseAdjustList?type=PLAN">调停课管理</a>
            </li>
        </ul>
    </div>
    <div id="TabbedPanels1" class="TabbedPanels">
                ...
                <div>
                    <div id="toolbar">
                        <div class="form-inline">
                            ...
                            <div class="form-group">
                                      &ensp;&ensp;
                                &ensp;&ensp;&ensp;&ensp; <c:if test="${cookie.adjustType.value != 'SELF' && cookie.adjustType.value != 'ADJUST_HISTORY' && cookie.adjustType.value != 'SUSPEND_HISTORY'}">
                                        <input type="radio" name="view_radio"  onclick="getTimetableMangerView()" checked="checked"  value="course"><spring:message code="all.timetable.edu"/>
                                        <input type="radio" name="view_radio"  onclick="getSeflMangerView()"  value="self"><spring:message code="all.timetable.self"/>
                                        <input type="radio" name="view_radio"  onclick="getTimetableAdjustHistoryView()"  value="adjust_history">调课记录
                                <input type="radio" name="view_radio"  onclick="getTimetableSuspendHistoryView()"   value="suspend_history">停课记录

                                     </c:if>
                                     <c:if test="${cookie.adjustType.value == 'SELF'}">
                                        <input type="radio" name="view_radio"  onclick="getTimetableMangerView()"   value="course"><spring:message code="all.timetable.edu"/>
                                        <input type="radio" name="view_radio"  onclick="getSeflMangerView()"  checked="checked"  value="timetable"><spring:message code="all.timetable.self"/>
                                         <input type="radio" name="view_radio"  onclick="getTimetableAdjustHistoryView()"  value="adjust_history">调课记录
                                         <input type="radio" name="view_radio"  onclick="getTimetableSuspendHistoryView()"   value="suspend_history">停课记录
                                     </c:if>
                                <c:if test="${cookie.adjustType.value == 'ADJUST_HISTORY'}">
                                    <input type="radio" name="view_radio"  onclick="getTimetableMangerView()"   value="course"><spring:message code="all.timetable.edu"/>
                                    <input type="radio" name="view_radio"  onclick="getSeflMangerView()"  value="timetable"><spring:message code="all.timetable.self"/>
                                    <input type="radio" name="view_radio"  onclick="getTimetableAdjustHistoryView()"  checked="checked"  value="adjust_history">调课记录
                                    <input type="radio" name="view_radio"  onclick="getTimetableSuspendHistoryView()"   value="suspend_history">停课记录
                                </c:if>
                                <c:if test="${cookie.adjustType.value == 'SUSPEND_HISTORY'}">
                                    <input type="radio" name="view_radio"  onclick="getTimetableMangerView()"   value="course"><spring:message code="all.timetable.edu"/>
                                    <input type="radio" name="view_radio"  onclick="getSeflMangerView()"  value="timetable"><spring:message code="all.timetable.self"/>
                                    <input type="radio" name="view_radio"  onclick="getTimetableAdjustHistoryView()" value="adjust_history">调课记录
                                    <input type="radio" name="view_radio"  onclick="getTimetableSuspendHistoryView()"  checked="checked"  value="suspend_history">停课记录
                                </c:if>
                            </div>
                        </div>
                    </div>
