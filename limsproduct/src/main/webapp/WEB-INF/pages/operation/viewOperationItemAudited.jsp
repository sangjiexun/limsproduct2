...
<body>
  ...
  <div class="edit-content-box">
    <div class="title">
	  ...
	  <c:if test="${flag==3}">
	  <a class="btn btn-new" href="${pageContext.request.contextPath}/operation/viewOperationItem?operationItemId=582&&flag=3&status=2">返回</a>
	  </c:if>
    ...
    <table>
      ...
      <tr>
        <th>所属<spring:message code="all.trainingRoom.labroom"/>：</th><td>${operationItem.labRoom.labRoomName}</td>
      ...
      <c:forEach items="${opeItemAudits}" var="current"  varStatus="i">
      <tr>
      <c:if test="${current.status==0 }">
      <td>审核人：</td><td>${current.user.cname }</td>
