...
<c:when test="${statue==3}">
 ...
 <tr><td colspan="8"><font style="color: red">未审核</font></td></tr>
  ...
  <th>部门：</th>
  <th>联系方式：</th>
<c:forEach items="${labRoomCenterDirectorAudit}" var="curr"  varStatus="i">
					       <tr>
					       ...
					       <td style="vertical-align:middle; text-align:center;">${curr.user.schoolAcademy.academyName}</td>
					       <td style="vertical-align:middle; text-align:center;">${curr.user.telephone}</td>
					       </tr>
</c:forEach>
</table>
</c:when>
<c:when test="${statue==4}">
 <table id="listTable" width="50%" cellpadding="0" cellspacing="0" class="tablesorter" >
  <th>审核人:</th>
  <th>工号：</th>
  <th>部门：</th>
...
<c:otherwise>
<table id="listTable" width="50%" cellpadding="0" cellspacing="0" class="tablesorter" >
 <tr><td colspan="6"><font style="color: red">上一级未审核</font></td></tr>
</table>
</c:otherwise>
</c:choose>
</div>
