...
<script type="text/javascript">
  ...
  function exportSelect(){
	 document.form.action="${pageContext.request.contextPath}/exportSelectMyCurriculum";
	 document.form.submit();
}
//查询
function submitSelect(){
 document.form.action="${pageContext.request.contextPath}/selectMyCurriculum";
	 document.form.submit();
}
 </script>
</head>
    ...
    <tr>
        <td>
             ...
             <form:select class="chzn-select" id="week" path="userByTeacher.id" style="width:100px">
              ...
              <ul  class="new_bulid"> <li class="new_bulid_1"><c:choose><c:when test="${newFlag}"><a href="${pageContext.request.contextPath}/exportMyCurriculum"><fmt:message key="navigation.export"/></a></c:when><c:otherwise><a href="javascript:void(0)" onclick="exportSelect();" ><fmt:message key="navigation.export"/></a></c:otherwise></c:choose></li></ul> 
        </td> 
    </tr>
</table>
 <!-- 下拉框的js -->
        ...
        <c:forEach items="${schoolClasses}" var="current"  varStatus="i">
            ...
            <td>${current.className}</td>
                    ...
                    <td id="weekday_${current.id}_${cur.termCode}">
                       ...
                       ${arrange.courseDetail.teacherNeed.course.courseName}第${arrange.memo}周
