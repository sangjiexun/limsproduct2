...
<script type="text/javascript">
  ...
  {
      window.location.href = "${pageContext.request.contextPath}/basic_data/exportLabBasic";
  }
  </script>
<!-- 下拉框的js -->
...
<body bgcolor="#ffffff" text="#000000" onbeforeprint="btnprint.style.display='none';btnback.style.display='none';" onafterprint="btnprint.style.display='';btnback.style.display=''>
  <div class="navigation">
    <div id="navigation">
	  <ul>
          ...
          <li class="end"><a href="javascript:void(0)"><spring:message code="left.report.labbasic" /></a></li>
	  </ul>
	</div>
  </div>
  ...
  <div id="TabbedPanels1" class="TabbedPanels">
      <ul class="TabbedPanelsTabGroup">
          <li class="TabbedPanelsTab1 selected" id="s1">
              <a href="javascript:void(0)">基本情况列表</a>
          </li>
          <input class="btn btn-new" onclick="exportExcel();" type="button"  value="导出" />
          <input class="btn btn-new" TYPE="button" value="打 印" id="btnprint" onclick="self.print();">
      </ul>
  ...
  <div class="content-box">
	...
	<form:form name="form" method="Post" action="${pageContext.request.contextPath}/report/searchLabRoomBasic?currpage=1" modelAttribute="labRoom">
		<ul>
			<li>实验中心名称:<form:input id="lab_name" path="labCenter.centerName"/></li>
            <li><input type="submit" value="查询" onclick="targetUrl('${pageContext.request.contextPath}/report/searchLabRoomBasic?currpage=1')"/>
	...
	<table border= "1 " width= "200 " >
     ...
     <tr>
        ...
        <th   width= "3% " colspan="2">实验人数</th>
        <th   width= "3% " colspan="2">实验人时数</th>
     ...
     <tbody>
		  <c:forEach items="${labRoomBasics}" var="curr">
		    <tr>
			  ...
			  <td>${curr[2]}</td>
			  <td>${curr[3]}</td>
			  <td><fmt:formatDate value="${curr[4]}" pattern="yyyy.MM"/></td>
			  <td>${curr[5]}</td>
			  <td>${curr[6]}</td>
			  <td>${curr[7]}</td>
			  <td>${curr[8]}</td>
    ...
    <a onclick="targetUrl('${pageContext.request.contextPath}/report/searchLabRoomBasic?currpage=1')" target="_self">首页</a>
	<a onclick="targetUrl('${pageContext.request.contextPath}/report/searchLabRoomBasic?currpage=${pageModel.previousPage}')" target="_self">上一页</a>
	第<select onchange="targetUrl(this.options[this.selectedIndex].value)">
	<option value="${pageContext.request.contextPath}/report/searchLabRoomBasic?currpage=${pageModel.currpage}">${pageModel.currpage}</option>
	<c:forEach begin="${pageModel.firstPage}" end="${pageModel.lastPage}" step="1" varStatus="j" var="current">	
    <c:if test="${j.index!=pageModel.currpage}">
    <option value="${pageContext.request.contextPath}/report/searchLabRoomBasic?currpage=${j.index}">${j.index}</option>
    </c:if>
    </c:forEach></select>页
	<a onclick="targetUrl('${pageContext.request.contextPath}/report/searchLabRoomBasic?currpage=${pageModel.nextPage}')" target="_self">下一页</a>
 	<a onclick="targetUrl('${pageContext.request.contextPath}/report/searchLabRoomBasic?currpage=${pageModel.lastPage}')" target="_self">末页</a>
    </div>
    <!-- 分页结束 -->
