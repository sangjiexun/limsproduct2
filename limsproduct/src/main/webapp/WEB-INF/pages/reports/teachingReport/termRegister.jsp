    ...
    <div class="navigation">
        <div id="navigation">
            <ul>
                ...
                <li class="end"><a href="javascript:void(0)">学期登记</a></li>
            </ul>
        </div>
    </div>
    <div id="TabbedPanels1" class="TabbedPanels">
        <ul class="TabbedPanelsTabGroup">
            <li class="TabbedPanelsTab1 selected" id="s1">
                <a href="javascript:void(0)">学期登记</a>
            </li>
            <a href=#><input class="btn btn-new" type="button" onclick="exportExcel();" value="导出"></a>
    ...
    <table class="con-box" border="0" cellpadding="5" cellspacing="0" bgcolor="#F3EFEE" height="30" width="100%">
        ...
        <tr>
                <form name="form" action="" method="post" modelAttribute="schoolTerm">
                    ...
                    <td style="width: 210px;">实验室:
                    <select id="labRoom" name="labRoom" class="chzn-select" style="width: 20%">
                        <option value="">请选择实验室</option>
                        <c:forEach items="${labRooms}" var="t">
                            <option value="${t.id}">${t.labRoomName}</option>
                        </c:forEach>
                    </select>
                    </td>
                    <td>
                    <input class="btn" value="查询"
                           onclick="subform('${pageContext.request.contextPath}/report/teachingReport/termRegister');"
                           type="button">
                    <a href="${pageContext.request.contextPath}/report/teachingReport/termRegister"><input class="cancel-submit" type="button"
                                                                                                           value="取消查询"></a>
                    <%--<a href=#><input class="btn" type="button" onclick="exportExcel();" value="导出"></a>--%>
                    ...
                    <tr>
                        <c:if test="${cStatus.index==1}">
                            <c:if test="${rStatus.count==1}">
                                <td class="tbl tbct" rowspan=${labRoomMap.size()} style="width:60px"><span
                                        id="className12-13">第一节<br>~<br>第二节</span></td>
                            </c:if>
                        </c:if>
                        <c:if test="${cStatus.index==2}">
                            <c:if test="${rStatus.count==1}">
                                <td class="tbl tbct" rowspan=${labRoomMap.size()} style="width:60px"><span
                                        id="className12-13">第三节<br>~<br>第四节</span></td>
                            </c:if>
                        </c:if>
                        <c:if test="${cStatus.index==3}">
                            <c:if test="${rStatus.count==1}">
                                <td class="tbl tbct" rowspan=${labRoomMap.size()} style="width:60px"><span
                                        id="className12-13">第五节<br>~<br>第六节</span></td>
                            </c:if>
                        </c:if>
                        <c:if test="${cStatus.index==4}">
                            <c:if test="${rStatus.count==1}">
                                <td class="tbl tbct" rowspan=${labRoomMap.size()} style="width:60px"><span
                                        id="className12-13">第七节<br>~<br>第八节</span></td>
                            </c:if>
                        </c:if>
                        <c:if test="${cStatus.index==5}">
                            <c:if test="${rStatus.count==1}">
                                <td class="tbl tbct" rowspan=${labRoomMap.size()} style="width:60px"><span
                                        id="className12-13">第九节</span></td>
                            </c:if>
                        </c:if>
                        <c:if test="${cStatus.index==6}">
                            <c:if test="${rStatus.count==1}">
                                <td class="tbl tbct" rowspan=${labRoomMap.size()} style="width:60px"><span
                                        id="className12-13">第十节</span></td>
                            </c:if>
                        </c:if>
                        <c:if test="${cStatus.index==7}">
                            <c:if test="${rStatus.count==1}">
                                <td class="tbl tbct" rowspan=${labRoomMap.size()} style="width:60px"><span
                                        id="className12-13">第十一节</span></td>
                            </c:if>
                        </c:if>
                        <c:if test="${cStatus.index==8}">
                            <c:if test="${rStatus.count==1}">
                                <td class="tbl tbct" rowspan=${labRoomMap.size()} style="width:60px"><span
                                        id="className12-13">第十二节</span></td>
                            </c:if>
                        </c:if>
                        <c:if test="${cStatus.index==9}">
                            <c:if test="${rStatus.count==1}">
                                <td class="tbl tbct" rowspan=${labRoomMap.size()} style="width:60px"><span
                                        id="className12-13">第十三节</span></td>
                            </c:if>
                        </c:if>

                        <td class="tb" valign="top"> <%-- 实训室显示 --%>
                            <b>
                                <c:set var="labroomCount" value="1"></c:set>
                                <c:forTokens items="${labroom.value}" delims="-" var="tech">
                                    <c:if test="${labroomCount==1}">  <%-- 去掉if条件可以显示实训室名称 --%>
                                        ${tech}
                                    ...
                                    <c:if test="${labroom.key==ltimetable.labRoom.id}">
                                        <c:if test="${ltimetable.timetableAppointment.weekday==iWeek.count&&ltimetable.timetableAppointment.timetableAppointmentSameNumbers.size()==0}">
                                            <c:if test="${cStatus.index<=4}">
                                                <c:if test="${ltimetable.timetableAppointment.startClass<=cStatus.index*2-1&&ltimetable.timetableAppointment.endClass>=cStatus.index*2-1||ltimetable.timetableAppointment.startClass>=cStatus.index*2&&ltimetable.timetableAppointment.endClass<=cStatus.index*2}">
                                                    ...
                                                    <c:forEach var="tTimetable"
                                                               items="${ltimetable.timetableAppointment.timetableTeacherRelateds}"
                                                               varStatus="tStatus">
                                                    ...
                                                    <c:if test="${ltimetable.timetableAppointment.timetableAppointmentSameNumbers.size()==0}">
                                                        ...
                                                        <p> 班级：${ltimetable.timetableAppointment.schoolClasses.className}</p>
                                                        <br>
                                                    </c:if>
                                                    <br>
                                                </c:if>
                                        ...
                                        <c:if test="${ltimetable.timetableAppointment.weekday==iWeek.count&&ltimetable.timetableAppointment.timetableAppointmentSameNumbers.size()>0}">
                                            <c:if test="${cStatus.index<=4}">
                                                ...
                                                <c:forEach var="entry"
                                                           items="${ltimetable.timetableAppointment.timetableAppointmentSameNumbers}"
                                                           varStatus="p">
                                    ...
                                    <c:if test="${labroom.key==lSelftimetable.labRoom.id}">
                                        <c:if test="${lSelftimetable.timetableAppointment.weekday==iWeek.count&&lSelftimetable.timetableAppointment.timetableAppointmentSameNumbers.size()==0}">
                                            <c:if test="${cStatus.index<=4}">
                                                <c:if test="${lSelftimetable.timetableAppointment.startClass<=cStatus.index*2-1&&lSelftimetable.timetableAppointment.endClass>=cStatus.index*2-1||lSelftimetable.timetableAppointment.startClass>=cStatus.index*2&&lSelftimetable.timetableAppointment.endClass<=cStatus.index*2}">
                                                    ...
                                                    <c:forEach var="tTimetable"
                                                               items="${lSelftimetable.timetableAppointment.timetableTeacherRelateds}"
                                                               varStatus="tStatus">
                                                        <br>${tTimetable.user.cname}
                                                    </c:forEach> &nbsp;
                                                    <!-- 显示周次节次 -->
                                        ...
                                        <c:if test="${lSelftimetable.timetableAppointment.weekday==iWeek.count&&lSelftimetable.timetableAppointment.timetableAppointmentSameNumbers.size()>0}">
                                            <c:if test="${cStatus.index<=4}">
                                                <c:set var="sameStart" value="-"></c:set>
                                                <c:forEach var="entry"
                                                           items="${lSelftimetable.timetableAppointment.timetableAppointmentSameNumbers}"
                                                           varStatus="p">
                                                    <c:if test="${entry.startClass<=cStatus.index*2-1&&entry.endClass>=cStatus.index*2-1||entry.startClass>=cStatus.index*2&&entry.endClass<=cStatus.index*2}">
...
//		LODOP.SET_SAVE_MODE("QUICK_SAVE",true);//快速生成（无表格样式,数据量较大时或许用到） 
    ...
    function printPreview() {
        /*LODOP=getLodop();
        var strStyleCSS = "<link type='text/css' rel='stylesheet' href='
        ${pageContext.request.contextPath}/css/style.css'>";
		    strStyleCSS +="<link rel='stylesheet' type='text/css' href='
        ${pageContext.request.contextPath}/css/timetable/lmsReg.css'>";
