...
<head>
    ...
    <script type="text/javascript">
        ...
        //取消查询
        function cancel() {
            window.location.href="${pageContext.request.contextPath}/openOperationItem/listOpenOperationItem?currpage=1&status=${status}&orderBy=${orderBy }";
        }
        ...
        function orderByNumber(){
            asc=!asc;
            if(asc){
                window.location.href="${pageContext.request.contextPath}/openOperationItem/listOpenOperationItem?currpage=1&status=${status}&orderBy=10";
            }else window.location.href="${pageContext.request.contextPath}/openOperationItem/listOpenOperationItem?currpage=1&status=${status}&orderBy=0";
        }
        function orderByName(){
            asc=!asc;
            if(asc){
                window.location.href="${pageContext.request.contextPath}/openOperationItem/listOpenOperationItem?currpage=1&status=${status}&orderBy=11";
            }else window.location.href="${pageContext.request.contextPath}/openOperationItem/listOpenOperationItem?currpage=1&status=${status}&orderBy=1";
        }
        function orderByLab(){
            asc=!asc;
            if(asc){
                window.location.href="${pageContext.request.contextPath}/openOperationItem/listOpenOperationItem?currpage=1&status=${status}&orderBy=12";
            }else window.location.href="${pageContext.request.contextPath}/openOperationItem/listOpenOperationItem?currpage=1&status=${status}&orderBy=2";
        }
        function orderByCourse(){
            asc=!asc;
            if(asc){
                window.location.href="${pageContext.request.contextPath}/openOperationItem/listOpenOperationItem?currpage=1&status=${status}&orderBy=13";
            }else window.location.href="${pageContext.request.contextPath}/openOperationItem/listOpenOperationItem?currpage=1&status=${status}&orderBy=3";
        }
        function orderByStatus(){
            asc=!asc;
            if(asc){
                window.location.href="${pageContext.request.contextPath}/openOperationItem/listOpenOperationItem?currpage=1&status=${status}&orderBy=14";
            }else window.location.href="${pageContext.request.contextPath}/openOperationItem/listOpenOperationItem?currpage=1&status=${status}&orderBy=4";
        }
    </script>
    <script type="text/javascript">
        /*所有rest方法*/
    ...
    <script type="text/javascript">
        function newOperationItem() {
            window.location.href="${pageContext.request.contextPath}/openOperationItem/editOpenOperationItem?itemId=-1&page=${page}";
        }
        //跳转
        ...
        function submitItem(id) {
            $.ajax({
                url:'${pageContext.request.contextPath}/openOperationItem/submitOpenOperationItem?page=${page}',
                type:'POST',
                data:{id: id},
                success:function(){
                    window.location.href="${pageContext.request.contextPath}/openOperationItem/listOpenOperationItem?currpage=${page}&status=1&orderBy=9";
                }
            });
        }
        // 项目导入
        ...
        function listItemMaterialRecordRest(id) {
            var index = layer.open({
                ...
                maxmin: true,
                shadeClose: true,
                content: "${pageContext.request.contextPath}/openOperationItem/viewOpenOperationItem/${status}/" + id,
            });
            layer.full(index);
        }
    </script>
...
<div id="TabbedPanels1" class="TabbedPanels">
    <ul class="TabbedPanelsTabGroup">
        ...
        <li class="TabbedPanelsTab" id="s1"><a href="${pageContext.request.contextPath}/openOperationItem/listOpenOperationItem?currpage=1&status=1&orderBy=${orderBy}">我的项目</a></li>
        <%--<c:if test="${sessionScope.selected_role eq 'ROLE_EXCENTERDIRECTOR'}">--%>
        <li class="TabbedPanelsTab" id="s2"><a href="${pageContext.request.contextPath}/openOperationItem/listOpenOperationItem?currpage=1&status=2&orderBy=${orderBy }">我的审核</a></li>
        <%--</c:if>--%>
        </c:if>
        <li class="TabbedPanelsTab" id="s3"><a href="${pageContext.request.contextPath}/openOperationItem/listOpenOperationItem?currpage=1&status=3&orderBy=${orderBy}">全部项目</a></li>
        <c:if test="${sessionScope.selected_role ne 'ROLE_STUDENT'}">
    ...
    <div class="TabbedPanelsContentGroup">
        <div class="TabbedPanelsContent">
            <div class="content-box">
                <div class="tool-box">
                    <form:form name="queryForm" action="${pageContext.request.contextPath}/openOperationItem/listOpenOperationItem?currpage=1&status=${status}&orderBy=${orderBy }" method="post" modelAttribute="operationItem">
                        <ul>
                            <li>名称:
...
<%--                            </td>--%>
                            <td>
                                <c:if test="${fn:length(curr.itemPlans) > 0}">
                                    <table class="table_tab">
                                        ...
                                        <c:if test="${curr.itemPlans.iterator().next().type eq 2}">
                                            <tr>
                                                <td width="7%">星期</td>
                                                <td width="7%">节次</td>
                                                <td width="7%">周次</td>
                                        ...
                                        <c:forEach items="${curr.itemPlans}" var="plan">
                                            <c:forEach items="${plan.timetableSelfCourse.timetableAppointments}"
                                                       var="ta">
                                                <c:forEach items="${ta.timetableAppointmentSameNumbers}" var="tasn">
                                                    <c:if test="${ta.timetableStyle eq 4 or ta.timetableStyle eq 6}">
                                                        <c:if test="${fn:length(tasn.getTimetableAppointment().getTimetableGroups()) > 0}">
                                                            <c:forEach items="${ta.timetableGroups}" var="group"
                                                                       varStatus="g">
                                                                <c:if test="${g.first}">
                                                                    <tr>
                                                                        <td>
                                                                                ${group.timetableBatch.batchName}/${group.groupName}
                                                                        </td>
                                                                        <td>${ta.weekday}</td>
                                                                        <td>
                                                                        ...
                                                                        <td>
                                                                            ${tasn.startWeek}-${tasn.endWeek}
                                                                        </td>
                                                                        <td>
                                                                            <c:forEach items="${ta.timetableLabRelateds}" var="lab" varStatus="l">
                                                                                ${lab.labRoom.labRoomName}
                                                                                <c:if test="${!l.last}">,</c:if>
                                                                            </c:forEach>
                                                                        </td>
                                                                        <td>
                                                                            <c:forEach items="${ta.timetableTeacherRelateds}" var="teacher" varStatus="t">
                                                                                ${teacher.user.cname}
                                                                                <c:if test="${!t.last}">,</c:if>
                                                                            </c:forEach>
                                                                        </td>
                                                                        <td>
                                                                            <button id='group_button_reality_${tasn.getTimetableAppointment().getTimetableGroups().iterator().next().id}'
                                                                                    class='btn btn-xs green'
                                                                                    onclick="setTimetableGroupNumbersReality('${plan.timetableSelfCourse.id}','${tasn.getTimetableAppointment().getTimetableGroups().iterator().next().id}','${tasn.getTimetableAppointment().getTimetableGroups().iterator().next().numbers}','${tasn.getTimetableAppointment().getTimetableGroups().iterator().next().timetableGroupStudentses.size()}',8)"
                                                                                    title='编辑'><span
                                                                                    class='glyphicon glyphicon'>${tasn.getTimetableAppointment().getTimetableGroups().iterator().next().numbers}/${tasn.getTimetableAppointment().getTimetableGroups().iterator().next().timetableGroupStudentses.size()}</span>
                                                                            </button>
                                                                        </td>
                                                                    </tr>
                                                                    <tr id=" + group_div_reality + "
                                                                        style="display: none;"></tr>
                                                                </c:if>
                                                            </c:forEach>
                                                        </c:if>
                                                    </c:if>
                                                    <c:if test="${ta.timetableStyle ne 4 and ta.timetableStyle ne 6}">
                                                        <tr>
                                                            <td>${ta.weekday}</td>
                                                            <td>
                                                                    ${tasn.startClass}-${tasn.endClass}
                            ...
                            <td>
                                <c:if test="${(fn:contains('zjcclims',PROJECT_NAME) && (
                                    ...
                                    <c:if test="${draft.id==curr.CDictionaryByLpStatusCheck.id}">
                                        <c:if test="${curr.userByLpCreateUser.username==currUser.username || fn:contains(curr.labRoom.labRoomAdmins,currUser.username)}">
                                            ...
                                            <a href="${pageContext.request.contextPath}/openOperationItem/editOpenOperationItem?itemId=${curr.id}&page=${page}">编辑</a>
                                            <a href="${pageContext.request.contextPath}/openOperationItem/deleteOpenOperationItem?operationItemId=${curr.id}&status=${status}&page=${page}" onclick="return confirm('确定删除？');">删除</a>
                                        </c:if>
                                    </c:if>
                                ...
                                <c:if test="${curr.CDictionaryByLpStatusCheck.CNumber eq '3' and sessionScope.selected_role ne 'ROLE_STUDENT'}">
                                    <c:if test="${PROJECT_NAME ne 'ndyzlims' && (fn:length(curr.itemPlans) == 0 or curr.itemPlans.iterator().next().type eq 0)}">
                                ...
                                <c:if test="${curr.CDictionaryByLpStatusCheck.CNumber eq '3' and sessionScope.selected_role eq 'ROLE_STUDENT'}">
                                    <a onclick="openDiffType('学生选课', '${pageContext.request.contextPath}/openOperationItem/arrangeForStudent?id=${curr.id}')" href="javascript:void(0);">学生选课</a>
                                </c:if>
                            </td>
                        </tr>
                    </c:forEach>
                ...
                <div class="page" >
                    ...
                    <a href="javascript:void(0)" onclick="targetUrl('${pageContext.request.contextPath}/openOperationItem/listOpenOperationItem?currpage=1&status=${status}&orderBy=${orderBy }')" target="_self">首页</a>
                    <a href="javascript:void(0)" onclick="targetUrl('${pageContext.request.contextPath}/openOperationItem/listOpenOperationItem?currpage=${pageModel.previousPage}&status=${status}&orderBy=${orderBy }')" target="_self">上一页</a>
                    第<select onchange="javascript:window.location.href = this.options[this.selectedIndex].value;">
                    <option value="${pageContext.request.contextPath}/openOperationItem/listOpenOperationItem?currpage=${pageModel.currpage}&status=${status}&orderBy=${orderBy }">${pageModel.currpage}</option>
                    <c:forEach begin="${pageModel.firstPage}" end="${pageModel.lastPage}" step="1" varStatus="j" var="current">
                        <c:if test="${j.index!=pageModel.currpage}">
                            <option value="${pageContext.request.contextPath}/openOperationItem/listOpenOperationItem?currpage=${j.index}&status=${status}&orderBy=${orderBy }">${j.index}</option>
                        </c:if>
                    </c:forEach></select>页
                    <a href="javascript:void(0)"  onclick="targetUrl('${pageContext.request.contextPath}/openOperationItem/listOpenOperationItem?currpage=${pageModel.nextPage}&status=${status}&orderBy=${orderBy }')" target="_self">下一页</a>
                    <a href="javascript:void(0)"  onclick="targetUrl('${pageContext.request.contextPath}/openOperationItem/listOpenOperationItem?currpage=${pageModel.lastPage}&status=${status}&orderBy=${orderBy }')" target="_self">末页</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="login_frame" style="display:none">
        ...
        <div class="content" >
            <li><a class="btn btn-new" id="hide" type="button" style="display: none;"
                   href="${pageContext.request.contextPath}/openOperationItem/listOpenOperationItem?currpage=1&status=1&orderBy=9">
                导入完成，返回实验项目列表</a></li>
        </div>
    </div>
</div>
...
<!-- 下拉框的js -->
<script type="text/javascript">
    function openDiffType(title, url) {
        var index = layer.open({
            type: 2,
            title: title,
            maxmin: true,
