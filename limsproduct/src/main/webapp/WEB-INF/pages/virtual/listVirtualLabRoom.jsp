...
<script type="text/javascript">
  ...
  /*  function openSetupLink(labRoomId,currpage,type){//将labRoomId page传递到后台
    ...
    function submitUrl(){
        window.location.href="${pageContext.request.contextPath}/virtual/updateVirtualLabRoom";
    }

    function getLabRoomSoft(labNum){
        ...
        var index = layer.open({
            ...
            maxmin: true,
            shadeClose: true,
            content: "${pageContext.request.contextPath}/virtual/getLabRoomSoft?labNum="+labNum+"",
        });
        layer.full(index);
    }
</script>
<div class="navigation">
	<div id="navigation">
		<ul>
			...
			<li class="end"><a href="javascript:void(0)"><spring:message code="left.virtualtraining.management"/></a></li>
		</ul>
	</div>
</div>
				...
				<div class="content-box">
					...
					<table class="tb" id="my_show">
						...
						<tr>
							...
							<th><%--<a href="javascript:void(0);" onclick="">--%>虚拟实验室简介<%--</a>--%></th>
							<th><%--<a href="javascript:void(0);" onclick="">--%>操作<%--</a>--%></th>
						</tr>
						</thead>
						<tbody>
						<c:forEach items="${virtualLabRoomList}" var="curr">
							<tr>
								<td>${curr.labRoomNumber}</td>
								<td>${curr.labRoomName}</td>
								<td>${curr.labRoomIntroduction}</td>
                                <td><a href='javascript:getLabRoomSoft(${curr.labRoomNumber})' >查看虚拟镜像</a></td>
								<%--<td><input type="button" onclick="getLabRoomSoft(${curr.labRoomNumber})" value="查看虚拟镜像"></td>--%>
							</tr>
						</c:forEach>
						</tbody>
					</table>
					...
					<div class="page" >
						...
						<a href="javascript:void(0)" onclick="targetUrl('${pageContext.request.contextPath}/virtual/listVirtualLabRoom?currpage=1')" target="_self">首页</a>
						<a href="javascript:void(0)" onclick="targetUrl('${pageContext.request.contextPath}/virtual/listVirtualLabRoom?currpage=${pageModel.previousPage}')" target="_self">上一页</a>
						第<select onchange="javascript:window.location.href = this.options[this.selectedIndex].value;">
						<option value="${pageContext.request.contextPath}/virtual/listVirtualLabRoom?currpage=${pageModel.currpage}">${pageModel.currpage}</option>
						<c:forEach begin="${pageModel.firstPage}" end="${pageModel.lastPage}" step="1" varStatus="j" var="current">
							<c:if test="${j.index!=pageModel.currpage}">
								<option value="${pageContext.request.contextPath}/virtual/listVirtualLabRoom?currpage=${j.index}">${j.index}</option>
							</c:if>
						</c:forEach></select>页
						<a href="javascript:void(0)"  onclick="targetUrl('${pageContext.request.contextPath}/virtual/listVirtualLabRoom?currpage=${pageModel.nextPage}')" target="_self">下一页</a>
						<a href="javascript:void(0)"  onclick="targetUrl('${pageContext.request.contextPath}/virtual/listVirtualLabRoom?currpage=${pageModel.lastPage}')" target="_self">末页</a>
					</div>
					<!-- 分页[e] -->
                    <!-- 下拉框的js -->
