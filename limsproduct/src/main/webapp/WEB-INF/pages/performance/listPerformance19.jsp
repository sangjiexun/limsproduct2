...
<head>
    ...
    function edit(id) {
        var index = layer.open({
            type: 2,
            title: '全校任选课授课授课情况-编辑',
            maxmin: true,
            shadeClose: true,
            content: "${pageContext.request.contextPath}/performance/editPerformance?id="+id+"&type=19",
        });
        layer.full(index);
    }
    //取消查询
    function cancel()
    {
        window.location.href="${pageContext.request.contextPath}/performance/listPerformance?currpage=1&type=19";
    }
    function save() {
        $.ajax({
            url:'${pageContext.request.contextPath}/performance/savePerformance?type=19',
            type:'POST',
...
<div class="navigation">
    <div id="navigation">
        <ul>
            ...
            <li class="end"><a href="javascript:void(0);">全校任选课授课授课情况</a></li>
        </ul>
    </div>
</div>
    ...
    <div class="TabbedPanels">
        <ul class="TabbedPanelsTabGroup">
            <li class="TabbedPanelsTab1 selected" id="s1">
                <a href="javascript:void(0)">全校任选课授课授课情况</a>
            </li>
        ...
        <div class="TabbedPanelsContentGroup">
            <div class="TabbedPanelsContent">
                <div class="content-box">
                    <%--<div class="title">--%>
                        <%--<div id="title">全校任选课授课授课情况</div>--%>
                        <%--<div class="btn btn-new popup_show">新建</div>--%>
                    <%--</div>--%>
                    <div class="tool-box">
<form:form name="queryForm" action="${pageContext.request.contextPath}/performance/listPerformance?currpage=1&type=19" method="post" modelAttribute="individualPerformance">
                            <ul>
                                <li>学期:
                ...
                <div class="content-box">
                    <table>
                        ...
                        <tr>
                            ...
                            <a href="${pageContext.request.contextPath}/performance/delectPerformance?id=${current.id}&type=19" onclick="return confirm('确认要删除吗？')">删除</a>
                            </td>
                        </tr>
                        </c:forEach>
                        </tbody>
                    ...
                    <!-- 分页[s] -->
                            ...
                            onclick="targetUrl('${pageContext.request.contextPath}/performance/listPerformance?currpage=1&type=19')"
                            target="_self">首页</a>
                            <a href="javascript:void(0)"
                            onclick="targetUrl('${pageContext.request.contextPath}/performance/listPerformance?currpage=${pageModel.previousPage}&type=19')"
                            target="_self">上一页</a>
                            ...
                            <option value="${pageContext.request.contextPath}/performance/listPerformance?currpage=${currpage}&type=19">${currpage}</option>
                            <c:forEach begin="${pageModel.firstPage}" end="${pageModel.lastPage}" step="1"
                                       varStatus="j" var="current">
                                <c:if test="${j.index!=currpage}">
                                    <option value="${pageContext.request.contextPath}/performance/listPerformance?currpage=${j.index}&type=19">${j.index}</option>
                                </c:if>
                            </c:forEach>
                            </select>页&nbsp;

                            <a href="javascript:void(0)"
                            onclick="targetUrl('${pageContext.request.contextPath}/performance/listPerformance?currpage=${pageModel.nextPage}&type=19')"
                            target="_self">下一页</a>
                            <a href="javascript:void(0)"
                            onclick="targetUrl('${pageContext.request.contextPath}/performance/listPerformance?currpage=${pageModel.lastPage}&type=19')"
                            target="_self">末页</a>
                            </div>
                    <!-- 分页[e] -->
                </div>
...
<!--弹出框start-->
<div class="popup_box">
    <div class="popup_tit">编辑全校任选课授课授课情况<input class="popup_tit_icon popup_close" type="reset" value="x" title="关闭"/></div>
                            <form:form  id="queryForm"  method="post" modelAttribute="individualPerformance" >
        <table class="tab_lab">
            <tr>
                <th>课程名称</th>
                <td colspan="3">
                           <form:select  path="theme" class="chzn-select">
                                <form:options  items="${schoolCourseInfoList}" itemLabel="courseName" itemValue="courseName" />
                           </form:select>
                </td>
                <th>课程号</th>
                <td>
                                <form:select  path="themeAddition" class="chzn-select">
                                    <form:options  items="${schoolCourseInfoList}" itemLabel="courseNumber" itemValue="courseNumber" />
                                </form:select>
                </td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <th>学分</th>
                <td>
                    <form:input path="credit" type="number" placeholder="请输入"/>
                </td>
            </tr>
            <tr>
                <th>备注</th>
