    ...
    <script type="text/javascript">
        ...
        function edit(id) {
            var index = layer.open({
                type: 2,
                title: '大创项目结题及申报情况-编辑',
                maxmin: true,
                shadeClose: true,
                content: "${pageContext.request.contextPath}/performance/editPerformance?id="+id+"&type=4",
            });
            layer.full(index);
        }
        //取消查询
        function cancel()
        {
            window.location.href="${pageContext.request.contextPath}/performance/listPerformance?currpage=1&type=4";
        }
        function save() {
            $.ajax({
                url:'${pageContext.request.contextPath}/performance/savePerformance?type=4',
                type:'POST',
...
<div class="navigation">
    <div id="navigation">
        <ul>
            ...
            <li class="end"><a href="javascript:void(0);">大创项目结题及申报情况</a></li>
        </ul>
    </div>
</div>
    ...
    <div class="TabbedPanels">
        <ul class="TabbedPanelsTabGroup">
            <li class="TabbedPanelsTab1 selected" id="s1">
                <a href="javascript:void(0)">大创项目结题及申报情况</a>
            </li>
        ...
        <div class="TabbedPanelsContentGroup">
            <div class="TabbedPanelsContent">
                <div class="content-box">
                    <%--<div class="title">--%>
                        <%--<div id="title">大创项目结题及申报情况</div>--%>
                        <%--<div class="btn btn-new popup_show">新建</div>--%>
                    <%--</div>--%>
           <form:form name="queryForm" action="${pageContext.request.contextPath}/performance/listPerformance?currpage=1&type=4" method="post" modelAttribute="individualPerformance">
                    <div class="tool-box">
                            <ul>
                                <li>学年:
                ...
                <div class="content-box">
                  ...
                  <c:forEach items="${individualPerformanceList}" var="current"  varStatus="i">
                        <tr>
                            ...
                            <td><fmt:formatDate value="${current.startTime.time}" pattern="yyyy-MM-dd"/></td>
                            <td><fmt:formatDate value="${current.endTime.time}" pattern="yyyy-MM-dd"/></td>
                            <td>${current.aidAmount}</td>
                            <td>${current.otherTeacher}</td>
                            <td>${current.schoolAcademy}</td>
                            <td>${current.memo}</td>
                            <td>
                                <a href="javascript:edit(${current.id});">编辑</a>
                                <a href="${pageContext.request.contextPath}/performance/delectPerformance?id=${current.id}&type=4" onclick="return confirm('确认要删除吗？')">删除</a>
                            </td>
                        </tr>
                  </c:forEach>
                        </tbody>
                    ...
                    <div class="page" >
                        ...
                        <a href="javascript:void(0)"
                           onclick="targetUrl('${pageContext.request.contextPath}/performance/listPerformance?currpage=1&type=4')"
                           target="_self">首页</a>
                        <a href="javascript:void(0)"
                           onclick="targetUrl('${pageContext.request.contextPath}/performance/listPerformance?currpage=${pageModel.previousPage}&type=4')"
                           target="_self">上一页</a>
                        ...
                        <select onchange="javascript:window.location.href = this.options[this.selectedIndex].value;">
                            <option value="${pageContext.request.contextPath}/performance/listPerformance?currpage=${currpage}&type=4">${currpage}</option>
                            <c:forEach begin="${pageModel.firstPage}" end="${pageModel.lastPage}" step="1"
                                       varStatus="j" var="current">
                                <c:if test="${j.index!=currpage}">
                                    <option value="${pageContext.request.contextPath}/performance/listPerformance?currpage=${j.index}&type=4">${j.index}</option>
                                </c:if>
                            </c:forEach>
                        </select>页&nbsp;

                        <a href="javascript:void(0)"
                           onclick="targetUrl('${pageContext.request.contextPath}/performance/listPerformance?currpage=${pageModel.nextPage}&type=4')"
                           target="_self">下一页</a>
                        <a href="javascript:void(0)"
                           onclick="targetUrl('${pageContext.request.contextPath}/performance/listPerformance?currpage=${pageModel.lastPage}&type=4')"
                           target="_self">末页</a>
                    </div>
                    <!-- 分页[e] -->
                </div>
...
<!--弹出框start-->
<div class="popup_box">
    <div class="popup_tit">编辑大创项目结题及申报情况<input class="popup_tit_icon popup_close" type="reset" value="x" title="关闭"/></div>
            ...
            <tr>
                ...
                <td>
                    <form:select path="themeAddition" class="chzn-select">
                        <option value="">请选择</option>
                        <form:option value="国家级" selected="selected">国家级</form:option>
                        <form:option value="市级">市级</form:option>
                        <form:option value="校级">校级</form:option>
                    </form:select>
                </td>
            </tr>
            <tr>
                <th>开题时间</th>
                <td>
                    <form:input  path="startTime" type="text" class="easyui-datebox" />
                </td>
                <th>结题时间</th>
                <td>
                    <form:input path="endTime" type="text" class="easyui-datebox" />
                </td>
                <th>资助金额</th>
                <td>
