    ...
    <script type="text/javascript">
        ...
        function edit(id) {
            var index = layer.open({
                ...
                maxmin: true,
                shadeClose: true,
                content: "${pageContext.request.contextPath}/performance/editPerformance?id="+id+"&type=16",
            });
            layer.full(index);
        }
        //取消查询
        function cancel()
        {
            window.location.href="${pageContext.request.contextPath}/performance/listPerformance?currpage=1&type=16";
        }
        function save() {
            $.ajax({
                url:'${pageContext.request.contextPath}/performance/savePerformance?type=16',
                type:'POST',
...
<div class="navigation">
    <div id="navigation">
        <ul>
            ...
            <li class="end"><a href="javascript:void(0);">年度发表论文</a></li>
        </ul>
    </div>
</div>
    ...
    <div class="TabbedPanels">
        <ul class="TabbedPanelsTabGroup">
            <li class="TabbedPanelsTab1 selected" id="s1">
                <a href="javascript:void(0)">年度发表论文</a>
            </li>
        ...
        <div class="TabbedPanelsContentGroup">
            <div class="TabbedPanelsContent">
                <div class="content-box">
                    <%--<div class="title">--%>
                        <%--<div id="title">年度发表论文</div>--%>
                        <%--<div class="btn btn-new popup_show">新建</div>--%>
                    <%--</div>--%>
                    <div class="tool-box">
<form:form name="queryForm" action="${pageContext.request.contextPath}/performance/listPerformance?currpage=1&type=16" method="post" modelAttribute="individualPerformance">
                            <ul>
                                <li>学年:
                   ...
                   <c:forEach items="${individualPerformanceList}" var="current"  varStatus="i">
                        <tr>
                            ...
                            <td><c:forEach items="${current.commonDocuments}" var="d">
                                <c:if test="${d.type==2}">
                                    <a class="btn btn-common" href="${pageContext.request.contextPath}/visualization/downloadImageForLabRoom?id=${d.id}">${d.documentName}</a>
                                </c:if>
                            </c:forEach>
                            </td>
                            <td>${current.memo}</td>
                            <td>
                                ...
                                <a href="${pageContext.request.contextPath}/performance/delectPerformance?id=${current.id}&type=16" onclick="return confirm('确认要删除吗？')">删除</a>
                            </td>
                        </tr>
                   </c:forEach>
                        </tbody>
                    ...
                    <div class="page" >
                        ...
                        <a href="javascript:void(0)"
                           onclick="targetUrl('${pageContext.request.contextPath}/performance/listPerformance?currpage=1&type=16')"
                           target="_self">首页</a>
                        <a href="javascript:void(0)"
                           onclick="targetUrl('${pageContext.request.contextPath}/performance/listPerformance?currpage=${pageModel.previousPage}&type=16')"
                           target="_self">上一页</a>
                        ...
                        <select onchange="javascript:window.location.href = this.options[this.selectedIndex].value;">
                            <option value="${pageContext.request.contextPath}/performance/listPerformance?currpage=${currpage}&type=16">${currpage}</option>
                            <c:forEach begin="${pageModel.firstPage}" end="${pageModel.lastPage}" step="1"
                                       varStatus="j" var="current">
                                <c:if test="${j.index!=currpage}">
                                    <option value="${pageContext.request.contextPath}/performance/listPerformance?currpage=${j.index}&type=16">${j.index}</option>
                                </c:if>
                            </c:forEach>
                        </select>页&nbsp;

                        <a href="javascript:void(0)"
                           onclick="targetUrl('${pageContext.request.contextPath}/performance/listPerformance?currpage=${pageModel.nextPage}&type=16')"
                           target="_self">下一页</a>
                        <a href="javascript:void(0)"
                           onclick="targetUrl('${pageContext.request.contextPath}/performance/listPerformance?currpage=${pageModel.lastPage}&type=16')"
                           target="_self">末页</a>
                    </div>
                    <!-- 分页[e] -->
                </div>
...
<!--弹出框start-->
<div class="popup_box">
    <div class="popup_tit">编辑年度发表论文<input class="popup_tit_icon popup_close" type="reset" value="x" title="关闭"/></div>
