...
<script type="text/javascript">
	function submitQuery() {
		...
		if(str == null) {
			$("#auditTag").val(-1);
			document.form.action="${pageContext.request.contextPath}/timetable/timetableAdminIframe?currpage=1&id=-1&status="+${status };
			document.form.submit();
		}else {
			$("#auditTag").val(str);
			document.form.action="${pageContext.request.contextPath}/timetable/timetableAdminIframe?currpage=1&id=-1&status="+${status };
...
<!-- <div class="navigation">
    <div id="navigation">
	  <ul>
	    <li><a href="javascript:void(0)">实训排课</a></li>
		<li class="end"><a href="javascript:void(0)">课表执行</a></li>
	  </ul>
	</div>
  </div> -->
...
<ul>
   ...
   <li><c:if test="${auditTag eq 1}"><input type="radio" name="auditStr" value="1" checked="checked" /></c:if>
   		<c:if test="${auditTag ne 1}"><input type="radio" name="auditStr" value="1" /></c:if>已审核</li>
   <li><c:if test="${auditTag ne 1}"><input type="radio" name="auditStr" value="0" checked="checked" /></c:if>
   		<c:if test="${auditTag eq 1}"><input type="radio" name="auditStr" value="0" /></c:if>未审核</li>
   <input type="hidden" name="auditTag" id="auditTag" />
   <li><input type="button" onclick="submitQuery(); return false;" value="查询" id="search" />
       <a href="${pageContext.request.contextPath}/timetable/timetableAdminIframe?currpage=1&status=${status }&id=-1"><input class="cancel-submit" type="button" value="取消查询"></a></li>
   </ul>
    ...
    <!--合并相同批次的变量计数  -->
     ...
     <c:if test="${rowspan>1&&ifRowspan!=current.courseCode }">
          ...
          <c:if test="${current.status==2 }">
              <sec:authorize ifAnyGranted="ROLE_SUPERADMIN,ROLE_EXCENTERDIRECTOR,ROLE_DEPARTMENTHEADER">
                  ...
                  <a href="javascript:void(0)" onclick="timetableAppAudit('${current.courseCode}',${current.timetableStyle},${current.status})" >审核</a>
              </sec:authorize>
          </c:if>
     ...
     <c:if test="${rowspan==1 }">
         <td>
          <c:if test="${current.status==10 }">
           ...
           <c:if test="${current.timetableStyle==5}">
		   <a onclick="history.go(0)" href='${pageContext.request.contextPath}/timetable/selfTimetable/doSelfNOGroupTimetableOkFromAdmin?term=${term}&courseCode=${current.courseCode}'>课程计划完成成</a><br>
           </c:if>
           <c:if test="${current.timetableStyle==6&&current.timetableGroups.iterator().hasNext()}">
		   <a onclick="history.go(0)" href='${pageContext.request.contextPath}/timetable/selfTimetable/doSelfGroupTimetableOkFromAdmin?term=${term}&courseCode=${current.courseCode}&batchId=${current.timetableGroups.iterator().next().timetableBatch.id}'>课程计划完成</a><br>
           </c:if>
...
<div class="page" >
    ...
    <a href="javascript:void(0)" onclick="targetUrl('${pageContext.request.contextPath}/timetable/timetableAdminIframe?currpage=1&id=-1&status=${status}')" target="_self">首页</a>			    
	<a href="javascript:void(0)" onclick="targetUrl('${pageContext.request.contextPath}/timetable/timetableAdminIframe?currpage=${pageModel.previousPage}&id=-1&status=${status}')" target="_self">上一页</a>
	第<select onchange="javascript:window.location.href = this.options[this.selectedIndex].value;">
	<option value="${pageContext.request.contextPath}/timetable/timetableAdminIframe?currpage=${page}&id=-1&status=${status}">${page}</option>
	<c:forEach begin="${pageModel.firstPage}" end="${pageModel.lastPage}" step="1" varStatus="j" var="current">	
    <c:if test="${j.index!=page}">
    <option value="${pageContext.request.contextPath}/timetable/timetableAdminIframe?currpage=${j.index}&id=-1&status=${status}">${j.index}</option>
    </c:if>
    </c:forEach></select>页
	<a href="javascript:void(0)"  onclick="targetUrl('${pageContext.request.contextPath}/timetable/timetableAdminIframe?currpage=${pageModel.nextPage}&id=-1&status=${status}')" target="_self">下一页</a>
 	<a href="javascript:void(0)"  onclick="targetUrl('${pageContext.request.contextPath}/timetable/timetableAdminIframe?currpage=${pageModel.lastPage}&id=-1&status=${status}')" target="_self">末页</a>
    </div>
...
<!-- 审核 -->
<div id="timetableAppAudit" class="easyui-window" title="排课审核" closed="true" modal="true" iconCls="icon-add" style="width:400px;height:100px">
	<form name="auditForm" action="${pageContext.request.contextPath}/timetable/doReleaseTimetable?courseCode=${current.courseCode }&flag=${current.timetableStyle }&term=${term }&courseNumber=${courseNumber}" method="post" target="_parent" >
	   	<table>
	        <tr>
	           	<td>审核：</td>
	           	<td colspan="4">
	        ...
	        <tr>
	           	<td colspan="5"><input type="button" onclick="submitAudit(); return false;" value="提交"> </td>
	       	</tr>
	 	</table>
	</form>
</div>
...
<script type="text/javascript">
	...
	var appStyle = 0;
	var status = 0;
	function timetableAppAudit(courseNo, flag, stat) {
		courseCode = courseNo;
		appStyle = flag;
		status = stat;
	...
	function submitAudit() {
		document.auditForm.action="${pageContext.request.contextPath}/timetable/saveTimetableAudit?courseCode="+courseCode+"&appStyle="+appStyle+"&status="+status;
