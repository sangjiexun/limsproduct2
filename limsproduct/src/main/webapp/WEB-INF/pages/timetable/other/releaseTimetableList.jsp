...
<link href="${pageContext.request.contextPath}/css/room/muchPress.css" rel="stylesheet" type="text/css" />
 ...
 var status=${status};
  ...
  function exportSelect(){
	 document.form.action="${pageContext.request.contextPath}/exportSearchReleaseTimetable";
	 document.form.submit();
}
//查询
function submitSelect(){
 document.form.action="${pageContext.request.contextPath}/searchReleaseTimetableList";
	 document.form.submit();
}
 </script>
</head>
...
<div  class="l_right">
    <div class="list_top">
        <ul class="top_tittle"><fmt:message key="coursearrange.title"/></ul>  
        <ul  class="new_bulid">
                <li class="new_bulid_1"><c:choose><c:when test="${newFlag}"><a href="${pageContext.request.contextPath}/exportReleaseTimetable"><fmt:message key="navigation.export"/></a></c:when>
       <c:otherwise><a href="javascript:void(0)" onclick="exportSelect();"><fmt:message key="navigation.export"/></a></c:otherwise></c:choose></li></ul>
          <ul class="new_bulid">
                ...
                <tbody>
                	<c:forEach items="${courseDetails}" var="current"  varStatus="i">	
                        <tr>
                            ...
                            <td>${current.course.classesName} </td>
                      ...
                      <td><a  href="${pageContext.request.contextPath}/publishTimetable?id=<%-- ${current.teacherNeed.course.id} --%>"><fmt:message key="course.release.title"/></a></td>
                        </tr>
                        </c:forEach>
                </tbody>
            </table>
   ...
   <a href="${pageContext.request.contextPath}/releaseTimetableList?currpage=${pageModel.firstPage}" target="_self"><fmt:message key="firstpage.title"/></a>				    
	<a href="${pageContext.request.contextPath}/releaseTimetableList?currpage=${pageModel.previousPage}" target="_self"><fmt:message key="previouspage.title"/></a>
		第<select onchange="javascript:window.location.href = this.options[this.selectedIndex].value;"><option value="${pageContext.request.contextPath}/releaseTimetableList?currpage=${page}">${page}</option><c:forEach begin="${pageModel.firstPage}" end="${pageModel.lastPage}" step="1" varStatus="j" var="current">	
    <c:if test="${j.index!=page}"><option value="${pageContext.request.contextPath}/releaseTimetableList?currpage=${j.index}">${j.index}</option></c:if></c:forEach></select>页
	<a href="${pageContext.request.contextPath}/releaseTimetableList?currpage=${pageModel.nextPage}" target="_self"><fmt:message key="nextpage.title"/></a>
	<a href="${pageContext.request.contextPath}/releaseTimetableList?currpage=${pageModel.lastPage}" target="_self"><fmt:message key="lastpage.title"/></a>
    </div>
