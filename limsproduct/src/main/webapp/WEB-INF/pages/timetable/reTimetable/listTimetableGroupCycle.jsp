                ...
                <div class="TabbedPanelsContent">
                    <div class="content-box">
                        ...
                        <form name="form2" method="Post"
                              action="${pageContext.request.contextPath}/timetable/doGroupReTimetable">
                            <table>
                                <thead>
                                <tr>
                                    <!--  <th>选择</th> -->
                                    ...
                                    <c:if test="${current.id eq groupId }">
                                        ...
                                        <tr>
                                            ...
                                            <td colspan=8>
                                                <table>
                                                    <c:if test="${current.timetableAppointments.size()>0}">
                                                        ...
                                                        <c:forEach items="${current.timetableAppointments}"
                                                                   var="current2" varStatus="xh">
                                                            <tr>
                                                                ...
                                                                <td colspan=2>
                                                                    <table>
                                                                        ...
                                                                        <c:if test="${current2.timetableAppointmentSameNumbers.size()>0}">
                                                                            ...
                                                                            <c:forEach var="entry"
                                                                                       items="${current2.timetableAppointmentSameNumbers}"
                                                                                       varStatus="p">
                                                                                <c:set var="v_param"
                                                                                       value="-${entry.startClass}-"></c:set>
                                                                ...
                                                                <td width="15%">
                                                                    <c:if test="${current.timetableAppointments.size()>0}">
                                                                        <c:forEach
                                                                                items="${current2.timetableItemRelateds}"
                                                                                var="current3" varStatus="j">
                                                                ...
                                                                <td width="15%">
                                                                    ...
                                                                    <c:forEach items="${current2.timetableLabRelateds}"
                                                                               var="current3">
                                                                ...
                                                                <td width="10%">
                                                                    <c:forEach
                                                                            items="${current2.timetableTeacherRelateds}"
                                                                            var="current4">
                                                                        ${current4.user.cname}&nbsp;
                                                                    </c:forEach>
                                                                </td>
                                                                    ...
                                                                    <c:if test="${current2.id ==null}">
                                                                        <c:if test="${current.timetableBatch.flag!=1}">
                                                                            <a class="btn btn-common"
                                                                               href="${pageContext.request.contextPath}/timetable/doGroupReTimetable?group=${current.id}&term=${schoolTerm.id}&courseCode=${courseCode}&appointment=${current2.id}">新排课</a>
                                                                        </c:if>
                                                                    </c:if>
                                                                    <c:if test="${current2.id !=null}">
                                                                        <c:if test="${current2.status!=2&&current.timetableAppointment.status!=1&&current.timetableBatch.flag!=1 }">
                                                                            <a onclick="return confirm('删除后，排课状态还原，发布、分组信息，学生选课信息还原！确定删除此条记录？')"
                                                                               href='${pageContext.request.contextPath}/timetable/reTimetable/deleteReGroupAppointment?id=${current2.id }'>删除</a>&nbsp;&nbsp;
                                                                        </c:if>
                                                                ...
                                                                <c:if test="${xh.count ==1&&current2.status!=2&&current2.status!=1&&current.timetableBatch.flag!=1 }">
                                                                    <td width="15%"
                                                                        rowspan="${current.timetableAppointments.size()}">
                                                                        <a class="btn btn-common"
                                                                           href="${pageContext.request.contextPath}/timetable/doGroupReTimetable?group=${current.id}&term=${schoolTerm.id}&courseCode=${courseCode}&appointment=-1">新排课</a>
                                                                    </td>
                                                                </c:if>

                                                                <td width="10%" align="center"
                                                                    style="border-top-width: 0px; border-right: #ff0000 0px solid; border-left: #ff0000 0px solid; border-bottom: #ff0000 0px solid;">${current.groupName}</td>
                                                    ...
                                                    <c:if test="${current.timetableAppointments.size()==0}">
                                                        <tr>
                                                            <td width="55%" colspan=5><font color="red">未有排课记录</font>
                                                            </td>
                                            ...
                                            <td rowspan="${rowspan}">
                                                <c:if test="${current.timetableAppointment.status!=2&&current.timetableAppointment.status!=1&&current.timetableBatch.flag!=1 }">
                                                    ...
                                                    <c:if test="${ifArrange==1}">
                                                        <a class="btn btn-common"
                                                           href="${pageContext.request.contextPath}/timetable/doReGroupTimetableOk?term=${schoolTerm.id}&courseCode=${courseCodes.get(0).schoolCourse.courseCode}&batchId=${current.timetableBatch.id}"
                                                           target=_parent>调课完成</a>
                                                    </c:if>
                                            ...
                                            <c:if test="${rowspan>1&&ifRowspan!=current.timetableBatch.batchName }">
                                                ...
                                                <c:set var="ifRowspan"
                                                       value="${current.timetableBatch.batchName }"></c:set>
                                            </c:if>
