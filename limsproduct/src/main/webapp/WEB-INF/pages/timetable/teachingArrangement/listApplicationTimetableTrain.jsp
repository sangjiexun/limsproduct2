        ...
        function TimetableAppointment(courseCode, flag) {
            ...
            if (flag == 1) {
                layer.open({
                    ...
                    shadeClose: true,
                    area: ['1000px', '450px'],
                    content: '${pageContext.request.contextPath}/teachingArrangement/openListTimetableAp?currpage=${pageModel.nextPage-1}&flag=0&courseCode=' + courseCode
                    + '&tableAppId=' + 0 + ''
                })
            } else {
                layer.open({
                    ...
                    shadeClose: true,
                    area: ['1000px', '450px'],
                    content: '${pageContext.request.contextPath}/teachingArrangement/openListTimetableApNoSoftware?term=${term}&courseCode=' + courseCode + '&tableAppId=' + 0 + ''
                })
            }
        }
...
<div class="iStyle_RightInner">
	<div class="navigation">
		<div id="navigation">
			<ul>
				...
				<li class="end"><a href="javascript:void(0)">课内实践申请</a></li>
			</ul>
		</div>
	</div>
	<div id="TabbedPanels1" class="TabbedPanels">
		<ul class="TabbedPanelsTabGroup" style="margin-bottom:10px;">
			...
			<li class="TabbedPanelsTab selected" id="s3"><a class="iStyle_Feelings_Tree_Leaf "
				...
				<div class="TabbedPanelsContent">
					<div class="tool-box" style="overflow:initial">
						<form:form name="form" method="Post"
								   action="${pageContext.request.contextPath}/teachingArrangement/listPracticalTimetableTrain?currpage=1&id=-1&flag=0"
							...
							<ul>
								...
								<li>
									<select class="chzn-select" name="detailName" id="detailName"
										...
										<select class="chzn-select" name="teacher" id="teacher" cssStyle="width:500px">
											...
											<c:forEach items="${teachers}" var="current" varStatus="i">
												<c:if test="${current.username == teacher}">
													<option value="${current.username}"
															selected>${current.cname} </option>
												</c:if>
												<c:if test="${current.username != teacher}">
								...
								<li><input type="submit" value="查询">
									<a href="${pageContext.request.contextPath}/teachingArrangement/listPracticalTimetableTrain?currpage=1&id=-1&flag=0"><input
					...
					<div class="content-box">
							...
							<c:forEach items="${schedulingCourseMap}" var="current" varStatus="i">
								...
								<tr>
									...
									<td rowspan="${rowcodespan }">
										...
										<!--课程相关的教师以及主任具有进度安排的权限  -->
										<sec:authorize ifNotGranted="ROLE_EXCENTERDIRECTOR, ROLE_SUPERADMIN">
											<sec:authorize ifAnyGranted="ROLE_TEACHER">
												<c:if test="${current.user.username==user.username}">
													<a href='javascript:void(0)'
													   onclick='TimetableAppointment("${current.schoolCourse.courseCode}",1)'>有软件排课</a>&nbsp;&nbsp;
													<a href='javascript:void(0)'
													   onclick='TimetableAppointment("${current.schoolCourse.courseCode}",0)'>无软件排课</a>
												</c:if>
											</sec:authorize>
										</sec:authorize>
						...
						<a href="javascript:void(0)"
						   onclick="targetUrl('${pageContext.request.contextPath}/teachingArrangement/listPracticalTimetableTrain?currpage=${pageModel.firstPage}&term=${termId }&id=<%=request.getParameter("id") %>&flag=${flag}')"
						   target="_self">首页</a>
						<a href="javascript:void(0)"
						   onclick="targetUrl('${pageContext.request.contextPath}/teachingArrangement/listPracticalTimetableTrain?currpage=${pageModel.previousPage}&term=${termId }&id=<%=request.getParameter("id") %>&flag=${flag}')"
						   target="_self">上一页</a>
						第
						<select onchange="targetUrl(this.options[this.selectedIndex].value)">
							...
							<c:forEach begin="${pageModel.firstPage}" end="${pageModel.lastPage}" step="1" varStatus="j"
								...
								<c:if test="${j.index!=page}">
									<option value="${pageContext.request.contextPath}/teachingArrangement/listPracticalTimetableTrain?currpage=${j.index}&term=${termId }&id=<%=request.getParameter("id") %>&flag=${flag}">${j.index}</option>
								</c:if>
							</c:forEach>
						</select>页
						<a href="javascript:void(0)"
						   onclick="targetUrl('${pageContext.request.contextPath}/teachingArrangement/listPracticalTimetableTrain?currpage=${pageModel.nextPage}&term=${termId }&id=<%=request.getParameter("id") %>&flag=${flag}')"
						   target="_self">下一页</a>
						<a href="javascript:void(0)"
						   onclick="targetUrl('${pageContext.request.contextPath}/teachingArrangement/listPracticalTimetableTrain?currpage=${pageModel.lastPage}&term=${termId }&id=<%=request.getParameter("id") %>&flag=${flag}')"
						   target="_self">末页</a>
						...
						<!-- 课程进度 -->
						<div id="doApjust" class="easyui-window" title="课程进度" closed="true" modal="true"
							 iconCls="icon-add" style="width:1000px;height:600px"></div>
...
<!-- 直接排课 -->
<div id="doStart" class="easyui-window" title="直接排课" closed="true" modal="true" iconCls="icon-add"
	...
	<div class="navigation">
		<div id="navigation">
			<ul>
				...
				<li class="end">
					<a href="${pageContext.request.contextPath}/teachingArrangement/listAcademicTimetableTerm?id=-1">教务直接排课</a>
				</li>
			</ul>
		</div>
	</div>
							...
							<table border="0" align="center">
								...
								<c:if test="${empty cStaticValue ||cStaticValue.staticValue !='1'}">
									<tr>
										...
										<td>
											<select class="chzn-select" data-placeholder='请选择实验室：' multiple="multiple"
													name="labRoom_id" id="labRoom_id" items="${labRoomMap}"
									...
									<c:if test="${selected_labCenter==12 }">
										...
										<tr>
											...
											<td>
												<input type="radio" id="allowUse" name="group1"
													   onclick="showDevice()">是</input>
										...
										<tr id="labRoomDevices">
											...
											<td>
												<select class="chzn-select" data-placeholder='请选择实验室设备：'
														multiple="multiple" name="labRoomDevice_id"
														id="labRoomDevice_id" items="" invalidMessage="不能超过30个字符！"
								...
								<c:if test="${!empty cStaticValue &&cStaticValue.staticValue == '1'}">
									...
									<tr>
										<td align=left>请选择实验室：</td>
										<td>
											<select class="chzn-select" data-placeholder='请选择实验室：' name="labRoom_id"
									...
									<tr>
										...
										<td>
											<select class="chzn-select" multiple="multiple" data-placeholder='请选择实验设备：'
								...
								<tr>
									...
									<td>
										<select class="chzn-select" data-placeholder='请选择上课老师：' multiple="multiple"
								...
								<tr>
									...
									<td>
										<select class="chzn-select" data-placeholder='请选择指导老师：' multiple="multiple"
