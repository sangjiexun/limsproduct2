...
<head>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            switch (s) {
                case -1:
                    $("#ss0").addClass("TabbedPanelsTab selected");
                    break;
                case 10:
                    $("#ss1").addClass("TabbedPanelsTab selected");
                    break;
                case 1:
                    $("#ss2").addClass("TabbedPanelsTab selected");
                    break;
                default:
...
//使得弹出框在屏幕顶端可见
    ...
    <script type="text/javascript">
        function submitQuery() {
            ...
            if (str == null) {
                $("#auditTag").val(-1);
                document.form.action = "${pageContext.request.contextPath}/teachingArrangement/timetablevertifyRelease?currpage=1&id=-1&status=" +${status };
                document.form.submit();
            } else {
                $("#auditTag").val(str);
                document.form.action = "${pageContext.request.contextPath}/teachingArrangement/timetablevertifyRelease?currpage=1&id=-1&status=" +${status };
...
<div id="TabbedPanels1" class="TabbedPanels">
    <ul class="TabbedPanelsTabGroup" style="margin-bottom:10px;">
        ...
        <sec:authorize ifNotGranted="ROLE_STUDENT">
            <li class="TabbedPanelsTab selected" id="s5"><a class="iStyle_Feelings_Tree_Leaf "
...
<div class="iStyle_RightInner">
                ...
                <div class="TabbedPanelsContent">
                    <div class="tool-box" style="overflow:initial">
                        <form:form name="form" method="Post"
                                   action="${pageContext.request.contextPath}/teachingArrangement/timetablevertifyRelease?currpage=1&id=-1&status=${status }"
                                   modelAttribute="timetableAppointment">
                            <ul>
                                <li>
                                ...
                                <li>
                                    <form:select class="chzn-select" path="schoolCourseInfo.courseNumber"
                                                 id="schoolCourseInfo_courseNumber" cssStyle="width:650px">
                                        ...
                                        <c:forEach items="${schoolCoursess}" var="cur" varStatus="i">
                                            ...
                                            <c:if test="${cur.courseNumber ne  courseNumber}">
                                                <form:option value="${cur.courseNumber}"
                                                             label="课程名称：${cur.courseName}  "/>
                                            </c:if>
                                            <c:if test="${cur.courseNumber eq  courseNumber}">
                                                <form:option value="${cur.courseNumber}" selected="true"
                                                             label="课程名称：${cur.courseName} "/>
                                            </c:if>
                                        </c:forEach>
                                    </form:select>&nbsp;
                                ...
                                <li>
                                    <c:if test="${empty auditStr}"><input type="radio" name="auditStr" value=""
                                                                          checked="checked"/></c:if>
                                    <c:if test="${!empty auditStr}"><input type="radio" name="auditStr"
                                                                           value=""/></c:if>全部
                                </li>
                                <li><c:if test="${auditStr eq 1}"><input type="radio" name="auditStr" value="1"
                                                                         checked="checked"/></c:if>
                                    <c:if test="${auditStr ne 1}"><input type="radio" name="auditStr" value="1"/></c:if>已发布
                                </li>
                                <li><c:if test="${auditStr eq 2}"><input type="radio" name="auditStr" value="2"
                                                                         checked="checked"/></c:if>
                                    <c:if test="${auditStr ne 2}"><input type="radio" name="auditStr" value="2"/></c:if>待审核
                                </li>
                                <li><c:if test="${auditStr eq 3}"><input type="radio" name="auditStr" value="3"
                                                                         checked="checked"/></c:if>
                                    <c:if test="${auditStr ne 3}"><input type="radio" name="auditStr" value="3"/></c:if>审核中
                                </li>
                                <li><c:if test="${auditStr eq 4}"><input type="radio" name="auditStr" value="4"
                                                                         checked="checked"/></c:if>
                                    <c:if test="${auditStr ne 4}"><input type="radio" name="auditStr" value="4"/></c:if>已拒绝
                                </li>
                                <li><input type="button" onclick="submitQuery(); return false;" value="查询" id="search"/>
                                </li>
                                <li>
                                    <a href="${pageContext.request.contextPath}/teachingArrangement/timetablevertifyRelease?currpage=1&status=${status }&id=-1"><input
                                            type="button" value="取消查询"></a></li>
                            </ul>
                    ...
                    <div class="content-box">
                        ...
                        <table>
                            ...
                            <c:forEach items="${timetableAppointments}" var="current" varStatus="i">
                                        ...
                                        <td rowspan="${rowspan }">${current.schoolCourseInfo.courseName}
                                            ...
                                            <c:if test="${current.timetableStyle==4}">
                                                <a href='javascript:void(0)'
                                                   onclick='listTimetableStudents("${current.schoolCourseDetail.courseDetailNo }")'>学生名单</a>
                                                <a href='javascript:void(0)'
                                                   onclick='manageBatch("${current.schoolCourseDetail.courseDetailNo }")'><br><font
                                                        color="blue">分批管理</font></a>
                                            </c:if>
                                            <!--自主排课学生名单  -->
                                            <c:if test="${current.timetableStyle==6}">zzzzz
                                                <a href='javascript:void(0)'
                                                   onclick='listTimetableSelfStudents("${current.timetableSelfCourse.id }")'>学生名单</a>
                                                <a href='javascript:void(0)'
                                                   onclick='manageBatchSelf("${current.timetableSelfCourse.id }")'><br><font
                                                        color="blue">分批管理</font></a>
                                            </c:if>
                                        </td>
                                    </c:if>
                                ...
                                <c:if test="${rowspanBatch>0&&ifRowspanBatch!=current.timetableGroups.iterator().next().timetableBatch.id }">
                                    ...
                                    <c:set var="ifRowspanBatch"
                                           value="${current.timetableGroups.iterator().next().timetableBatch.id }"></c:set>
                                </c:if>
                                ...
                                <td colspan=2 width="90px;">
                                    <table>
                                        ...
                                        <c:if test="${current.timetableAppointmentSameNumbers.size()>0}">
                                            ...
                                            <c:forEach var="entry" items="${current.timetableAppointmentSameNumbers}"
                                                       varStatus="p">
                                ...
                                <td>
                                    ...
                                    <c:forEach var="entry" items="${current.timetableLabRelateds}">
                                        <label title="<c:out value="${entry.labRoom.labRoomName}" />"><c:out
                                                value="${entry.labRoom.labRoomName}"/></label>
                                        <a target="_blank"
                                           href="${pageContext.request.contextPath}/timetable/listTeachingLabRoomDevice?timetableId=${current.id}&labRoomId=${entry.labRoom.id}">查看教学用设备</a>
                                        <br>
                                    </c:forEach>
                                </td>
                                ...
                                <td>
                                    ...
                                    <c:if test="${current.timetableStyle!=4&&current.timetableStyle!=6&&current.timetableStyle!=5&&current.timetableStyle!=9}">
                                        <a href='javascript:void(0)'
                                           onclick='listTimetableStudents("${current.schoolCourseDetail.courseDetailNo }")'>
                                            <c:set value="0" var="counta"/>
                                            <c:forEach items="${current.schoolCourseDetail.schoolCourseStudents}"
                                    ...
                                    <c:if test="${current.timetableStyle==4||current.timetableStyle==9||current.timetableStyle==10}">
                                        <c:forEach items="${current.timetableGroups}" var="varGroup">
                                            ...
                                            <sec:authorize
                                                    ifAnyGranted="ROLE_EXCENTERDIRECTOR,ROLE_SUPERADMIN,ROLE_EXPERIMENTALTEACHING">
                                                <a href='javascript:void(0)'
                                                   onclick='listTimetableStudentsGroup("${varGroup.id}","${current.id}","${current.timetableStyle}")'>已选学生名单</a>
                                            </sec:authorize>
                                    ...
                                    <c:if test="${current.timetableStyle==6}">
                                        <c:forEach items="${current.timetableGroups}" var="varGroup">
                                            ...
                                            <sec:authorize
                                                    ifAnyGranted="ROLE_EXCENTERDIRECTOR,ROLE_SUPERADMIN,ROLE_EXPERIMENTALTEACHING">
                                                <a href='javascript:void(0)'
                                                   onclick='listTimetableStudentsGroup("${varGroup.id}","${current.id}",6)'>已选学生名单</a>
                                            </sec:authorize>
                                        </c:forEach>
                                    </c:if>
                                </td>
                                ...
                                <td id="timetableStyle${current.id}">
                                    ...
                                    <c:if test="${current.timetableStyle==4}">
                                        ...
                                        <c:forEach items="${current.timetableGroups}" var="varGroup">
                                            <sec:authorize
                                                    ifAnyGranted="ROLE_EXCENTERDIRECTOR,ROLE_SUPERADMIN,ROLE_EXPERIMENTALTEACHING">
                                                <a id="groupName4${current.id}" href='javascript:void(0)'
                                                   onclick='listTimetableGroup("${current.schoolCourseDetail.schoolTerm.id}","${current.weekday}","1","${current.courseCode}","-1","${varGroup.id}")'>${varGroup.timetableBatch.batchName}/${varGroup.groupName} </a><br>
                                            </sec:authorize>
                                            <sec:authorize
                                                    ifNotGranted="ROLE_EXCENTERDIRECTOR,ROLE_SUPERADMIN,ROLE_EXPERIMENTALTEACHING">
                                                <a id="groupName4${current.id}" href='javascript:void(0)'
                                                   onclick='listTimetableGroup("${current.schoolCourseDetail.schoolTerm.id}","${current.weekday}","1","${current.courseCode}","-1","${varGroup.id}")'>${varGroup.timetableBatch.batchName}/${varGroup.groupName} </a><br>
                                            </sec:authorize>
                                    ...
                                    <c:if test="${current.timetableStyle==6}">
                                        ...
                                        <c:forEach items="${current.timetableGroups}" var="varGroup">
                                            <sec:authorize
                                                    ifAnyGranted="ROLE_EXCENTERDIRECTOR,ROLE_SUPERADMIN,ROLE_EXPERIMENTALTEACHING">
                                                <a id="groupName6${current.id}" href='javascript:void(0)'
                                                   onclick='listTimetableSelfGroup("${current.timetableSelfCourse.schoolTerm.id}","${current.weekday}","1","${current.timetableSelfCourse.id}","-1")'>${varGroup.timetableBatch.batchName}/${varGroup.groupName}</a><br>
                                            </sec:authorize>
                                            <sec:authorize
                                                    ifNotGranted="ROLE_EXCENTERDIRECTOR,ROLE_SUPERADMIN,ROLE_EXPERIMENTALTEACHING">
                                                <a id="groupName6${current.id}"
                                                   href='javascript:void(0)'>${varGroup.timetableBatch.batchName}/${varGroup.groupName}</a><br>
                                            </sec:authorize>
                                            <c:if test="${varGroup.timetableBatch.ifselect==0}">
                                                自动选课<br>
                                            </c:if>
                                    ...
                                    <c:if test="${current.timetableStyle==9}">
                                        ...
                                        <c:forEach items="${current.timetableGroups}" var="varGroup">
                                            <sec:authorize
                                                    ifAnyGranted="ROLE_EXCENTERDIRECTOR,ROLE_SUPERADMIN,ROLE_EXPERIMENTALTEACHING">
                                                <a id="groupName9${current.id}" href='javascript:void(0)'
                                                   onclick='listTimetableGroupCycle("${current.schoolCourseDetail.schoolTerm.id}","${current.weekday}","1","${current.courseCode}","-1","${varGroup.id}")'>${varGroup.groupName} </a><br>
                                            </sec:authorize>
                                            <sec:authorize
                                                    ifNotGranted="ROLE_EXCENTERDIRECTOR,ROLE_SUPERADMIN,ROLE_EXPERIMENTALTEACHING">
                                                <a id="groupName9${current.id}" href='javascript:void(0)'
                                    ...
                                    <c:if test="${current.timetableStyle==10}">
                                        ...
                                        <c:forEach items="${current.timetableGroups}" var="varGroup">
                                            <sec:authorize
                                                    ifAnyGranted="ROLE_EXCENTERDIRECTOR,ROLE_SUPERADMIN,ROLE_EXPERIMENTALTEACHING">
                                                <a id="groupName4${current.id}" href='javascript:void(0)'
                                                   onclick='listTimetableGroupCycle("${current.schoolCourseDetail.schoolTerm.id}","${current.weekday}","1","${current.courseCode}","-1","${varGroup.id}")'>${varGroup.timetableBatch.batchName}/${varGroup.groupName} </a><br>
                                            </sec:authorize>
                                            <sec:authorize
                                                    ifNotGranted="ROLE_EXCENTERDIRECTOR,ROLE_SUPERADMIN,ROLE_EXPERIMENTALTEACHING">
                                                <a id="groupName4${current.id}" href='javascript:void(0)'
                                ...
                                <sec:authorize
                                    ...
                                    <c:if test="${current.schoolCourse.state!=0}">
                                        <td>
                                            <sec:authorize
                                                    ifAnyGranted="ROLE_EXCENTERDIRECTOR,ROLE_SUPERADMIN,ROLE_EXPERIMENTALTEACHING">
                                                <a onclick="sureDelete()"
                                                   href='${pageContext.request.contextPath}/teachingArrangement/deleteAppointment?id=${current.id }&term=${term}&currpage=${page}&courseNumber=${courseNumber}'>删除</a><br>
                                                <!--如果已发布 -->
                                                <c:if test="${current.status==10}">
                                                    <c:if test="${current.timetableStyle==2}">
                                                        <a href='javascript:void(0)'
                                                           onclick='doUpdateTimetable2("${current.schoolCourse.courseCode }",${current.id })'>编辑</a>
                                                    </c:if>
                                                    <c:if test="${current.timetableStyle==3}">
                                                        <a href='javascript:void(0)'
                                                           onclick='doUpdateTimetable3("${current.id }")'>编辑</a><br>
                                                    </c:if>
                                                    <c:if test="${current.timetableStyle==4}">
                                                        <a href='javascript:void(0)'
                                                           onclick='doUpdateTimetable4("${current.schoolCourse.courseCode }","${current.id }")'>编辑</a><br>
                                                    </c:if>
                                                    <c:if test="${current.timetableStyle==5}">
                                                        <a href='javascript:void(0)'
                                                           onclick='doUpdateTimetable5("${current.timetableSelfCourse.id }",${current.id })'>编辑</a>
                                                    </c:if>
                                                    <c:if test="${current.timetableStyle==6}">
                                                        <a href='javascript:void(0)'
                                                           onclick='doUpdateTimetable6("${current.timetableSelfCourse.id }","${current.id }")'>编辑</a><br>
                                                    </c:if>
                                                </c:if>
                                            </sec:authorize>
                                        </td>
                                ...
                                <c:if test="${rowspan>1&&ifRowspan!=current.courseCode }">
                                    <td rowspan="${rowspan }">
                                        <c:if test="${current.status==10 }">
                                            进行中<br>
                                            <c:if test="${current.timetableStyle==2}">
                                                <a onclick="changeStatus('${pageContext.request.contextPath}/timetable/doJudgeTimetableOkFromAdmin?courseCode=${current.courseCode}&currpage=${page}')">课程计划完成</a><br>
                                            </c:if>
                                            <c:if test="${current.timetableStyle==3}">
                                                <a onclick="changeStatus('${pageContext.request.contextPath}/timetable/doNoGroupTimetableOkFromAdmin?term=${term}&courseCode=${current.courseCode}')">课程计划完成</a><br>
                                            </c:if>
                                            <c:if test="${current.timetableStyle==4&&current.timetableGroups.iterator().hasNext()}">
                                                <a onclick="changeStatus('${pageContext.request.contextPath}/timetable/doReGroupTimetableOkFromAdmin?term=${term}&courseCode=${current.courseCode}&batchId=${current.timetableGroups.iterator().next().timetableBatch.id}')">课程计划完成</a><br>
                                            </c:if>
                                            <c:if test="${current.timetableStyle==5}">
                                                <a onclick="changeStatus('${pageContext.request.contextPath}/timetable/selfTimetable/doSelfNOGroupTimetableOkFromAdmin?term=${term}&courseCode=${current.courseCode}')">课程计划完成</a><br>
                                            </c:if>
                                            <c:if test="${current.timetableStyle==6&&current.timetableGroups.iterator().hasNext()}">
                                                <a onclick="changeStatus('${pageContext.request.contextPath}/timetable/selfTimetable/doSelfGroupTimetableOkFromAdmin?term=${term}&courseCode=${current.courseCode}&batchId=${current.timetableGroups.iterator().next().timetableBatch.id}')">课程计划完成</a><br>
                                            </c:if>
                                        </c:if>
                                        ...
                                        <c:if test="${current.status==2 }">
                                            <c:if test="${sessionScope.selected_role eq 'ROLE_SUPERADMIN' || sessionScope.selected_role eq 'ROLE_DEPARTMENTHEADER'}">
                                                ...
                                                <a href="javascript:void(0)"
                                                   onclick="timetableAppAudit('${current.courseCode}',${current.timetableStyle},${current.status},${current.timetableLabRelateds},1)">审核</a>
                                            </c:if>
                                        </c:if>
                                        <c:if test="${current.status==3 }">
                                            <c:if test="${sessionScope.selected_role eq 'ROLE_SUPERADMIN'}">
                                                <a href="javascript:void(0)"
                                                   onclick="timetableAppAudit('${current.courseCode}',${current.timetableStyle},${current.status},${current.timetableLabRelateds},2)">审核</a>
                                            </c:if>
                                        ...
                                        <c:if test="${sessionScope.selected_role eq 'ROLE_SUPERADMIN'}">
                                            <c:if test="${current.status!=1 && current.status!=4 && current.status!=10}"><!-- 未审核 -->
                                                <a href="javascript:void(0)"
                                                   onclick="timetableAppAudit('${current.courseCode}',${current.timetableStyle},${current.status},${current.timetableLabRelateds},2)">审核</a>
                                            </c:if>
                                        </c:if>
                                        ...
                                        <c:if test="${current.status==10 }">
                                            ...
                                            <c:if test="${current.timetableStyle==2}">
                                                <%--<a onclick="history.go(0)"
                                                   href='${pageContext.request.contextPath}/timetable/doJudgeTimetableOkFromAdmin?courseCode=${current.courseCode}&currpage=1'>--%>
                                                <a onclick="changeStatus('${pageContext.request.contextPath}/timetable/doJudgeTimetableOkFromAdmin?courseCode=${current.courseCode}&currpage=1')">课程计划完成</a><br>
                                            </c:if>
                                            <c:if test="${current.timetableStyle==3}">
                                                <a onclick="changeStatus('${pageContext.request.contextPath}/timetable/doNoGroupTimetableOkFromAdmin?term=${term}&courseCode=${current.courseCode}')">课程计划完成</a><br>
                                            </c:if>
                        ...
                        <div class="page">
                            ...
                            <a href="javascript:void(0)"
                               onclick="targetUrl('${pageContext.request.contextPath}/teachingArrangement/timetablevertifyRelease?currpage=1&id=-1&status=${status}')"
                               target="_self">首页</a>
                            <a href="javascript:void(0)"
                               onclick="targetUrl('${pageContext.request.contextPath}/teachingArrangement/timetablevertifyRelease?currpage=${pageModel.previousPage}&id=-1&status=${status}')"
                               target="_self">上一页</a>
                            第<select
                                onchange="targetUrl(this.options[this.selectedIndex].value)">
                            <option value="${pageContext.request.contextPath}/teachingArrangement/timetablevertifyRelease?currpage=${page}&id=-1&status=${status}">${page}</option>
                            <c:forEach begin="${pageModel.firstPage}" end="${pageModel.lastPage}" step="1" varStatus="j"
                                       var="current">
                                <c:if test="${j.index!=page}">
                                    <option value="${pageContext.request.contextPath}/teachingArrangement/timetablevertifyRelease?currpage=${j.index}&id=-1&status=${status}">${j.index}</option>
                                </c:if>
                            </c:forEach></select>页
                            <a href="javascript:void(0)"
                               onclick="targetUrl('${pageContext.request.contextPath}/teachingArrangement/timetablevertifyRelease?currpage=${pageModel.nextPage}&id=-1&status=${status}')"
                               target="_self">下一页</a>
                            <a href="javascript:void(0)"
                               onclick="targetUrl('${pageContext.request.contextPath}/teachingArrangement/timetablevertifyRelease?currpage=${pageModel.lastPage}&id=-1&status=${status}')"
                               target="_self">末页</a>
                        </div>
    ...
    <!-- 修改排课信息弹窗 -->
    <div id="doUpdateTimetable" class="easyui-window" title="修改排课信息" closed="true" modal="true" iconCls="icon-add"
         style="width:1050px;height:500px">
    </div>


    <!-- 查看学生名单 -->
    <div id="doSearchStudents" class="easyui-window" title="查看学生名单" closed="true" modal="true" iconCls="icon-add"
         style="width:1000px;height:500px">
    </div>

    <!-- 分批管理--针对二次排课分批 -->
    <div id="manageBatch" class="easyui-window" title="分批管理" closed="true" modal="true" iconCls="icon-add"
         style="width:1000px;height:500px">
    </div>

    <!-- 查看学生名单 -->
    <div id="doSearchGroup" class="easyui-window" title="查看学生分组" closed="true" modal="true" iconCls="icon-add"
         style="width:1000px;height:500px">
    </div>

    <!-- 审核 -->
    <div id="timetableAppAudit" class="easyui-window" title="排课审核" closed="true" modal="true" iconCls="icon-add"
         style="width:400px;height:120px">
        <form name="auditForm"
              action="${pageContext.request.contextPath}/timetable/doReleaseTimetable?courseCode=${current.courseCode }&flag=${current.timetableStyle }"
    ...
    <script type="text/javascript">
        ...
        function submitAudit() {
            document.auditForm.action = "${pageContext.request.contextPath}/teachingArrangement/saveTimetablevertifyRelease?courseCode=" + courseCode + "&appStyle=" + appStyle + "&status=" + status + "&labrelate=" + labrelate + "&tag=" + tag + "&page=${page}";
            document.auditForm.submit();
        }

        //课程计划完成
        function changeStatus(sUrl){
            $.ajax({
                    url:sUrl,
                    dataType:"json",
                    type:'GET',
