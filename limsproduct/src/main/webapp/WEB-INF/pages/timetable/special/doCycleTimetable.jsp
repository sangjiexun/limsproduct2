...
<div class=" cf ">
                    ...
                    <div class="course_scheduling">
                        <div>
                            <div class="tool_box brl5 cf bgg pa5">
	                            ...
	                            <div class="">
		                            <div class="cf mt10">
		                             <c:if test="${empty timetableGroupBycourseNo}">
		                                <div class="other_time r h25 lh25 b1 br5 plr10" onclick="addApp()">添加课程安排</div>
										 <a class="btn r" onclick="confirmButton()">确认</a>
									 </c:if>

										<c:if test="${!empty timetableGroupBycourseNo}">
											<a class="btn r" onclick="javascript:volid(0);" style="background-color: #4B4B4B">已确认</a>
										</c:if>
		                            </div>

		                            <div id="doSelfTimetable">
		                                <select  id="items" class="chosen-select"  tabindex="4" class="">
		                                	...
		                                	<select  id="labRooms" class="chosen-select"  tabindex="4" class="">
		                                ...
		                                <select  id="user" class="chosen-select" tabindex="4" class="">
		                                  ...
		                                  <select  id="tutor" class="chosen-select" tabindex="4" class="">
		                                    <option value="">请选指导员选择</option>
		                                   <%--  <option value ="${user.username}" selected> ${user.cname}[${user.username}]</option> --%>
		                           ...
		                           <table id="group" class="w100p">
			                                ...
			                                <tbody>
			                                	<c:forEach items="${timetableGroupBycourseNo}" var="currapp" varStatus="status">
					                                <tr>
														...
														<td><input type="button" value="查看" onclick="checkStu(${currapp.id})"></td>
					                                </tr>
			                                	</c:forEach>
				                                <tr>
				                                </tr>
			                            ...
			                            <form:form action="${pageContext.request.contextPath}/timetable/saveEditGroup?courseNo=${courseNo}&courseCode=${courseCode}&type=1" id="edit_form"  method="post" >
			                             ...
			                             <ul id="weeks">
			                             <c:forEach items="${schoolweek}" var="currapp" varStatus="status">
			                             <li class="l mlr10">
											 <input type="checkbox" id="${currapp}"/>
	...
	<!-- 查看学生名单 -->
	<div id="doSearchStudents" class="easyui-window" title="查看学生名单" modal="true"	closed="true" iconCls="icon-add" style="width:800px;height:500px;">
	</div>
...
<script type="text/javascript">
	...
	var weeks =selectedweek.join(",");
		$.ajax({
               ...
               type:'POST',
               data:{weeks:weeks, courseNo:${courseNo},courseCode:${courseCode},flag:1},
               error:function (request){
                 alert('请求错误!');
               },
               success:function(data){
               	if(data != '-1'){
               	location.href="${pageContext.request.contextPath}/timetable/listGeneralTimetable1?courseNo="+${courseNo}+"&isBatch=0";
               	}else{
    ...
    function confirmButton(){
        var count=0;
            		$.ajax({
                        ...
                        type:'POST',
                        data:{courseCode:${courseCode},courseNo:${courseNo},batchCount:1},
                        error:function (request){
                          alert('请求错误!');
                        },
                        success:function(data){
    ...
    function showTimetable(){
     window.location.href="${pageContext.request.contextPath}/timetable/listGeneralTimetable1?&courseNo="+${courseNo}+"&weeks="+weeks+"&isBatch=0";
    }
	...
	//查看分组学生名单
 	function checkStu(groupId){
		var con = '<iframe scrolling="yes" id="message1" frameborder="0"  src="${pageContext.request.contextPath}/timetable/viewGroupStudent?groupId='
			+ groupId+'&flag=2'
