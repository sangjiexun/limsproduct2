...
<c:if test="${state le 2 && isAudit == 0}">
 ...
 <c:forEach items="${deans}" var="teacher" varStatus="i">
 <tr align="center" >
     <td>审核人：</td>
     <td>${teacher.cname }</td> 
     <td>工号:</td>
...
<tr align="center" >
     <td>审核结果</td>
     <c:if test="${lStationReservationResult.result eq '1'}">
     	<td>通过</td> 
     </c:if>
     <c:if test="${lStationReservationResult.result eq '0'}">
...
function saveDeanAudit(){
	...
	window.location.href="${pageContext.request.contextPath}/timetableChangeAudit/saveDeanAudit?id=${id}&tage=${tage}&state=${state }&page=${page}&auditResult="+auditResult+"&remark="+remark;
}
</script>
</body>
<!-------------列表结束----------->
