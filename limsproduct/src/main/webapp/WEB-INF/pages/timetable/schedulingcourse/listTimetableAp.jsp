...
function licence(courseCode){
  /*var timeid=${timetableSelfCourse.id};
	...
	$($("#softwares option:selected")).each(function(){
		softwaresArray.push(this.value);
	});
		$.ajax({
	         url:"${pageContext.request.contextPath}/timetable/vocationalTimetable/isSoftWareConfilict",
	         dataType:"html",
	         type:'POST',
	         data:{term:${schoolCourseDetail.schoolTerm.id},classes:classesArray.join(","),weeks:$("#weeks").val(),weekday:$("#weekday").val(),softwares:softwaresArray.join(",")},
	...
	$.ajax({
        ...
        dataType:"html",
        type:'POST',
        data:{classes:classesArray.join(","),weeks:$("#weeks").val(),teachers:teachersArray.join(","),weekday:$("#weekday").val(),term:${schoolCourseDetail.schoolTerm.id},courseCode:courseCode},
        success:function(resultt)
        {
...
<tr>
	...
	<td>
     ...
     <c:out value="${cur.value}" />  
	     ...
	     <td>
		   <a href='${pageContext.request.contextPath}/timetable/selfTimetable/deleteTimeTableAppointment?id=${current.id }&courseCode=${current.courseCode}'>删除</a><%--&nbsp;&nbsp;
			<a  href='javascript:void(0)' onclick='editAp(${current.id},${courseCode})'>编辑</a>&nbsp;&nbsp;
	       --%></td>
</tr>
...
<form name="form" method="Post" action="${pageContext.request.contextPath}/timetable/saveTimetableAp?currpage=${currpage}">
</c:if>
...
<tr>
  ...
  <%--
   ...
   --%>
     ...
     <td>
   <select class="chzn-select"   data-placeholder='请选择' name="operationitems" id="operationitems" style="width:180px" required="true"  onchange="selectoperationitmes()">
  ...
  <select class="chzn-select" multiple="multiple" data-placeholder='请选择节次' name="classes" id="classes" style="width:150px" required="true">
   ...
   <td>
  <select class="chzn-select" multiple="multiple" data-placeholder='请选择授课教师' name="teachers" id="teachers" style="width:100px" required="true">
   ...
   <td>
  <select class="chzn-select" multiple="multiple" data-placeholder='请选软件' name="softwares" id="softwares" style="width:100px" required="true" onchange="licence('${courseCode}')">
