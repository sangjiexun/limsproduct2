...
<div class="navigation">
	<div id="navigation">
		<ul>
			...
			<li><a href="javascript:void(0)"><spring:message code="left.timetable.financial" /></a></li>
...
<div id="TabbedPanels1" class="TabbedPanels">
	<ul class="TabbedPanelsTabGroup" style="margin-bottom:10px;">
		...
		<li class="TabbedPanelsTab" id="s2"><a class="iStyle_Feelings_Tree_Leaf " href="${pageContext.request.contextPath}/timetable/listGeneralTimetable">课表查询</a></li>--%>
		<li class="TabbedPanelsTab selected" id="s3"><a class="iStyle_Feelings_Tree_Leaf " href="${pageContext.request.contextPath}/timetable/listTimetableTrain?currpage=1&id=-1&flag=0">课内实践申请</a></li>
		<li class="TabbedPanelsTab" id="s4"><a class="iStyle_Feelings_Tree_Leaf " href="${pageContext.request.contextPath}/timetable/listTimetableTrain?currpage=1&id=-1&flag=1">二次排课</a></li>
		<%--<sec:authorize ifNotGranted="ROLE_TEACHER">
		<li class="TabbedPanelsTab" id="s5"><a class="iStyle_Feelings_Tree_Leaf " href="${pageContext.request.contextPath}/timetable/timetablevertify?currpage=1&id=-1&status=-1">实践性教学排课审核及发布</a></li>
	</sec:authorize>--%>
	</ul>
</div>
...
<div class="tool-box" style="overflow:initial">
	<form:form name="form" method="Post" action="${pageContext.request.contextPath}/timetable/listTimetableTrain?currpage=1&id=-1&flag=0"	modelAttribute="schoolCourseDetail">
	<ul>
	   <li>学期：
	   </li><li>
       ...
       <c:forEach items="${schoolTerms}" var="current">
		...
		<li><a href="${pageContext.request.contextPath}/timetable/listTimetableTrain?currpage=1&id=-1"><input type="button" value="取消查询"></a></li>
...
<c:if test="${rowcodespan>1&&ifCodeRowspan!=current.schoolCourse.courseCode }">
	<td rowspan="${rowcodespan }">
		    ...
		    <sec:authorize ifAnyGranted="ROLE_TEACHER">
		    <c:if test="${current.user.username==user.username}">
			<a  href='javascript:void(0)' onclick='TimetableAppointment("${current.schoolCourse.courseCode}",1)'>有软件排课</a>&nbsp;&nbsp;
			<a  href='javascript:void(0)' onclick='TimetableAppointment("${current.schoolCourse.courseCode}",0)'>无软件排课</a>
			</c:if>
			</sec:authorize>
			</sec:authorize>
			<sec:authorize ifAnyGranted="ROLE_EXCENTERDIRECTOR, ROLE_SUPERADMIN">
