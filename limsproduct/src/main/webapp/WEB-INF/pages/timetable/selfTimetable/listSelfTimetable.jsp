...
function showTimetable(term,weekday,classids) {
    ...
    var con = '<iframe scrolling="yes" id="message" frameborder="0"  src="${pageContext.request.contextPath}/timetable/selfTimetable/doSelfTimetableIframe?term=' + term + '&weekday='+weekday+'&classids='+classids+'&courseCode=' + courseCode + '&labroom=' + labroom + '" style="width:100%;height:100%;"></iframe>'
...
function newCourseCode() {
    <%--var sessionId=$("#sessionId").val();--%>
    <%--var con = '<iframe scrolling="yes" id="message" frameborder="0"  src="${pageContext.request.contextPath}/timetable/selfTimetable/listCourseCodeIframeMain?currpage=1" style="width:100%;height:100%;"></iframe>'--%>
    <%--$("#newCourseCode").html(con);  --%>
    <%--//获取当前屏幕的绝对位置--%>
    <%--var topPos = window.pageYOffset;--%>
    <%--//使得弹出框在屏幕顶端可见--%>
    <%--$('#newCourseCode').window({left:"0px", top:topPos+"px"});--%>
    <%--$("#newCourseCode").window('open');--%>
    window.location.href="${pageContext.request.contextPath}/timetable/selfTimetable/listCourseCodes?currpage=1";
}
...
$(function(){
    ...
    $("#newCourseCode").window({
        top: 0 ,
        left: 0  
    })
    $(".newCourseCode").css('height',600);
})
...
var sessionId=$("#sessionId").val();
var con = '<iframe scrolling="yes" id="message" frameborder="0"  src="${pageContext.request.contextPath}/timetable/selfTimetable/doIframeNoGroupSelfTimetable?term=' + term +'&weekday='+weekday+'&classids='+classids+'&courseCode='+courseCode+'&labroom='+labroom+'" style="width:100%;height:100%;"></iframe>'
$('#showTimetable').html(con); 
//获取当前屏幕的绝对位置
var topPos = window.pageYOffset;
//使得弹出框在屏幕顶端可见
...
<ul>
    ...
    <li><a href="javascript:void(0)">自主排课（含分组)</a></li>
    <li class="end"><a href="javascript:void(0)">查看选课日历</a></li>
</ul>
</div>
</div>
<!--userPermission -->
...
<form action="${pageContext.request.contextPath}/timetable/selfTimetable/listSelfTimetable" method="post">
...
<tr>
    ...
    <a href="${pageContext.request.contextPath}/timetable/selfTimetable/listSelfTimetable"><input type="button" value="取消查询"></a>
    <input type="hidden" value=-1 id="courseCode" name="courseCode">
    </td>

    <td>
    <a class="btn btn-common" href="javascript:void(0)" onclick="newCourseCode()">返回开放项目管理</a>
    </td>    
</tr>
</tbody>
</table>
...
<c:forEach var="class" varStatus="cStatus" begin="1" end="9">
    ...
    <tr style="background:#FEFFDA;" >
        ...
        <c:if test="${cStatus.index==9}">
           <td class="tbl tbct"  style="width:60px"><span id="className12-13"><br>第十三节<br>&nbsp;</span></td>  
        </c:if>
        
        <c:forEach begin="1" end="7"  varStatus="iWeek">
        <%--<td class="tb" valign="top" >--%>
        <td class="tb" ondblclick="showTimetable('8','${iWeek.count}','${cStatus.index}')" valign="top" >
                <!-- 自主排课显示 -->
                <c:forEach var="labSelfTimetable" items="${labSelfTimetable}" varStatus="lStatus">
                    ...
                    <c:if test="${cStatus.index<=4}">
                        <c:if test="${labSelfTimetable.timetableAppointment.startClass<=cStatus.index*2-1&&labSelfTimetable.timetableAppointment.endClass>=cStatus.index*2-1||labSelfTimetable.timetableAppointment.startClass>=cStatus.index*2&&labSelfTimetable.timetableAppointment.endClass<=cStatus.index*2}">
                            ...
                            <a href='javascript:void(0)' onclick='listTimetableNoGroup("${labSelfTimetable.timetableAppointment.timetableSelfCourse.schoolTerm.id}","${labSelfTimetable.timetableAppointment.weekday}","1","${labSelfTimetable.timetableAppointment.timetableSelfCourse.id}","-1")'>
                            <input type="button" style="color:#CC3333" value="${labSelfTimetable.timetableAppointment.timetableSelfCourse.courseCode}-${labSelfTimetable.timetableAppointment.timetableSelfCourse.schoolCourseInfo.courseName}">
                            </a><br>
                            </c:if>
                            </c:if>
                    ...
                    <c:if test="${cStatus.index>4}">
                        <c:if test="${labSelfTimetable.timetableAppointment.startClass<=cStatus.index+4&&labSelfTimetable.timetableAppointment.endClass>=cStatus.index+4}">
                            ...
                            <!-- 分批处理 -->
                            <c:if test="${labSelfTimetable.timetableAppointment.timetableGroups.size()>0 }">
                            <a href='javascript:void(0)' onclick='listTimetableGroup("${labSelfTimetable.timetableAppointment.timetableSelfCourse.schoolTerm.id}","${labSelfTimetable.timetableAppointment.weekday}","1","${labSelfTimetable.timetableAppointment.timetableSelfCourse.id}","-1")'>
                            <input type="button" style="color:#FF0033" value="[${labSelfTimetable.labRoom.labRoomNumber}&nbsp;${labSelfTimetable.labRoom.labRoomName}]">
                            </a><br>
                            </c:if>
                             <!-- 不分批处理 -->
                            <c:if test="${labSelfTimetable.timetableAppointment.timetableGroups.size()==0 }">
                            <a href='javascript:void(0)' onclick='listTimetableNoGroup("${labSelfTimetable.timetableAppointment.timetableSelfCourse.schoolTerm.id}","${labSelfTimetable.timetableAppointment.weekday}","1","${labSelfTimetable.timetableAppointment.timetableSelfCourse.id}","-1")'>
                            <input type="button" style="color:#CC3333" value="[${labSelfTimetable.labRoom.labRoomNumber}&nbsp;${labSelfTimetable.labRoom.labRoomName}]">
                            </a><br>
                            </c:if>
                    ...
                    <c:if test="${cStatus.index<=4}">
                            ...
                            <c:if test="${labSelfTimetable.timetableAppointment.timetableGroups.size()>0 }">
                            <a href='javascript:void(0)' onclick='listTimetableGroup("${labSelfTimetable.timetableAppointment.timetableSelfCourse.schoolTerm.id}","${labSelfTimetable.timetableAppointment.weekday}","1","${labSelfTimetable.timetableAppointment.timetableSelfCourse.id}","-1")'>
                            <input type="button" style="color:#FF0033" value="${labSelfTimetable.timetableAppointment.timetableSelfCourse.courseCode}-${labSelfTimetable.timetableAppointment.timetableSelfCourse.schoolCourseInfo.courseName}">
                            </a><br>
                            </c:if>
                             <!-- 不分批处理 -->
                            <c:if test="${labSelfTimetable.timetableAppointment.timetableGroups.size()==0 }">
                ...
                <c:forEach var="ltimetable" items="${labReTimetable}" varStatus="lStatus">
                    ...
                    <c:if test="${cStatus.index<=4}">
                        <c:if test="${ltimetable.timetableAppointment.startClass<=cStatus.index*2-1&&ltimetable.timetableAppointment.endClass>=cStatus.index*2-1||ltimetable.timetableAppointment.startClass>=cStatus.index*2&&ltimetable.timetableAppointment.endClass<=cStatus.index*2}">
                            ...
                            <input type="button"  onclick="alert('请到二次排课模块处理二次排课业务！')" value="${ltimetable.timetableAppointment.schoolCourse.courseCode}-${ltimetable.timetableAppointment.schoolCourse.schoolCourseInfo.courseName}">
                            <br>
                            </c:if>
                            </c:if>
                    ...
                    <c:if test="${cStatus.index>4}">
                        <c:if test="${ltimetable.timetableAppointment.startClass<=cStatus.index+4&&ltimetable.timetableAppointment.endClass>=cStatus.index+4}">
                            ...
                            <!-- 分批处理 -->
                            <c:if test="${ltimetable.timetableAppointment.timetableGroups.size()>0 }">
                            <input type="button" onclick="alert('请到二次排课模块处理二次排课业务！')" style="color:#00A600" value="[${ltimetable.labRoom.labRoomNumber}&nbsp;${ltimetable.labRoom.labRoomName}]">
                            <br>
                            </c:if>
                             <!-- 不分批处理 -->
                            <c:if test="${ltimetable.timetableAppointment.timetableGroups.size()==0 }">
                    ...
                    <c:if test="${cStatus.index<=4}">
                            ...
                            <!-- 分批处理 -->
                            <c:if test="${ltimetable.timetableAppointment.timetableGroups.size()>0 }">
                            <input type="button" onclick="alert('请到二次排课模块处理二次排课业务！')" style="color:#00A600" value="${ltimetable.timetableAppointment.schoolCourse.courseCode}-${ltimetable.timetableAppointment.schoolCourse.schoolCourseInfo.courseName}">
                            <br>
                            </c:if>
                             <!-- 不分批处理 -->
                            <c:if test="${ltimetable.timetableAppointment.timetableGroups.size()==0 }">
...
<!--新建选课组 -->
<div id="newCourseCode" class="easyui-window" title="自建项目管理" modal="true" dosize="true" maximizable="true" collapsible="true" minimizable="false" closed="true" iconcls="icon-add" style="width: 1100px; height:630px;">
</div>
</sec:authorize>
