		...
		$.extend($.fn, {
			contextMenu : function(o) {
				...
				if (o.menuId == undefined)
					return false;// 如果没有menuId则退出
				if (o.fadeInSpeed == undefined)
					o.fadeInSpeed = defaults.fadeInSpeed;
				if (o.fadeOutSpeed == undefined)
					o.fadeOutSpeed = defaults.fadeOutSpeed;
				if (o.showShadow == undefined)
					o.showShadow = defaults.showShadow;
				...
				$(this).each(function() {
					...
					$triggerElement.unbind('contextmenu').bind('contextmenu',
							function(e) {
								var $currentTriggerElement = $(this);
								var contextMenu = $currentTriggerElement
										.data('contextMenu');
								// 检查菜单是否被屏蔽
								if ($currentTriggerElement.data('contextMenu').isEnabled === false)
									return false;
								...
								if (disabledMenuItemIdList.length > 0) {
									...
									queryStr = 'li:not('
											+ strDisabledMenuItemIdList + ')';
									$menu.find(strDisabledMenuItemIdList)
											.addClass('disabled');
								} else {
									queryStr = 'li';
								}
								$menu.find('li').find('a').unbind('click');
								$menu.find(queryStr).find('a').bind('click',
								...
								$(document).unbind('mousedown').bind(
			...
			enableContextMenuItems : function(o) {
				$(this).each(function() {
					...
					if (o == undefined) {
						...
						contextMenu.disabledMenuItemIdList = $.grep(
								contextMenu.disabledMenuItemIdList, function(
										value, index) {
