...
(function($) {
	...
	function _2c(_2d) {
		...
		$(".tabs-scroller-left", _2f).unbind(".tabs").bind("click.tabs",
				function() {
		...
		$(".tabs-scroller-right", _2f).unbind(".tabs").bind("click.tabs",
				function() {
					var _32 = $(".tabs-wrap", _2f);
					var pos = Math.min(_32.scrollLeft() + _2e.scrollIncrement,
							_1(_2d));
					_32.animate({
								scrollLeft : pos
							}, _2e.scrollDuration);
		...
		for (var i = 0, len = _33.length; i < len; i++) {
			...
			tab.unbind(".tabs").bind("click.tabs", {
						...
						_2e.onContextMenu.call(_2d, e, e.data.p
										.panel("options").title);
					});
	...
	function _37(_38, pp, _39) {
		...
		var _3d = $("<a href=\"javascript:void(0)\" class=\"tabs-inner\"></a>")
				.appendTo(tab);
		var _3e = $("<span class=\"tabs-title\"></span>").html(_3a.title)
				.appendTo(_3d);
