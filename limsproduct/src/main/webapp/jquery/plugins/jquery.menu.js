...
(function($) {
	...
	function _1c(_1d, pos) {
		...
		if (pos) {
			...
			if (_1e.left + $(_1d).outerWidth() > $(window).width()
					+ $(document).scrollLeft()) {
				_1e.left = $(window).width() + $(document).scrollLeft()
						- $(_1d).outerWidth() - 5;
			}
	...
	function _2f(_30, _31) {
		...
		if (_31.iconCls) {
			$("<div class=\"menu-icon\"></div>").addClass(_31.iconCls)
					.appendTo(_33);
		}
		if (_31.id) {
			_33.attr("id", _31.id);
	...
	$.fn.menu.methods = {
		...
		setIcon : function(jq, _3f) {
			return jq.each(function() {
						...
						if (_40.iconCls) {
							$(_40.target).children("div.menu-icon")
									.removeClass(_40.iconCls)
									.addClass(_3f.iconCls);
						} else {
							$("<div class=\"menu-icon\"></div>")
									.addClass(_3f.iconCls).appendTo(_3f.target);
						}
					});
		},
		getItem : function(jq, _41) {
			var _42 = {
				target : _41,
				id : $(_41).attr("id"),
				text : $.trim($(_41).children("div.menu-text").html()),
				disabled : $(_41).hasClass("menu-item-disabled"),
				href : $(_41).attr("href"),
				onclick : _41.onclick
			};
