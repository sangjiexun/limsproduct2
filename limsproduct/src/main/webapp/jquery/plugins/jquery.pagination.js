...
(function($) {
	function _1(_2) {
		...
		if (_3.showPageList) {
			...
			for (var i = 0; i < _3.pageList.length; i++) {
				var _5 = $("<option></option>").text(_3.pageList[i])
						.appendTo(ps);
			...
			for (var i = 0; i < _3.buttons.length; i++) {
				...
				if (_6 == "-") {
					...
					$("<a href=\"javascript:void(0)\"></a>").addClass("l-btn")
							.css("float", "left").text(_6.text || "").attr(
									"icon", _6.iconCls || "").bind("click",
									eval(_6.handler || function() {
	...
	function _11(_12) {
		...
		num.parent().next().find("span").html(_13.afterPageText.replace(
				/{pages}/, _14));
		...
		if (_13.loading) {
			$(_12).find("a[icon=pagination-load]").find(".pagination-load")
	...
	function _16(_17, _18) {
		...
		_19.loading = _18;
		if (_19.loading) {
			$(_17).find("a[icon=pagination-load]").find(".pagination-load")
					.addClass("pagination-loading");
		} else {
			$(_17).find("a[icon=pagination-load]").find(".pagination-load")
					.removeClass("pagination-loading");
