...
$(document).ready(function () {
    ...
    url = zuulUrl + "api/timetable/manage/apiSelfTimetableBatchList";
    urlChild = zuulUrl + "api/timetable/manage/apiTimetableGroupList";
    type = $("#type").val();
    var t = "<button class='btn btn-xs green' title='添加'  onclick=\"showdiv()\" ><span class='glyphicon glyphicon-plus'>添加批次</span></button>";
    if(type == 0){
        t = "";
    }
    //初始化表格,动态从服务器加载数据
        ...
        oTableInit.Init = function () {
            $("#table_list").bootstrapTable({
                ...
                columns: [{
                    ...
                    title: "<div style=\"height:20px;\">操作&nbsp;" + t,
                    width: "25%",
                    field: "empty",
                    formatter: function (value, row, index) {
                        var result = "";
                        if (row.baseActionAuthDTO.addActionAuth && type != 0) {
    ...
    InitSubTable = function (index, row, $detail) {
        ...
        $(cur_table).bootstrapTable({
            ...
            columns: [{
                ...
                title: "<div style=\"height:20px;\">操作&nbsp;" + (type != 0 ? "<button class='btn btn-xs green' title='添加'  onclick=\"saveTimetableGroup(" + batchId + ")\" ><span class='glyphicon glyphicon-plus'>添加分组</span></button>" : ""),
                width: "20%",
                field: "empty",
                formatter: function (value, row, index) {
                    var result = "<div style=\"height:20px;\">";
                    if (row.baseActionAuthDTO.deleteActionAuth && type != 0) {
...
function deleteTimetableBatch(batchId) {
    if (confirm('是否确认删除？')) {
        $.ajax({
            url: zuulUrl + "api/timetable/manage/apiDeleteTimetableBatch?batchId=" + batchId,
            headers:{Authorization: getJWTAuthority()},
            async: false,
            contentType: "application/json;charset=utf-8",
            dataType: "json",
...
function deleteTimetableGroup(groupId,batchId) {
    if (confirm('是否确认删除？')) {
        $.ajax({
            url: zuulUrl + "api/timetable/manage/apiDeleteTimetableGroup?groupId=" + groupId,
            contentType: "application/json;charset=utf-8",
            dataType: "json",
            type: "post",
            headers:{Authorization: getJWTAuthority()},
...
function doSelfReGroupCourse(groupId, batchId,sameNumberId) {
    ...
    var index = layer.open({
        ...
        shadeClose: true,
        area: ['1100px', '500px'],
        content: contextPath + '/openOperationItem/newSelfReGroupTimetableCourse?currpage=1&flag=0&timetableStyle=6&selfId=' + selfId + "&term=" + term + "&groupId=" + groupId
        + '&tableAppId=' + 0+ '&sameNumberId=' + sameNumberId+'&opId='+opId,
        end: function () {
            refreshBootstrapChildTable(batchId);
        }
    });
