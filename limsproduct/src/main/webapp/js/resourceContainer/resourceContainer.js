    ...
    getDirectoryChildren({
        ...
        success:function (data) {
            ...
            $.each(data.directorys,function (index,directory) {
                mkdir($(obj),directory.directoryName,directory.directoryId,directory.subDirectoryNumbers);
            });
        }
    });
...
function mkdir(obj, dirName, dirID,subDirectoryNum,isGot) {
    var fileNum;
    $.ajax({
        url: resourceContainerHost + "/getFilesnumByDirectoryIDList?DirectoryIDList=" + dirID,
        dataType: "json",
        type: "GET",
        async: false,
        success: function(data) {
...
function openUploadWindowByPath( pathTitle,versionFlag,e) {
    e = e|| window.event;
...
function checkSuffix(filename) {
    ...
    var videoregx = /(mp4|avi|wmv|rmvb|mpg)$/;
    var pdfregx = /(pdf)$/;
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (afterUpLoad != null) {
                ...
                if(returnedFileIDArray.length = multifilelist.size){
                    alert("上传成功！");
                    location.reload(true);
                }
            }
