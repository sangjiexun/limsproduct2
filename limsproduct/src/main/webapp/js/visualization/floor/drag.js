...
var Common = {
		...
		/* setInterval("w()", 100);
		function w() {
			var dragDivwidth=$(".panorama").width();
		}
		setInterval("var s=$('.panorama').width()", 100);
		console.log(s) */
        Drag.prototype = {
            init: function(titleBar, dragDiv, Options) {
                ...
                this.dragArea = { maxLeft: -($("#viewer_img").width()), maxRight:0, maxTop: 0, maxBottom: Common.getViewportSize.h - dragDiv.offsetHeight - 2 };
                	// Common.getViewportSize.w - dragDiv.offsetWidth - 2
    			console.log($("#viewer_img").width())
                ...
                if (Options) {
                    ...
                    if (this.keepOrigin) { this.opacity = 100; }
                    if (Options.area) {
