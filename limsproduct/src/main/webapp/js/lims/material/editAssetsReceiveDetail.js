...
layui.use(['laypage', 'layer', 'table', 'element','form','laydate','upload'], function() {
    ...
    table.on('tool(assetsList)', function(obj) { //注：tool 是工具条事件名，parenthead 是 table 原始容器的属性 lay-filter="对应的值"
        ...
        //打开编辑物资名录页面
        if(obj.event === 'edit') {
            var realURL=contextPath + '/lims/api/material/addAssetsReceiveItemAPI?id='+data.id+'&&receiveId='+id;;
            var index = layer.open({
                type: 2 //此处以iframe举例
                ,
                title: '编辑物资名录',
    ...
    //添加物资名录
    var active = {
        addAssetsReceiveItem: function() {
            var realURL=contextPath + '/lims/api/material/addAssetsReceiveItemAPI?receiveId='+id;
            //多窗口模式，层叠置顶
            var index = layer.open({
                type: 2 //此处以iframe举例
                ,
    ...
    form.on('submit(saveAssetsReceiveDetail)', function(data){
        console.log('保存')
        ...
        $.ajax({
            url: contextPath + '/lims/api/material/saveAssetsReceiveDetail?imageUrl='+imageUrl+'&&names='+names+'&&sizes='+sizes,
            data: data,
            async: false,
            type: "POST",
            contentType: "application/json;charset=UTF-8",
            success:function (res) {
                console.log(res);
                window.location.href="../material/editAssetsReceiveDetailAPI?id="+res;
                if(id!==''){
    ...
    form.on('select(goodsCategory)', function(data){
        ...
        var id=data.value
        $.ajax({//判断该物资类别是否需要归还
            url: contextPath + '/lims/api/material/checkAssetsClassificationIsNeedReturn?id='+id,
            async: false,
            type: "POST",
            contentType: "application/json;charset=UTF-8",
            success:function (res) {
