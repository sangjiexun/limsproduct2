...
layui.use(['laypage', 'layer', 'table', 'element'], function() {
    ...
    var tableIns=table.render({
        elem: '#assetsReceiveList',
        url: $("#contextPath").val()+'/lims/api/material/assetsReceiveList', //数据接口
        ...
        limits: [5, 7, 10, 20],
        limit: 5 //每页默认显示的数量
        ,id: 'assetsReceiveList'
    });
    //数据获取
    var active2 = {
        search: function () {
            var keywords=$("#keywords").val();
            table.reload('assetsReceiveList', {
                url: $("#contextPath").val()+'/lims/api/material/assetsReceiveList',
    ...
    table.on('tool(assetsReceiveList)', function(obj) { //注：tool 是工具条事件名，parenthead 是 table 原始容器的属性 lay-filter="对应的值"
        ...
        //打开查看物资名录页面
        if(obj.event === 'detail') {
            var realURL=contextPath + '/lims/api/material/checkAssetsReceiveDetailAPI?id='+data.id;
        ...
        //打开编辑物资申请页面
        if(obj.event === 'edit') {
            var realURL=contextPath + '/lims/api/material/editAssetsReceiveDetailAPI?id='+data.id;
            var index = layer.open({
                type: 2 //此处以iframe举例
                ,
                title: '编辑物资名录',
    ...
    //新建物资申领页面
    var active = {
        newAssetsReceive: function() {
            var realURL=contextPath + '/lims/api/material/editAssetsReceiveDetailAPI';
            //多窗口模式，层叠置顶
            var index = layer.open({
                type: 2 //此处以iframe举例
                ,
