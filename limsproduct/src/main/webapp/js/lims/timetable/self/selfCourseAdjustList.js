...
function newSelfCourse(selfId) {
    var url = contextPath + '/lims/timetable/self/newSelfCourse?id=-1&term=-1';
    if(selfId!=null){
        url = contextPath + '/lims/timetable/self/newSelfCourse?id='+ selfId +'&term=-1';
    }
    var index = layer.open({
        type: 2,
        title: '教学班管理',
...
function deleteTimetable(term, selfId) {
    if (confirm('是否确认删除？')) {
        $.ajax({
            url: zuulUrl + "api/timetable/manage/apiDeleteTimetableBySelfId?term=" + term + "&selfId=" + selfId,
            contentType: "application/json;charset=utf-8",
            dataType: "json",
            type: "post",
            headers:{Authorization: getJWTAuthority()},
...
function deleteTimetableSelfCourse(selfId) {
    if (confirm('是否确认删除？')) {
        $.ajax({
            url: zuulUrl + "api/timetable/self/apiDeleteTimetableSelfCourse?selfId=" + selfId,
            contentType: "application/json;charset=utf-8",
            dataType: "json",
            type: "post",
            headers:{Authorization: getJWTAuthority()},
        ...
        columns: [{
            ...
            formatter: function (value, row, index) {
                ...
                if (row.timetableStatus == 1) {
                    ...
                    if (row.baseActionAuthDTO.addActionAuth) {
                        // result += "<table><tr><td height=\"25px\"><div style=\"height:20px;\"><a href='javascript:;' class='btn btn-xs green' title='编辑'  onclick=\"newSelfReNoGroupCourse(" + row.termId + ",'" + row.selfId + "')\" ><span class='glyphicon glyphicon-plus'>不分批排</span></a>&nbsp;";
                        // result += "<a href='javascript:;' class='btn btn-xs green' title='编辑'  onclick=\"newSelfReGroupCourse(" + row.termId + ",'" + row.selfId + "')\" ><span class='glyphicon glyphicon-plus'>分批排课</span></a>&nbsp;</td></tr></table>";
                    }
                } else if (row.timetableStatus == 2) {
                    if (row.baseActionAuthDTO.publicActionAuth) {
...
function auditTimetable(termId, courseNo) {
    var index = layer.open({
        ...
        shadeClose: true,
        area: ['1100px', '500px'],
        content: contextPath + '/lims/timetable/course/auditTimetable?termId='+termId.toString()+'&courseNo=' + courseNo.toString()+'&type=2',
        end: function () {
            refreshBootstrapTable();
        }
    });
