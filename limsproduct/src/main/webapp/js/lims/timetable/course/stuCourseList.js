        ...
        columns: [{
            ...
            formatter: function (value, row, index) {
                ...
                for (var i = 0, len = timetableBatchDTOS.length; i < len; i++) {
                    ...
                    // 遍历组别
                    var gp = "<table border='1'><tr><td>组别</td><td>课程安排</td><td>已选/容量</td><td>操作</td></tr>";
                    for (var j=0; j < timetableBatchDTOS[i].timetableGroupDTOs.length; j++) {
                        ...
                        gp += "<tr><td>"+timetableBatchDTOS[i].timetableGroupDTOs[j].groupName+"</td><td>"+ ti +"</td>" +
...
function dropGroupStudent(groupId) {
    if (confirm('是否确认退选？')) {
        $.ajax({
            url: zuulUrl + "api/timetable/student/apiDeleteGroupStudent?groupId=" + groupId,
            contentType: "application/json;charset=utf-8",
            headers:{Authorization: getJWTAuthority()},
            async: false,
            dataType: "json",
