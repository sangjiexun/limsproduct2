...
$(document).ready(function () {
    ...
    urlCourse =zuulUrl+"api/school/apiEduSchoolCourseByPage";
...
function deleteTimetable(term, courseNo) {
    if (confirm('是否确认删除？')) {
        ...
        //getJWTAuthority();
        $.ajax({
            url: zuulUrl + "api/timetable/manage/apiDeleteTimetableByCourseNo?term=" + term + "&courseNo=" + courseNo,
            contentType: "application/json;charset=utf-8",
            headers:{Authorization: getJWTAuthority()},
            async: false,
            dataType: "json",
        ...
        columns: [{
            ...
            formatter: function (value, row, index) {
                ...
                if(row.timetableStatus==11){
                    // result =   "<a href='javascript:;' class='btn btn-xs green' title='查看'  onclick=\"publicTimetableForSelf("+row.timetableStyle+",'" + row.selfId + "',1)\" ><span class='glyphicon glyphicon-ok'>调课完成</span></a>&nbsp;";
        ...
        columns: [{
            ...
            formatter: function (value, row, index) {
                ...
                if(row.timetableStatus==11){
                    ...
                    // result =   "<a href='javascript:;' class='btn btn-xs green' title='查看'  onclick=\"publicTimetable("+row.timetableStyle+",'" + row.courseNo + "',1)\" ><span class='glyphicon glyphicon-ok'>调课完成</span></a>&nbsp;";
                }
                if(row.timetableStatus==12){
                    var auditButtonStr = "点击审核";
                    if(authName == 'ROLE_TEACHER'){
                ...
                if(row.timetableStatus==13){// 临时增加，调停课审核服务走通之后弃用
                    result =   "调课完成</span></a>&nbsp;";
                }
                return result;
            }
