...
$(document).ready(function () {
    ...
    $('#weekday').select2({
        ...
        placeholder: '请选择星期...',
        ajax: {
            url: contextPath + "/lims/api/timetable/common/apiWeekDayAdjustTimetableBySelect",
            dataType: "json",
            delay: 250,//延时0.5秒之后再进行请求
            type: "post",
            data: function (params) {
    ...
    $('#classes').select2({
        ...
        placeholder: '请选择节次...',
        ajax: {
            url: contextPath + "/lims/api/timetable/common/apiClassesAdjustTimetableBySelect",
            dataType: "json",
            delay: 250,//延时0.5秒之后再进行请求
            type: "post",
            data: function (params) {
    ...
    */
    $("#submitButton").on('click', function () {
        if (validform().form()) {
            ...
            $.ajax({
                url: contextPath + "/lims/api/school/apiSaveTimetableAppointmentByEduAdjustCourse",
            ...
            $("#weeks").val("").change();
            if(confirm("保存完成，是否回到教务排课，取消则继续进行排课")){
                var index = parent.layer.getFrameIndex(window.name);
                parent.layer.close(index);
            }
        } else {
...
function doEduReCourse(sameNumberId) {
    ...
    var index = layer.open({
        ...
        shadeClose: true,
        area: ['1100px', '500px'],
        content: contextPath + '/lims/timetable/engineer/educourse/updateEduAdjustCourse?currpage=1&flag=0&timetableStyle=5&courseNo=' + courseNo + "&term=" + term
