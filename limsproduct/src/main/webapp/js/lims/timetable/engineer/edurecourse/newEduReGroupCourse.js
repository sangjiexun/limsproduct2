...
var url = contextPath + "/lims/api/timetable/manage/apiTimetableBatchList";
    ...
    var TableInit = function () {
        ...
        oTableInit.Init = function () {
            $("#table_list").bootstrapTable({
                ...
                //设置为limit可以获取limit, offfset, search, sort, order
                ...
                formatter: function (value, row, index) {
                    ...
                    if(timetableDTOs.length>0){
                        ...
                        for (var i = 0, len = timetableDTOs.length; i < len; i++) {
                            ...
                            var result = "<button class='btn'  onclick='doEduReGroupCourse(" + timetableDTOs[i].groupId + ',\"' + courseNo +"\","+batchId+  ","+timetableDTOs[i].sameNumberId+")' ><span class='glyphicon glyphicon-edit'>编辑</span></button>&nbsp;";
                            result += "<button class='btn'  onclick='deleteTimetable(" + timetableDTOs[i].sameNumberId + ","+batchId+")' ><span class='glyphicon glyphicon-remove'>删除</span></button>&nbsp;";
...
function doEduReGroupCourse(groupId, courseNo,batchId,sameNumberId) {
    ...
    var index = layer.open({
        ...
        shadeClose: true,
        area: ['1100px', '500px'],
        content: contextPath + '/lims/timetable/engineer/edurecourse/newEduReGroupTimetableCourse?currpage=1&flag=0&timetableStyle=4&courseNo=' + courseNo + "&term=" + term + "&groupId=" + groupId
...
function chooseCopyTimetableGroup(id, courseNo, batchId) {
    ...
    var index = layer.open({
        ...
        shadeClose: true,
        area: ['1100px', '500px'],
        content: contextPath + '/lims/timetable/engineer/edurecourse/chooseCopyTimetableGroup?id=' + id.toString() +
