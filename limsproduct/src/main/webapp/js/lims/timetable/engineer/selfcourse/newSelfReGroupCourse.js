...
var url = contextPath + "/lims/api/timetable/manage/apiSelfTimetableBatchList";
var urlChild = contextPath + "/lims/api/timetable/manage/apiTimetableGroupList";
$(document).ready(function () {
        ...
        oTableInit.Init = function () {
            $("#table_list").bootstrapTable({
                ...
                onEditableSave: function (field, row, oldValue, $el) {
                    ...
                    $.ajax({
                        url: contextPath + "/lims/api/timetable/manage/apiUpdateTimetableBatch",
                        type: "post",
                        data: {
                            "batchId":row.id,
                            "startDate":startDate,
   ...
   //父子表
            ...
            columns: [{
                ...
                formatter: function (value, row, index) {
                    ...
                    if (row.baseActionAuthDTO.updateActionAuth) {
                        if (row.groupStudentNumbers > 0) {
                            result += "<button  id='" + group_button_reality + "' class='btn btn-xs green' onclick=\"setTimetableGroupNumbersReality(" + selfId + "," + row.id +","+ row.groupStudentNumbers+")\" title='编辑' ><span class='glyphicon glyphicon'>" + row.groupStudentNumbers + "人(可维护)</span></button>&nbsp;";
                        } else {
                ...
                formatter: function (value, row, index) {
                    ...
                    if(timetableDTOs.length>0){
                        ...
                        for (var i = 0, len = timetableDTOs.length; i < len; i++) {
                            ...
                            var result = "<button  onclick='doSelfReGroupCourse(" + timetableDTOs[i].groupId + ","+batchId+  ","+timetableDTOs[i].sameNumberId+")' ><span class='glyphicon glyphicon-edit'>编辑</span></button>&nbsp;";
                ...
                formatter: function (value, row, index) {
                    ...
                    if (row.baseActionAuthDTO.addActionAuth){
                        result += "&nbsp;<button class='btn btn-xs green' title='编辑'  onclick=\"doSelfReGroupCourse(" + row.id + "," + batchId + ",-1)\" ><span class='glyphicon glyphicon-plus'>排课</span></button>&nbsp;";
                    }
    ...
    $("#submitButton").on('click', function () {
        ...
        $.ajax({
            url: contextPath + "/lims/api/timetable/manage/apiSaveTimetableBatch",
...
function deleteTimetableBatch(batchId) {
    if (confirm('是否确认删除？')) {
        $.ajax({
            url: contextPath + "/lims/api/timetable/manage/apiDeleteTimetableBatch?batchId=" + batchId,
            contentType: "application/json;charset=utf-8",
            dataType: "json",
            type: "post",
            async: false,
...
function saveTimetableGroupNumbers(groupId, numbers) {
    ...
    if (confirm('是否确认保存？')) {
        $.ajax({
            url: contextPath + "/lims/api/timetable/manage/saveTimetableGroupNumbers?groupId=" + groupId + "&numbers=" + numbers,
            contentType: "application/json;charset=utf-8",
            dataType: "json",
            type: "post",
            async: false,
...
//设置分组实际名单
function setTimetableGroupNumbersReality(selfId, groupId,groupStudentNumbers) {
    //变量：学生名单复选框的id和name
...
function deleteTimetableGroup(groupId,batchId) {
    if (confirm('是否确认删除？')) {
        $.ajax({
            url: contextPath + "/lims/api/timetable/manage/apiDeleteTimetableGroup?groupId=" + groupId,
            contentType: "application/json;charset=utf-8",
            dataType: "json",
            type: "post",
            async: false,
...
function saveTimetableGroup(batchId) {
    if (confirm('是否确认添加？')) {
        $.ajax({
            url: contextPath + "/lims/api/timetable/manage/apiSaveTimetableGroup?batchId=" + batchId,
            contentType: "application/json;charset=utf-8",
            dataType: "json",
            type: "post",
            async: false,
...
function saveTimetableGroupNumbersReality(groupId,groupStudentNumbers) {
    ...
    if (confirm('是否确认保存？')) {
        ...
        $.ajax({
            url: contextPath + "/lims/api/timetable/manage/saveTimetableGroupNumbersReality?groupId="+groupId+"&usernames="+selectUsername,
            contentType: "application/json;charset=utf-8",
            dataType: "json",
            type: "post",
            async: false,
...
*二次不分批排课弹出窗口
*/
function doSelfReGroupCourse(groupId, batchId,sameNumberId) {
    term = $("#term").val();
    selfId = $("#selfId").val();
    var index = layer.open({
        ...
        shadeClose: true,
        area: ['1100px', '500px'],
        content: contextPath + '/lims/timetable/engineer/selfcourse/newSelfReGroupTimetableCourse?currpage=1&flag=0&timetableStyle=6&selfId=' + selfId + "&term=" + term + "&groupId=" + groupId
